...
u32 pinSet[3][8] = {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K8AAYX_MIPI_YUV,currSensorName)))
	{
		...
		mdelay(50);
			...
			{
		PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_S5K3H2YX_MIPI_RAW \n");
		PK_DBG("[ON_general 2.8V]pinSetIdx:%d \n",pinSetIdx);
    ...
    else
    {
		...
		{
			PK_DBG("++++kdCISModulePower--off get in---SENSOR_DRVNAME_S5K3H2YX_MIPI_RAW \n");
			PK_DBG("++++kdCISModulePower--off get in---pinSetIdx=%d \n", pinSetIdx);
			...
			if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name))//lct.wangxun
			{
				PK_DBG("[CAMERA SENSOR] Fail to OFF digital power\n");
				//return -EIO;
				goto _kdCISModulePowerOn_exit_;
