...
#if 0
static void msdc_key_unlock()
{
	MSDC_WRITE8(EN18IOKEY_BASE,0x58);
	MSDC_WRITE8(EN18IOKEY_BASE,0xfa);
	MSDC_WRITE8(EN18IOKEY_BASE,0x65);
	MSDC_WRITE8(EN18IOKEY_BASE,0x83);
}
static void msdc_key_lock()
{
	MSDC_WRITE8(EN18IOKEY_BASE,0x0);
	MSDC_WRITE8(EN18IOKEY_BASE,0x0);
	MSDC_WRITE8(EN18IOKEY_BASE,0x0);
	MSDC_WRITE8(EN18IOKEY_BASE,0x0);
}

static void msdc_set_enio18(struct mmc_host *host,u32 v18)
{
	switch(host->id){
		case 1:
			msdc_key_unlock();
			if(v18)
				MSDC_SET_FIELD(MSDC_EN18IO_SW_BASE,MSDC1_EN18IO_SW,1);
			else
				MSDC_SET_FIELD(MSDC_EN18IO_SW_BASE,MSDC1_EN18IO_SW,0);
			msdc_key_lock();
			break;
		case 2:
			msdc_key_unlock();
			if(v18)
				MSDC_SET_FIELD(MSDC_EN18IO_SW_BASE,MSDC2_EN18IO_SW,1);
			else
				MSDC_SET_FIELD(MSDC_EN18IO_SW_BASE,MSDC2_EN18IO_SW,0);
			msdc_key_lock();
			break;
		default:
			break;
	...
	switch(id){
		case 1:
			...
			if(power_domain == MSDC_VMC)
				...
				msdc_en18io_sel = 1;
			msdc_key_unlock();
			MSDC_SET_FIELD(MSDC_EN18IO_CMP_SEL_BASE,MSDC1_EN18IO_CMP_EN,0x1);
			MSDC_SET_FIELD(MSDC_EN18IO_CMP_SEL_BASE,MSDC1_EN18IO_SEL1,msdc_en18io_sel);
			msdc_key_lock();
			break;
		case 2:
			power_domain = MSDC_POWER_MC2;
			if(power_domain == MSDC_VGP)
				msdc_en18io_sel = 0;
			else
				msdc_en18io_sel = 1;
			msdc_key_unlock();
			MSDC_SET_FIELD(MSDC_EN18IO_CMP_SEL_BASE,MSDC2_EN18IO_CMP_EN,0x1);
			MSDC_SET_FIELD(MSDC_EN18IO_CMP_SEL_BASE,MSDC2_EN18IO_SEL,msdc_en18io_sel);
			msdc_key_lock();
			break;
		default:
			break;
...
static void msdc_set_smt(struct mmc_host *host,int set_smt)
{
	switch(host->id){
		...
		case 1:
			...
			break;
		case 2:
			MSDC_SET_FIELD_DISCRETE(MSDC2_SMT_BASE1,MSDC2_SMT_CLK,set_smt);
			MSDC_SET_FIELD_DISCRETE(MSDC2_SMT_BASE1,MSDC2_SMT_CMD,set_smt);
			MSDC_SET_FIELD_DISCRETE(MSDC2_SMT_BASE1,MSDC2_SMT_DAT1_0,set_smt);
			MSDC_SET_FIELD_DISCRETE(MSDC2_SMT_BASE2,MSDC2_SMT_DAT2_3,set_smt);
			break;
		case 3:
...
void msdc_set_host_level_pwr(int id, int level)
{
    unsigned int ret = 0;

    if (id == 1) {
        if (level) {
            ret = pmic_config_interface(0xAF,0x21,0x71,0); /* VGP=1.8V */
        } else {
            ret = pmic_config_interface(0xAF,0x71,0x71,0); /* VGP=3.3V */
        }
        if (ret != 0) {
            print("PMIC: Set MSDC Vol Level Fail\n");
        } else {
...
void msdc_set_host_pwr(int id, int on)
{
    unsigned int ret = 0;

    if (id == 1) {
        /* MSDC1: Vcore:VMCH, Vio: VGP
         * VMCH/VGP [6:4]=7h (3.3V), [0]=1 (on), 0 (off) 
         */
        if (on) {
            ret = pmic_config_interface(0xAF,0x71,0x71,0); /* 3.3V VGP_EN=1 */
        } else {
            ret = pmic_config_interface(0xAF,0x70,0x71,0); /* 3.3V VGP_EN=0 */
        }    
        if (ret != 0) {
            print("PMIC: Set MSDC Host Power Fail\n");
        } else {
            mdelay(3);
        }
    }
...
void msdc_set_card_pwr(int id, int on)
{
    ...
    if (id == 0) {
        /* MSDC0: Vcore:VMC, Vio:VMC
         * VMC [6:4]=7h (3.3V), [0]=1 (on), 0 (off) 
         */
        if (on) {
            ret = pmic_config_interface(0xA7,0x71,0x71,0); /* 3.3V VMC_EN=1 */
        } else {
            ret = pmic_config_interface(0xA7,0x70,0x71,0); /* 3.3V VMC_EN=0 */
        }       
    } else {
        /* MSDC1: Vcore:VMCH, Vio: VGP
         * VMCH/VGP [6:4]=7h (3.3V), [0]=1 (on), 0 (off) 
         */
        if (on) {
            ret = pmic_config_interface(0xAB,0x71,0x1,0); /* 3.3V VMCH_EN=1 */
        } else {
            ret = pmic_config_interface(0xAB,0x70,0x1,0); /* 3.3V VMCH_EN=0 */
        }
    }
    
    if (ret != 0) {
        print("PMIC: Set MSDC Card Power Fail\n");
    } else {
        mdelay(3);    
    }
}
...
#endif
#if 0 // PLL manager will set MT6588 MSDC default clock source . CHECK ME
...
static void msdc_pin_pud(struct mmc_host *host, int mode)
{
	switch(host->id){
		case 0:
			...
			break;
		case 1:
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_POLARITY_BASE,MSDC1_PUPD_CMD,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_ENABLE_BASE,MSDC1_PUPD_CMD,1);
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_POLARITY_BASE,MSDC1_PUPD_DAT,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_ENABLE_BASE,MSDC1_PUPD_DAT,1);
			break;
		case 2:
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_POLARITY_BASE1,MSDC2_PUPD_CMD,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE1,MSDC2_PUPD_CMD,1);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_POLARITY_BASE1,MSDC2_PUPD_DAT1_0,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_POLARITY_BASE2,MSDC2_PUPD_DAT2_3,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE1,MSDC2_PUPD_DAT1_0,1);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE2,MSDC2_PUPD_DAT2_3,1);
			break;
		case 3:
...
static void msdc_pin_pnul(struct mmc_host *host, int mode)
{
	switch(host->id){
		case 1:
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_ENABLE_BASE,MSDC1_PUPD_CMD,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC1_PUPD_ENABLE_BASE,MSDC1_PUPD_DAT,mode);
			break;
		case 2:
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE1,MSDC2_PUPD_CMD,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE1,MSDC2_PUPD_DAT1_0,mode);
			MSDC_SET_FIELD_DISCRETE(MSDC2_PUPD_ENABLE_BASE2,MSDC2_PUPD_DAT2_3,mode);
			break;
		default:
			break;
		}
