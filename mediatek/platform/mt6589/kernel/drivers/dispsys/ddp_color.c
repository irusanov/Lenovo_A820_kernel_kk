...
} color_proc_node_struct;

static long color_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    int ret = 0;
...
static int color_proc_open(struct inode *inode, struct file *file)
{
    color_proc_node_struct *pNode = NULL;

    DISP_DBG("enter color_proc_open() process:%s\n", current->comm);

    //Allocate and initialize private data
    file->private_data = kmalloc(sizeof(color_proc_node_struct), GFP_ATOMIC);
    ...
    pNode = (color_proc_node_struct *)file->private_data;
...
static int color_proc_release(struct inode *inode, struct file *file)
{
    DISP_MSG("enter color_proc_release() process:%s\n", current->comm);
...
static ssize_t color_proc_read(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
    return 0;
}

static int color_proc_flush(struct file * file , fl_owner_t a_id)
{
    return 0;
}

static struct file_operations color_proc_fops = {
    .owner      = THIS_MODULE,
    .open		= color_proc_open,
    .read       = color_proc_read,
    .flush      = color_proc_flush,
	.release	= color_proc_release,
    .unlocked_ioctl = color_proc_unlocked_ioctl,
};
...
void ddp_color_init(void)
{
    ...
    pEntry = create_proc_entry("mtk_mdp_color", 0444, NULL);
    if (NULL != pEntry)
    {
        pEntry->proc_fops = &color_proc_fops;
    }
}


static int __init color_mdp_init(void)
{
    return 0;
}

static void __exit color_mdp_exit(void)
{
    return;
}

module_init(color_mdp_init);
module_exit(color_mdp_exit);
