...
static unsigned int g_is_fb_secure_layer = 0;
static unsigned int cnt_security_layer = 0;
#endif
...
static int cnt=3;
static int _mtkfb_get_3d_offset(UINT32 layer_type, bool *r_1st, bool *landscape,
...
//    LCD_WaitForNotBusy();
    ...
    LCD_CHECK_RET(LCD_LayerSetWindowOffset(FB_LAYER, 0, 0));
    LCD_LayerSetSize(FB_LAYER,var->xres,var->yres);
	...
	if (mtkfb_current_layer_type != mtkfb_using_layer_type)
	{
		...
		{
            LCD_CHECK_RET(LCD_Layer3D_Prepare(FB_LAYER, &new_id_offset,FALSE));
            LCD_CHECK_RET(LCD_LayerSet3D(FB_LAYER, 0, 0, 0));
            LCD_CHECK_RET(LCD_LayerSet3D((FB_LAYER+new_id_offset), 0, 0, 0));

			if (!dal_shown)
				LCD_CHECK_RET(LCD_LayerEnable((FB_LAYER+new_id_offset), 0));
            else
            {
		...
		else
		{
            ...
            if(new_id_offset < 0)//keep first half 3D frame in FB_LAYER(right), second in FB_LAYER-1(left)
            {
                if(r_1st == 1)
                    r_1st = 0;
                else
    ...
    else if (mtkfb_using_layer_type != LAYER_2D)//also set layer2 addr
    {
        bool r_1st = 0, landscape = 0;
		UINT32 offset_x, offset_y;
        UINT32 width, height;
...
#if defined (MTK_WFD_SUPPORT)
static int mtkfb_wfd_secure_output_stub(void)
{
	printk("mtkfb_wfd_secure_output_stub, g_is_fb_secure_layer=%d, cnt_security_layer=%d\n", g_is_fb_secure_layer, cnt_security_layer);
	if(g_is_fb_secure_layer || cnt_security_layer)
	{
		//MTK_WFD_Set_Security_Output(1);
	}
	else
	{
		//MTK_WFD_Set_Security_Output(0);
	}
    ...
    cached_layer_config[id].security = layerInfo->security;
{
    ...
    if (layerInfo->src_phy_addr != NULL) {
    	...
    	cached_layer_config[id].addr = disp_sync_query_buffer_mva(layerInfo->layer_id, (unsigned int)layerInfo->next_buff_idx);
    	if(cached_layer_config[id].addr==0 && fbdev->layer_enable==1)
    	{
            MTKFB_ERR("error, mtk_disp query buffer mva==0!, layer=%d, next_buff_idx=%d enable=%d \n",
                layerInfo->layer_id, (unsigned int)layerInfo->next_buff_idx, fbdev->layer_enable);
    	}
    }
#else
    cached_layer_config[id].addr = (unsigned int)layerInfo->src_phy_addr;
	...
	if(FB_LAYER != id && (FB_LAYER + 1) != id){
		...
		else
		{
		    ...
		    if(layer_addr == 0)
		    {
                MTKFB_ERR("layer enable but layer_addr=searched_node->src_mva=0 \n");
		    }
		}
	}
	else{
...
static int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case MTKFB_SET_FB_LAYER_SECURE:
	{
		...
		{
		//printk("MTKFB_SET_FB_LAYER_SECURE, g_is_fb_secure_layer=%d\n", t);
...
////////////////////////////////////////////FM De-sense
	...
	case MTKFB_FBLAYER_ENABLE:
	{
		...
		if (mtkfb_using_layer_type != LAYER_2D)
			LCD_CHECK_RET(LCD_LayerEnable(FB_LAYER&0x1 ? FB_LAYER-1 : FB_LAYER+1 , enable));
        atomic_set(&OverlaySettingDirtyFlag, 1);
        atomic_set(&OverlaySettingApplied, 0);
        mutex_unlock(&OverlaySettingMutex);
...
////////////////////////////////////////////////
    default:
        printk("[FB Driver] error: mtkfb_ioctl Not support, info=0x%08x, cmd=0x%08x, arg=0x%08x, num=%d\n", (unsigned int)info, (unsigned int)cmd, (unsigned int)arg,  _IOC_NR(cmd));
        return -EINVAL;
    }
}
...
void mtkfb_disable_non_fb_layer(void)
{
    ...
    if (dirty)
    {
/*remove this for it will clear AEE layer after system server reboot
