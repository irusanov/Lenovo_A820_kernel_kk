...
static struct task_struct *detect_alive_task = NULL;
...
static int _DISP_CleanUpKThread(void *data) {
	...
	while(1) {
		...
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			layerId = realtime_layer_config[i].layer;
			if(isAEEEnabled && (layerId == DISP_DEFAULT_UI_LAYER_ID)) {
				DISP_LOG("aee on\n");
				disp_sync_release(layerId);
			} else {
				if(realtime_layer_config[i].layer_en) {
					fence = realtime_layer_config[i].buff_idx - 1;
				} else {
					fence = realtime_layer_config[i].buff_idx;
				}
				if(fence > 0) {
					disp_sync_signal_fence(layerId, fence);
					disp_sync_release_buffer(layerId);
				}
			}
		}
	}
...
//	LCD_CHECK_RET(LCD_Init());
	...
	dispif_info[MTKFB_DISPIF_PRIMARY_LCD].lcmOriginalWidth = dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayWidth;
	dispif_info[MTKFB_DISPIF_PRIMARY_LCD].lcmOriginalHeight = dispif_info[MTKFB_DISPIF_PRIMARY_LCD].displayHeight;
...
static void _DISP_DumpLayer(OVL_CONFIG_STRUCT* pLayer)
{
  ...
  #endif
                {
                	m4u_mva_map_kernel(pLayer->addr, meta.size, 0, (unsigned int*)&meta.pData, &meta.size);
                    MMProfileLogMeta(MTKFB_MMP_Events.Layer[pLayer->layer], MMProfileFlagPulse, &meta);
...
static int _DISP_DetectAliveKThread(void *data)
{
	unsigned int vsync_timeout_cnt = 0;
	unsigned int disp_thread_timeout_cnt = 0;
	struct sched_param param = { .sched_priority = RTPM_PRIO_SCRN_UPDATE };
	sched_setscheduler(current, SCHED_RR, &param);
	while(1)
	{
		...
		if (!is_early_suspended){
			...
			if(DISP_THREAD_TIMEOUT < ((unsigned int)disp_thread_cost_time/1000)){
				printk("dete_hang:%lld,%lld,%lld\n",disp_thread_start_time, sched_clock(),disp_thread_cost_time);
				disp_thread_timeout_cnt++;
			}
			else
				disp_thread_timeout_cnt = 0;
			
			DISP_LOG("[disp detect thread]vsync = %d, disp_thread = %d, disp_thread_cost = %lld ns\n", vsync_timeout_cnt,disp_thread_timeout_cnt,disp_thread_cost_time);			
			////wait Vsync to check if vsync is alive?
			vsync_wq_flag = 0;
			...
			if(vsync_timeout_cnt>3 || disp_thread_timeout_cnt>3){
				printk("[ERROR, display detect hang], vsync timeout = %d, SW thread timeout = %d\n",vsync_timeout_cnt,disp_thread_timeout_cnt);	
...
void DISP_InitVSYNC(unsigned int vsync_interval)
{
    ...
    if (IS_ERR(detect_alive_task)) 
    {
        DISP_LOG("DISP_InitVSYNC(): Cannot create disp_detect_alive_kthread\n");
        return;
    }
