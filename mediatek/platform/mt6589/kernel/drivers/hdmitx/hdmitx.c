...
static int last_scaling_factor = 0;
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
    ...
    if (enable)
    {
        ...
        config.srcModule = HDMI_SRC_RDMA;
...
static void dpi_setting_res(u8 arg)
{
	...
	last_scaling_factor = p->scaling_factor;
...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        {
            HDMI_LOG("video resolution configuration, arg=0x%x, %d\n", arg, hdmi_reschange);
			p->scaling_factor = (arg >> 8) & 0xFF;
			HDMI_LOG("video resolution configuration, scale_factor: %d, last_scaling_factor: %d\n", p->scaling_factor, last_scaling_factor);

            RETIF(!p->is_enabled, 0);
            RETIF(IS_HDMI_NOT_ON(), 0);

            if ((hdmi_reschange == (arg & 0xFF)) && (last_scaling_factor == p->scaling_factor))
            {
                HDMI_LOG("hdmi_reschange=%ld\n", hdmi_reschange);
                break;
            }

            hdmi_reschange = arg & 0xFF;
            p->is_clock_on = false;
            atomic_set(&hdmi_rdma_update_event, 1);
            wake_up_interruptible(&hdmi_rdma_update_wq);
            int tmp = 0;
            ...
            if (hdmi_bufferdump_on > 0)
            {
                MMProfileLogEx(HDMI_MMP_Events.HDMIState, MMProfileFlagStart, ResChange, arg & 0xFF);
            }
            ...
            hdmi_config_pll(arg & 0xFF);
            dpi_setting_res((u8)(arg & 0xFF));
        ...
        case MTK_HDMI_GET_DEV_INFO:
        {
			...
			HDMI_LOG("DEV_INFO configuration get displayType-%d , %d, %d\n", hdmi_info.displayType, p->hdmi_height, p->hdmi_width);
        ...
        case MTK_HDMI_GET_CAPABILITY:
        {
            ...
            HDMI_LOG("[hdmi][HDMI] query_type %x\n", query_type);
            if( query_type == HDMI_RESIZE_SUPPORT)
                query_type = true;
            else 
                r = -EFAULT;
