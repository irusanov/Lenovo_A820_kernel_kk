...
static int spi_setup_xfer(struct spi_transfer *xfer, u32 len, u32 flag)
{
...
static int threadfunc3(void *data)
{
	...
	while(1){
		...
		for(i=0;i<SPI_STRESS_MAX;i++){
			spi_message_init(p);
			ret = spi_setup_xfer(&stress_xfer[i],len,0);
			if(ret != 0){									
...
static int threadfunc4(void *data)
{
	...
	while(1){
		...
		for(i=0;i<SPI_STRESS_MAX;i++){			
			ret = spi_setup_xfer(&stress_xfer_con[i],len,1);
			if(ret != 0){									
				...
				const char *buf, size_t count)
{
	...
	if( !strncmp ( buf, "-h", 2 ) ) {
		...
		spi_setup_xfer(&transfer,32,0);
		spi_message_add_tail(&transfer, &msg);
		spi_setup_xfer(&transfer2,1024,0);
		spi_message_add_tail(&transfer2, &msg);
		spi_setup_xfer(&transfer3,32,0);
	...
	//			xfer = (struct spi_transfer *)kzalloc(SPI_STRESS_MAX*sizeof( struct spi_transfer) , GFP_KERNEL);
			...
			for(i=0;i<SPI_STRESS_MAX;i++){			
				ret = spi_setup_xfer(&stress_xfer[i],32,0);
				if(ret != 0){									
					SPIDEV_LOG("Message set up err:%d\n", ret);
				}
...
static DEVICE_ATTR(spi, 440, NULL, spi_store);
