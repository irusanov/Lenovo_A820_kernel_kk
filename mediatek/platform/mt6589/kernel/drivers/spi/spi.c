...
void spi_check_mpu_violation(u32 addr, int wr_vio)
{
    printk(KERN_CRIT "SPI checks EMI MPU violation.\n");
    printk(KERN_CRIT "addr = 0x%x, %s violation.\n", addr, wr_vio? "Write": "Read");
    printk(KERN_CRIT "DMA SRC = 0x%x.\n", __raw_readl(SPI1_BASE + SPI_TX_SRC_REG));
    printk(KERN_CRIT "DMA DST = 0x%x.\n", __raw_readl(SPI1_BASE + SPI_RX_DST_REG));
    printk(KERN_CRIT "DMA COUNT = 0x%x.\n", __raw_readl(SPI1_BASE + SPI_CFG1_REG));
    printk(KERN_CRIT "DMA CON = 0x%x.\n", __raw_readl(SPI1_BASE + SPI_CMD_REG));
}
static int __init mt_spi_init ( void )
{
	int ret;
	
	/*register EMI MPU call back funtion*/	
	emi_mpu_notifier_register(MST_ID_MMPERI_1, spi_check_mpu_violation);
