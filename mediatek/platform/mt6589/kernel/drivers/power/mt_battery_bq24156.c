...
int g_pmic_init_for_bq24156 = 0;

/* 1: enable BATDRV_log */
int Enable_BATDRV_LOG = 0;

void pchr_turn_off_charging_bq24156 (void);
void pchr_turn_on_charging_bq24156 (void);
...
const unsigned int BATTERY_DRIVER_GPT = GPT5;

const char POWER_BATTERY_TAG[] = "Power/Battery";
...
/**********************************************************
  ...
  *
  *********************************************************/
const int BQ24156_STAT_READY = 0x0;
const int BQ24156_STAT_CHARGING = 0x1;
const int BQ24156_STAT_CHARGE_DONE = 0x2;
const int BQ24156_STAT_FAULT = 0x3;

///////////////////////////////////////////////////////////////////////////////////////////
//// Thermal related flags
///////////////////////////////////////////////////////////////////////////////////////////
/* constant value */
#define BQ24156_TT_NOTHING                  0
#define BQ24156_TT_EN_BTT_CHRTIMER  1
#define BQ24156_TT_DIS_BTT_CHRTIMER 2
#define BQ24156_TT_EN_ONLY_BTT          3

int g_battery_thermal_throttling_flag = BQ24156_TT_EN_ONLY_BTT; // 0:nothing, 1:enable batTT&chrTimer, 2:disable batTT&chrTimer, 3:enable batTT, disable chrTimer
...
const int BQ24156_TT_SHUTDOWN_THREAHOLD = 60;
...
/* 1: over voltage test 
    ...
    3: over current test
    4: battery over voltage test
    5: charging over time test
   */
...
void BAT_ResetBMTStatusChargingTime(void);
...
{
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG, "[BATTERY] %s\r\n", __FUNCTION__);
    }
...
cust_charging_current_enum g_temp_CC_value = Cust_CC_450MA;
...
int bat_thread_control(int start) //for psensor firmware download in factory mode
{
    ...
    else
    {
        ...
        pchr_turn_off_charging_bq24156();
    }
    return 1;
}
        ...
        if ( BMT_status.bat_exist )                
        {
            ...
            else 
            {
                ...
                else
                {
                    ...
                    {   
                        /* enter full charge tracking mode, keep UI display with 99% */
                        if(bat_volt_check_point >= 100 )
                        {                    
...
/* bcct support, ref changelist #1114696 */
extern void bq24156_set_ac_current(void);
int g_bcct_flag = 0;
int g_bcct_value = 0;
...
int get_bat_charging_current_level(void)
{
    kal_uint8 ret_val=0;
    kal_uint8 low_chg = 0;
    kal_uint8 input_limit = 0;
        
    //Get current level
    low_chg = bq24156_get_low_chg();
    input_limit = bq24156_get_input_charging_current();
    
    //Parsing
    if (input_limit == 0x0) return 100;
    else if (low_chg == 0x1) return 325;
    else if (input_limit == 0x1) return 500;            
...
{
    printk("[BATTERY:bq24156] set_bat_charging_current_limit (%d)\r\n", current_limit);
...
void ChargerHwInit_bq24156(void)
{
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY] %s\n", __FUNCTION__);
    }
    ...
    bq24156_set_iterm(0x3);     /* Termination current 200mA */
}

void bq24156_set_ac_current(void)
{
    kal_uint8 reg_set_value=0;
    kal_uint8 low_chg = 0;

    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] %s \r\n", __FUNCTION__);
    }    

    switch (g_temp_CC_value) {
        case Cust_CC_1600MA:
        ...
        case Cust_CC_1500MA:
            ...
            break;
        case Cust_CC_1400MA:
            reg_set_value = 0x8; //1350mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_1300MA:
            reg_set_value = 0x7; //1250mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_1200MA:
            reg_set_value = 0x6; //1150mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_1100MA:
            reg_set_value = 0x5; //1050mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_1000MA:
            reg_set_value = 0x4; //950mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_900MA:
            reg_set_value = 0x3; //850mA
            bq24156_set_input_charging_current(0x3); // no input limit
            break;
        case Cust_CC_800MA:
            reg_set_value = 0x2; //750mA
            bq24156_set_input_charging_current(0x2); // 800mA input limit
            break;
        case Cust_CC_700MA:
        case Cust_CC_650MA:
            reg_set_value = 0x1; //650mA
            bq24156_set_input_charging_current(0x2); // 800mA input limit
            break;
        case Cust_CC_550MA:
        ...
        case Cust_CC_70MA:
            ...
            break;
        case Cust_CC_0MA:
            xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG, "[BATTERY:bq24156] current is 0mA, disable charger\r\n");
            bq24156_set_ce(0x0);
            return;
            break;
        default:
            /* unknown current, disable charger */
            xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] unknown current(%d), disable charger\r\n", g_temp_CC_value);
            bq24156_set_ce(0x0);
            return;
            break;
    }
    ...
    {    
        /* JEITA spec is 200mA when T under 0C */
        reg_set_value= 0x0; //lowest possible charge current for bq24156 is 325mA
        low_chg = 1;
    }
#endif

    /* issue I2C command to BQ24156 */
    if (low_chg)
    {
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] LOW_CHG=1\n");
        bq24156_set_low_chg(0x1);
    }
    else
    {
        bq24156_set_low_chg(0x0);
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] reg_set_value=0x%x\n", reg_set_value);
        bq24156_set_icharge((kal_uint32)reg_set_value);
    }
}

void select_charging_current_bq24156(void)
{
    if (g_ftm_battery_flag) /* Factory Mode */
    {
        /* Factory Mode log always show */
        printk("[BATTERY:bq24156] FTM charging : %d\r\n", charging_level_data[0]);    
    ...
    {    
        if ( BMT_status.charger_type == STANDARD_HOST ) 
        {
            if (g_Support_USBIF == 1) /* follow BC 1.1 spec */
            {
                if (g_usb_state == USB_SUSPEND)
                {
                    g_temp_CC_value = USB_CHARGER_CURRENT_SUSPEND;
                    if (Enable_BATDRV_LOG == 1) {
                        xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG, "[BATTERY:bq24156] g_usb_state == %d, disable charging\r\n", g_usb_state);
                    }
                }
                else if (g_usb_state == USB_UNCONFIGURED)
                {
                    ...
                    if (Enable_BATDRV_LOG == 1) {
                        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:bq24156] 100mA() \r\n");
                    }
                }
                else if (g_usb_state == USB_CONFIGURED)
                {
                    ...
                    if (Enable_BATDRV_LOG == 1) {
                        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:bq24156] bq24156_set_ac_current(), CC value (%d) \r\n", g_temp_CC_value);
                    }
                }
                else
                {
            ...
            {    
                ...
                {
                    g_temp_CC_value = Cust_CC_500MA; //default current is 500MA    
                }                         
#else
                g_temp_CC_value = USB_CHARGER_CURRENT;
                if (Enable_BATDRV_LOG == 1) {
                    xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG,"[BATTERY:bq24156] use USB_CHARGER_CURRENT\r\n");    
                }            
#endif
            }
        } 
...
#endif
            if (Enable_BATDRV_LOG == 1) {
                xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] NONSTANDARD_CHARGER, use AC_CHARGER_CURRENT\r\n");
            }
        } 
        else if (BMT_status.charger_type == STANDARD_CHARGER) 
        {
            g_temp_CC_value = AC_CHARGER_CURRENT;
            if (Enable_BATDRV_LOG == 1) {
                xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] STANDARD_CHARGER, use AC_CHARGER_CURRENT\r\n");
            }
        }
        else if (BMT_status.charger_type == CHARGING_HOST) 
        {
            g_temp_CC_value = USB_CHARGER_CURRENT;
            if (Enable_BATDRV_LOG == 1) {
                xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156] CHARGING_HOST, use USB_CHARGER_CURRENT\r\n");
            }
        }
        else 
        {   /* unknown charger type, current under 500mA */
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24156] unknown charger type, current under 500mA \r\n");    
            }
            g_temp_CC_value = Cust_CC_450MA;
            bq24156_set_ac_current();
            return;
        ...
        case TEMP_NEG_10_TO_POS_0:
            bq24156_set_oreg(0x19);     /* 4.0V */
            break;
        
        case TEMP_POS_0_TO_POS_10:
        case TEMP_POS_45_TO_POS_60:
            bq24156_set_oreg(0x1E);     /* 4.1V */
            break;
        
        case TEMP_POS_10_TO_POS_45:
        default:
            if(g_enable_high_vbat_spec == 1)
            {
                bq24156_set_oreg(0x2B);     /* 4.36V */
...
{
#ifndef MTK_BQ24156_NO_CD_GPIO    
    /* enable charging process by GPIO */
    mt_set_gpio_mode(gpio_number,gpio_on_mode);  
    mt_set_gpio_dir(gpio_number,gpio_on_dir);
    mt_set_gpio_out(gpio_number,gpio_on_out);
#endif
    ...
    {
        printk("[BATTERY:bq24156] Charger Error, turn OFF charging !\r\n");
        pchr_turn_off_charging_bq24156();
    }
    else if( (get_boot_mode()==META_BOOT) || (get_boot_mode()==ADVMETA_BOOT) )
    {   
        printk("[BATTERY:bq24156] In meta or advanced meta mode, disable charging.\r\n");    
        pchr_turn_off_charging_bq24156();
    }
    else
    {
        ChargerHwInit_bq24156();
    
        if (Enable_BATDRV_LOG == 1) {
            printk("[BATTERY:bq24156] pchr_turn_on_charging !\r\n");
        }
        ...
        if (g_bcct_flag == 1)
        {
            ...
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24156] select_charging_curret_bcct !\n");
            }
        }
        else
        {
            select_charging_current_bq24156();
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24156] select_charging_curret_bq24156 !\n");
            }
        }
        ...
        {
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:bq24156] charger enable !\r\n");
            }
        }
        else
        {
            printk("[BATTERY:bq24156] wait gFG_booting_counter_I_FLAG==2 (%d)\r\n", gFG_booting_counter_I_FLAG);
        }    
    }
}
...
{ 
    ...
    {   
        ...
        BAT_ResetBMTStatusChargingTime();
...
int BAT_CheckBatteryStatus_bq24156(void)
{
    ...
    /* TODO: confirm schematic for power path different */
    /* Get V_BAT_SENSE */
    if (g_chr_event == 0) 
    {        
    ...
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY:ADC:bq24156] VCHR:%d BAT_SENSE:%d I_SENSE:%d TBAT:%d (%d)\n", 
    ...
    if( gForceADCsolution==1 )
    {
        ...
        /* update latest charging current */
        ...
        /* update latest SOC */
        if (BMT_status.bat_full || g_bat_full_user_view) 
        {
            BMT_status.SOC = 100;
        }        
    ...
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, 
            "[BATTERY:AVG:bq24156] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG == 1) {        
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:FG:bq24156] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n", 
           BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
    ...
    if(Enable_BATDRV_LOG == 1)
    {
        xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY] Battery Protection Check !!\n");
    }
    ...
    {
        printk(  "[BATTERY:bq24156] JEITA : fail\n");
...
#endif /* MTK_JEITA_STANDARD_SUPPORT */
        {
            if (Enable_BATDRV_LOG == 1) {
                xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG,
                    "[BATTERY:bq24156] Battery Re-charging !!BMT_status.bat_vol = %d\n\r", BMT_status.bat_vol);
            }
...
{    
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] Charging over %d hr stop !!\n\r", MAX_CHARGING_TIME);            
...
void mt_battery_notify_check(void)
{
    ...
    if(g_BN_TestMode == 0x0000)
    {
        ...
        {
            printk("[TestMode] Disable Safety Timer : no UI display\n");
        }
        else
        {
    ...
    else if(g_BN_TestMode == 0x0001) /* over voltage test */
    {
        g_BatteryNotifyCode = 0x0001;
...
{
    /* TODO: confirm with Hoping about HW */
#if 0
    ...
    if( baton_count >= 3)
    {
        ...
        {
            printk("[BATTERY] Battery is not exist, power off bq24156 and system (%d)\n", baton_count);
            pchr_turn_off_charging_bq24156();
            arch_reset(0,NULL);      
        }
    }
#else
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY] Disable check battery exist for SMT\n");
    }
#endif
}
...
//// bq24156
///////////////////////////////////////////////////////////////////////////////////////////
void pmic_init_for_bq24156(void)
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[pmic_init_for_bq24156] Start\n");
...
//    upmu_set_rg_buf_pwd_b(1);       //RG_BUF_PWD_B
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[pmic_init_for_bq24156] Done\n");
}
...
void BAT_thread_bq24156(void)
{        
    int BAT_status = 0;
    kal_uint32 bq24156_status=0;
    int i = 0;
    int chr_err_cnt = 0;
    int ret_val = 0;
    
#if defined (MTK_KERNEL_POWER_OFF_CHARGING)
    static kal_bool first_time = true;
#endif

    /* run full check first, reason? */
    if(boot_check_once==1)
    {
        if( upmu_is_chr_det() == KAL_TRUE )
        {
            ret_val=get_bat_sense_volt(1);
            if(ret_val > RECHARGING_VOLTAGE)
            {
                g_bat_full_user_view = KAL_TRUE;                
            }
    ...
    if (Enable_BATDRV_LOG == 1) {        
#if defined(MTK_JEITA_STANDARD_SUPPORT)        
        xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG, "[BATTERY_TOP] LOG. %d,%d,%d,%d,%d----------------------------\n", 
...
#endif /* defined(MTK_JEITA_STANDARD_SUPPORT) */
    }    

    /* check thermal throttling mechanism */
    /* only enable battery temperature check */
    if( (g_battery_thermal_throttling_flag == BQ24156_TT_EN_ONLY_BTT) ||
        (g_battery_thermal_throttling_flag == BQ24156_TT_EN_BTT_CHRTIMER) )
    {
        /* configure temperature? */
        if(battery_cmd_thermal_test_mode == 1){
            BMT_status.temperature = battery_cmd_thermal_test_mode_value;
            printk("[Battery:TestMode] In thermal_test_mode, Tbat=%d\n", BMT_status.temperature);
        }
    
#if defined(MTK_JEITA_STANDARD_SUPPORT)
        //ignore default rule        
#else    
        if(BMT_status.temperature >= BQ24156_TT_SHUTDOWN_THREAHOLD)
        {
#if defined(CONFIG_POWER_EXT)
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] CONFIG_POWER_EXT, no update mt6320_battery_update_power_down.\n");
#else
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery] Tbat(%d)>=%d, system need power down.\n",
                BMT_status.temperature, BQ24156_TT_SHUTDOWN_THREAHOLD);
    ...
    {   
        ...
        /* clean average method store value */
        for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {
    ...
    BAT_status = BAT_CheckBatteryStatus_bq24156();

    /* question: when charging_state is CHR_ERROR, BAT_status always return PMU_STATUS_FAIL,
    ...
    /* same code already in BAT_CheckBatteryStatus_bq24156(), try to remove it? */
    if(battery_cmd_thermal_test_mode == 1){
        BMT_status.temperature = battery_cmd_thermal_test_mode_value;
	...
	{
		first_time = false;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Not in kernel power off charging mode\n");
	}
	if(first_time)
	{	
		first_time = false;
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Config for MTK_KERNEL_POWER_OFF_CHARGING\n");
	}
    ...
    /* No Charger or Battery abnormal */
    ...
    else
    {
        ...
        if( (g_battery_thermal_throttling_flag == BQ24156_TT_DIS_BTT_CHRTIMER) ||
            (g_battery_thermal_throttling_flag == BQ24156_TT_EN_ONLY_BTT) )
        {
            xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG,
                "[BATTERY] Disable Safety Timer. g_battery_thermal_throttling_flag=%d, bat_thr_test_mode=%d, bat_thr_test_value=%d\n",
        ...
        bq24156_status = bq24156_get_chip_status();
        
        /* check battery full, 0x2 means charge done, p.28, table 3 */
        if( bq24156_status == BQ24156_STAT_CHARGE_DONE )
        {
#if defined(MTK_JEITA_STANDARD_SUPPORT)
...
#endif            
            {
                xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "[BATTERY:bq24156 Battery real full and disable charging (%d) \n", bq24156_status);
                BAT_BatteryFullAction();
                return;
            }
            else
            {
                /* for BQ24156 error handling */
                xlog_printk(ANDROID_LOG_DEBUG, POWER_BATTERY_TAG, "[BATTERY:bq24156] HW Battery real full ?? (%d,%d) \n", bq24156_status, BMT_status.bat_vol);
                pchr_turn_off_charging_bq24156();
                msleep(100);
            }
        }
        ...
        pchr_turn_on_charging_bq24156();        
        if (Enable_BATDRV_LOG == 1) {
            printk("[BATTERY:bq24156] Total charging timer=%ld, bq24156_status=%d \n", 
                BMT_status.total_charging_time, bq24156_status);
        }
    }
...
int bat_thread_kthread(void *x)
{
    ...
    pmic_init_for_bq24156();
    mutex_unlock(&bat_mutex);

	while(1)
    {
        if((g_bat_init_flag == 1) && (g_pmic_init_for_bq24156 == 1)) //make sure mt6320_battery_probe register finished
            break;
        else
            msleep(1000);
    }
...
void charger_hv_detect_sw_workaround_init(void)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "%s : done\n", __FUNCTION__);
...
static int mt6320_battery_probe(struct platform_device *dev)    
{
    ...
    /* question: wait BQ24156 init OK? */

    return 0;
}

#endif /* TODO: matching who? */
...
{    
    ...
    /* create device node for information passing */
    ...
    xlog_printk(ANDROID_LOG_INFO, POWER_BATTERY_TAG, "******** mtk_battery_cmd!! ********\n" );
...
MODULE_DESCRIPTION("MT6320 Battery Device Driver withBQ24156");
MODULE_LICENSE("GPL");
