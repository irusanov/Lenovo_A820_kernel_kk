...
void ms_emi(unsigned long long timestamp, unsigned char cnt, unsigned int *value)
{
	unsigned long nano_rem = do_div(timestamp, 1000000000);
	switch (cnt) {
	case 17: trace_printk(MS_EMI_FMT FMT17, MS_EMI_VAL VAL17); break;
	}
}
...
#define MS_MCI_UD_FMT	"%x,%x\n"
#define MS_MCI_UD_VAL	value[0],value[1]
void ms_mci(unsigned long long timestamp, unsigned char cnt, unsigned int *value)
{
	switch (cnt) {
	case 2: trace_printk(MS_MCI_UD_FMT, MS_MCI_UD_VAL); break;
	}
}
