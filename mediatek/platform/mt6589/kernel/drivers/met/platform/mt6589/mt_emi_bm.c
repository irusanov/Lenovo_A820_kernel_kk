...
void MET_MCI_Event_Set(unsigned int evt0, unsigned int evt1)
{
...
void MET_MCI_Event_Read()
{
	printk("MCI_EVENT0 = %x\n", readl(MCI_EVENT0_SEL));
	printk("MCI_EVENT1 = %x\n", readl(MCI_EVENT1_SEL));
}

unsigned int MET_MCI_GetEvent(int counter)
{
  unsigned int iCount;

  switch(counter){
    case 0:
      iCount = readl(MCI_EVENT0_SEL);
      break;
    case 1:
      iCount = readl(MCI_EVENT1_SEL);
      break;
    default:
      return BM_ERR_WRONG_REQ;
  }
...
unsigned int MET_MCI_GetEventCount(int evt_counter)
{
  unsigned int iCount;
