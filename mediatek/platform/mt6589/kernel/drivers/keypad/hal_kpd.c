...
void sb_kpd_disable(void)
{
	...
	mt_reg_sync_writew(0x1, KP_PMIC);
}
#else
void sb_kpd_enable(void)
{
...
static void enable_kpd(int enable)
{
	if(enable ==1)
		{
			mt_reg_sync_writew((u16)(enable), KP_PMIC);
			kpd_print("89 platform KEYPAD is enabled\n");
		}
	else if(enable == 0)
		{
			mt_reg_sync_writew((u16)(enable), KP_PMIC);
			kpd_print("89 platform KEYPAD is disabled\n");
		}
...
void kpd_get_pmic_keymap_state(u16 state[])
{
	int i = 0;
	int flag = 0;
	for(i=0;i<5;i++){
		mt_reg_sync_writew(0x04+(4*i), (KP_BASE+0x20));
		printk(KPD_SAY "IPO register address = 0x%x\n",0x04+(4*i));
		do{
			flag = (*(volatile u16 *)(KP_BASE+0x28));
		}while((*(volatile u16 *)(KP_BASE+0x28))!=1);
		state[i]=*(volatile u16 *)(KP_BASE+0x24);
		printk(KPD_SAY "IPO register [i]= 0x%x, flag = %d\n", i, state[i], flag);
	}
	printk(KPD_SAY "IPO register = %x %x %x %x %x\n",state[0], state[1], state[2], state[3], state[4]);
}

static void kpd_ipo_mode_handler(void)
{
	int i, j;
	bool pressed;
	u16 new_state[KPD_NUM_MEMS], change, mask;
	...
	kpd_get_pmic_keymap_state(new_state);
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			pressed = !(new_state[i] & mask);
			if (kpd_show_hw_keycode) {
				printk(KPD_SAY "(%s)IPO mode HW keycode = %u\n",
				       pressed ? "pressed" : "released",
				       hw_keycode);
			}
			BUG_ON(hw_keycode >= KPD_NUM_KEYS);
...
#ifndef EVB_PLATFORM
	if(enable == 1){
		kpd_print("enable kpd wake source!\n");
		kpd_ipo_mode_handler();
		upmu_set_rg_smps_autoff_dis(0x01);

	}else{
		kpd_print("disable kpd wake source!\n");
		upmu_set_rg_smps_autoff_dis(0x00);

	}
#endif
	//bit = *(volatile u32 *)(0xF0001048);
	//kpd_print("kpd_wakeup_src_setting bit: 0x%x\n",bit);
}
