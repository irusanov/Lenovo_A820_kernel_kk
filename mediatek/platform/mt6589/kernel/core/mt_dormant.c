...
* macro
**********************************/
#define BOOTROM_PWR_CTRL        (SRAMROM_BASE + 0x20)
#define BOOTROM_BOOT_ADDR       (SRAMROM_BASE + 0x30)
...
static void restore_gic_distributor_shared(u32 *pointer, unsigned gic_distributor_address)
{
    ...
    if (num_spis) {
        ...
        if (reg_read( IO_VIRT_TO_PHYS(SPM_SLEEP_ISR_RAW_STA) ) & WAKE_SRC_MD_WDT) {
            i = MT_MD_WDT1_IRQ_ID / GIC_PRIVATE_SIGNALS;
            j = MT_MD_WDT1_IRQ_ID % GIC_PRIVATE_SIGNALS;
            id->pending.set[i] |= (1 << j);
            i = MT_MD_WDT2_IRQ_ID / GIC_PRIVATE_SIGNALS;
            j = MT_MD_WDT2_IRQ_ID % GIC_PRIVATE_SIGNALS;
            id->pending.set[i] |= (1 << j);
        }
