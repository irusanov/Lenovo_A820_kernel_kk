    ...
    CAM_PIPE_MGR_SCEN_HW_ENUM   ScenHw)
{
    ...
    switch(ScenSw)
    {
        ...
        case CAM_PIPE_MGR_SCEN_SW_VIDEO_PRV:
        {
            ...
            {
                //PipeLockTable |= (1<<CAM_PIPE_MGR_PIPE_CDP_CONCUR);
            }
            break;
        }
...
static int CamPipeMgr_Probe(struct platform_device *pDev)
{
    ...
    strcpy(
        CamPipeMgr.PipeName[CAM_PIPE_MGR_PIPE_CDP_CAM],
        CAM_PIPE_MGR_PIPE_NAME_CDP_CAM);
    strcpy(
        CamPipeMgr.PipeName[CAM_PIPE_MGR_PIPE_CDP_CONCUR],
        CAM_PIPE_MGR_PIPE_NAME_CDP_CONCUR);
    //
    CamPipeMgr_UpdatePipeLockTable(CamPipeMgr.Mode.ScenSw);
    CamPipeMgr.LogMask = (  CAM_PIPE_MGR_PIPE_MASK_CAM_IO|
                            CAM_PIPE_MGR_PIPE_MASK_POST_PROC|
                            CAM_PIPE_MGR_PIPE_MASK_CDP_CAM|
                            CAM_PIPE_MGR_PIPE_MASK_CDP_LINK);
    //
    LOG_MSG("X");
    return Ret;
}
