...
static int resume_cunt=0;
static int suspend_cunt=0;
				...
				unsigned long state)
{
	...
	{
		printk("suspend_cunt=%d,resume_cunt=%d,\n",suspend_cunt,resume_cunt);
...
static int mtktscpu_read_log(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "[ mtktscpu_read_log] log = %d,talking_flag=%d,suspend_cunt=%d,resume_cunt=%d\n",mtktscpu_debug_log,talking_flag,suspend_cunt,resume_cunt);
