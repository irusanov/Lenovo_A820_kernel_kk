...
{
	{"MTK_MD1_SUPPORT",	modem_3g},
	{"MTK_MD2_SUPPORT",	modem_3g},
...
static int get_ccci_dfo_setting(char item[], unsigned int *val)
{
	...
	for (i=0; i<(sizeof(ccci_dfo_setting)/sizeof(dfo_item_t)); i++) { // CCCI DFO feature index
		...
		printk("[ccci/ctl] (0) Get DFO:%s, 0x%08X, item:%s\n", ccci_name, ccci_value, item);
	...
	printk("[ccci/ctl] (0)DFO[%d]:%s not found\n", i+1, item);
	return -1;
}
...
static void cal_md_mem_usage(void)
{
	...
	printk("[ccci/ctl] (0) cal_md_mem_usage()+\n");
	...
	if(get_ccci_dfo_setting("MTK_ENABLE_MD2", &tmp) == 0) {
		if(tmp > 0) 
			md2_en = 1;
	}
	...
	if(get_ccci_dfo_setting("MD2_SIZE", &tmp) == 0) {
		tmp = round_up(tmp, 0x200000);
	...
	if(get_ccci_dfo_setting("MD2_SMEM_SIZE", &tmp) == 0) {
		tmp = round_up(tmp, 0x200000);
		md_share_mem_size[MD_SYS2] = tmp;
	}
	...
	if(get_ccci_dfo_setting("MTK_MD2_SUPPORT", &tmp) == 0) {
	...
	if(md1_en && (md_share_mem_size[MD_SYS1]>0) && (md_resv_mem_size[MD_SYS1]>0)) {
		...
		// Reset to zero
		printk("[ccci/ctl] (0) DFO Setting for md1 will reset to 0: <%d:0x%08X:0x%08X>\n", 
				md1_en, md_resv_mem_size[MD_SYS1], md_share_mem_size[MD_SYS1]);
		md1_en = 0;
	...
	if(md2_en && (md_share_mem_size[MD_SYS2]>0) && (md_resv_mem_size[MD_SYS2]>0)) {
		// Setting is OK
	} else if (md2_en && ((md_share_mem_size[MD_SYS2]<=0) || (md_resv_mem_size[MD_SYS2]<=0))) {
		printk("[ccci/ctl] (0)[Error]DFO Setting for md2 wrong: <%d:0x%08X:0x%08X>\n", 
				md2_en, md_resv_mem_size[MD_SYS2], md_share_mem_size[MD_SYS2]);
		md_share_mem_size[MD_SYS2] = MD2_SMEM_SIZE;
		md_resv_mem_size[MD_SYS2] = MD2_MEM_SIZE;
	} else {
		// Reset to zero
		printk("[ccci/ctl] (0) DFO Setting for MD2 will reset to 0: <%d:0x%08X:0x%08X>\n", 
				md2_en, md_resv_mem_size[MD_SYS2], md_share_mem_size[MD_SYS2]);
		md2_en = 0;
		md_share_mem_size[MD_SYS2]=0;
	...
	if( (md_usage_case&(MD1_EN|MD2_EN))==(MD1_EN|MD2_EN)) { // Both two MD enabled
		modem_size_list[0] = md_resv_mem_size[MD_SYS1]+md_share_mem_size[MD_SYS1]+md_share_mem_size[MD_SYS2];
		...
		modem_size_list[0] = md_resv_mem_size[MD_SYS2]+md_share_mem_size[MD_SYS2];
		modem_size_list[1] = 0;
...
int parse_ccci_dfo_setting(void *dfo_tbl, int num)
{
	...
	printk("[ccci/ctl] (0) DFO:parse_ccci_dfo_setting()+ dfo_tbl:%d\n", (int)dfo_tbl);
...
int parse_ccci_dfo_setting(void *dfo_data, int num)
{
	...
	#ifdef __USING_MD1_2G__
	ccci_dfo_setting[0].value = modem_2g;
	#endif
...
static void collect_md_setting(void)
{
	...
	printk("[ccci/ctl] (0) cms collect_md_setting: md_usage_case:0x%x, addr[0]:0x%08x, addr[1]:0x%08x\n",
		md_usage_case, (unsigned int)addr[0],(unsigned int)addr[1]);

	if( (md_usage_case&(MD1_EN|MD2_EN))==(MD1_EN|MD2_EN)) { // Both two MD enabled
		md1_en = 1;
		md2_en = 1;
		md_resv_mem_addr[MD_SYS1] = (unsigned int)addr[0];
		md_resv_mem_addr[MD_SYS2] = (unsigned int)addr[1];
		md_resv_smem_addr[MD_SYS1] = (unsigned int)(addr[0] + md_resv_mem_size[MD_SYS1]);
		md_resv_smem_addr[MD_SYS2] = (unsigned int)(addr[0] + md_resv_mem_size[MD_SYS1] \
		...
		md1_en = 0;
		md2_en = 1;
		md_resv_mem_addr[MD_SYS2] = (unsigned int)addr[0];
		md_resv_smem_addr[MD_SYS2] = (unsigned int)(addr[0] + md_resv_mem_size[MD_SYS2]);
		md_resv_smem_base = (unsigned int)addr[0];
	...
	if ( (md_resv_mem_addr[MD_SYS2]&(32*1024*1024 - 1)) != 0 )
		printk("[ccci/ctl] (0) md2 memory addr is not 32M align!!!\n");
	...
	if ( (md_resv_smem_addr[MD_SYS2]&(2*1024*1024 - 1)) != 0 )
		printk("[ccci/ctl] (0) md2 share memory addr %08x is not 2M align!!\n", md_resv_smem_addr[MD_SYS2]);

	printk("[ccci/ctl] (0) MD1_EN(%d):MD2_EN(%d):MemBase(0x%08X)\n", md1_en, md2_en, md_resv_smem_base);

	printk("[ccci/ctl] (0) MemStar(0x%08X:0x%08X):MemSize(0x%08X:0x%08X)\n", \
		md_resv_mem_addr[MD_SYS1], md_resv_mem_addr[MD_SYS2], \
		md_resv_mem_size[MD_SYS1], md_resv_mem_size[MD_SYS2]);

	printk("[ccci/ctl] (0) SmemStart(0x%08X:0x%08X):SmemSize(0x%08X:0x%08X)\n", \
		md_resv_smem_addr[MD_SYS1], md_resv_smem_addr[MD_SYS2], \
		md_share_mem_size[MD_SYS1], md_share_mem_size[MD_SYS2]);
}
...
unsigned int get_resv_mem_size_for_md(int md_id)
{
	switch(md_id)
	{
		...
		case MD_SYS2:
			return md_resv_mem_size[MD_SYS2];
...
unsigned int get_resv_share_mem_size_for_md(int md_id)
{
	switch(md_id)
	{
		...
		case MD_SYS2:
			return md_share_mem_size[MD_SYS2];
...
unsigned int get_md_mem_start_addr(int md_id)
{
	switch(md_id)
	{
		...
		case MD_SYS2: 
			return md_resv_mem_addr[MD_SYS2];
...
unsigned int get_md_share_mem_start_addr(int md_id)
{
	switch(md_id)
	{
		...
		case MD_SYS2: 
			return md_resv_smem_addr[MD_SYS2];
		default:
			return 0;
	}
}
...
int get_td_eint_info(int md_id, char * eint_name, unsigned int len)
{
	...
	int ret = get_td_eint_num(eint_name, len);
	printk("[ccci/rpc] get_td_eint_num ret:%d\n", ret);
	return ret;
...
int get_md_gpio_info(int md_id, char *gpio_name, unsigned int len)
{
	...
	int ret = mt_get_md_gpio(gpio_name, len);
	printk("[ccci/rpc] mt_get_md_gpio:%d\n", ret);
	return ret;
...
int get_md_gpio_val(int md_id, unsigned int num)
{
	...
	int ret = mt_get_gpio_in(num);
	printk("[ccci/rpc] mt_get_gpio_in:%d\n", ret);
	return ret;
