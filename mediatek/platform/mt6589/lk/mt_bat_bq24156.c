...
void bq24156_set_ac_current(void)
{
    ...
    switch (g_temp_CC_value) {
        ...
        case Cust_CC_70MA:
            ...
            printf("[BATTERY:bq24156] current(%d) too low current, disable charger\r\n", g_temp_CC_value);
            bq24156_set_ce(0x0);
            return;
            break;
        case Cust_CC_0MA:
            printf("[BATTERY:bq24156] current is 0mA, disable charger\r\n");
            bq24156_set_ce(0x0);
            return;
            break;
        default:
            /* unknown current, disable charger */
            printf("[BATTERY:bq24156] unknown current(%d), disable charger\r\n", g_temp_CC_value);
            bq24156_set_ce(0x0);
            return;
            break;
    }
    ...
    if (low_chg)
    {
        printf("[BATTERY:bq24156] LOW_CHG=1\r\n");
        bq24156_set_low_chg(0x1);
    }
    else
    {
        bq24156_set_low_chg(0x0);
        printf("[BATTERY:bq24156] reg_set_value=0x%x\r\n", reg_set_value);
...
void select_charging_current_bq24156(void)
{
...
#endif
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24156] use USB_CHARGER_CURRENT\r\n");    
        }            
    } 
...
#endif
        if (Enable_BATDRV_LOG == 1) {
            printf( "[BATTERY:bq24156] NONSTANDARD_CHARGER, use AC_CHARGER_CURRENT\r\n");
        }
    } 
...
#endif
        if (Enable_BATDRV_LOG == 1) {
            printf( "[BATTERY:bq24156] STANDARD_CHARGER, use AC_CHARGER_CURRENT\r\n");
        }
    }
    else if (BMT_status.charger_type == CHARGING_HOST) 
    {
        g_temp_CC_value = USB_CHARGER_CURRENT;
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24156] CHARGING_HOST, use USB_CHARGER_CURRENT\r\n");
        }
    }
    else 
    {   /* unknown charger type, current under 500mA */
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24156] unknown charger type, current under 500mA \r\n");    
        }
...
{
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY] pchr_turn_off_charging_bq24156 !\r\n");
    }
	...
	{
	    printf("[BATTERY:bq24156] over voltage, enable high impedance mode!\r\n");
...
void pchr_turn_on_charging_bq24156 (void)
{
    ...
    else
    {
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY] pchr_turn_on_charging_bq24156 !\r\n");
        }
...
void BAT_GetVoltage_bq24156(void)
{ 
...
void BAT_GetVoltage_notbat_bq24156(void)
{   
    /* Get V_BAT_SENSE */
    if (g_chr_event == 0) 
    {        
...
void check_battery_exist(void)
{
    ...
    {        
        if( baton_count >= 3)
        {
                ...
                {
                    printf("[BATTERY] Battery is not exist, power off bq24156 and system (%d) 1\n", baton_count);
                    pchr_turn_off_charging_bq24156();
                    #ifndef NO_POWER_OFF
                    mt6575_power_off();
                    #endif 
    ...
    {        
        if( baton_count >= 3)
        {
            printf("[BATTERY] Battery is not exist, power off bq24156 and system (%d) 2\n", baton_count);
            pchr_turn_off_charging_bq24156();
            #ifndef NO_POWER_OFF
            mt6575_power_off();
            #endif        
...
void BAT_thread_bq24156(void)
{
...
#ifndef MTK_BQ24156_NO_CD_GPIO
    printf("[BATTERY] GPIO_CHR_CE_PIN=%d\n", GPIO_CHR_CE_PIN);
#endif
    ...
    else
    {
        ...
        if( bq24156_status == 0x2 )
        {
            ...
            g_HW_Charging_Done = 1;            
            //pchr_turn_off_charging_bq24156();
            printk("[BATTERY:bq24156] Battery real full and disable charging (%d) \n", bq24156_status); 
            return;
        }
        ...
        pchr_turn_on_charging_bq24156();        
        if (Enable_BATDRV_LOG == 1) {
                printk(  "[BATTERY:bq24156] Total charging timer=%d, bq24156_status=%d \n\r", 
...
if ((upmu_is_chr_det() == KAL_TRUE))
    {
        printf("[mt65xx_bat_init] bq24156 hw init\n");     
        bq24156_hw_init();
        bq24156_dump_register();   
				pchr_turn_on_charging_bq24156();
    }
