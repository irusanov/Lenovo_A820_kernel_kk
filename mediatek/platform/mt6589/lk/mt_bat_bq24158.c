...
void bq24158_set_ac_current(void)
{
    ...
    else 
    {
        ...
        {    
            bq24158_config_interface_reg(0x02,0x7A);    //for 4.1v CV threshold
        }
    }     
#else
    bq24158_config_interface_reg(0x01,0xF8);
...
void select_charging_curret_bq24158()
{
    ...
    {        
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24158] BMT_status.charger_type == STANDARD_HOST \r\n");    
        }
    } 
    ...
    {   
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24158] BMT_status.charger_type == NONSTANDARD_CHARGER \r\n");    
        }
    } 
    else if (BMT_status.charger_type == STANDARD_CHARGER) 
    {
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24158] BMT_status.charger_type == STANDARD_CHARGER \r\n");    
        }
    }
    else if (BMT_status.charger_type == CHARGING_HOST) 
    {
        ...
        bq24158_set_ac_current();
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24158] BMT_status.charger_type == CHARGING_HOST \r\n");    
        }
    }
    else 
    {
        ...
        bq24158_config_interface_reg(0x01,0x78);
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY:bq24158] default set g_temp_CC_value = USB_CHARGER_CURRENT \r\n");    
        }
    }
}
...
{
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY] pchr_turn_off_charging_bq24158 !\r\n");
    }
...
void pchr_turn_on_charging_bq24158 (void)
{
    ...
    else
    {
        ...
        if (Enable_BATDRV_LOG == 1) {
            printf("[BATTERY] pchr_turn_on_charging_bq24158 !\r\n");
        }
...
void check_battery_exist(void)
{
    ...
    {        
        if( baton_count >= 3)
        {
                ...
                {
                    printf("[BATTERY] Battery is not exist, power off bq24158 and system (%d) 1\n", baton_count);
                    pchr_turn_off_charging_bq24158();
                    #ifndef NO_POWER_OFF
                    mt6575_power_off();
                    #endif 
    ...
    {        
        if( baton_count >= 3)
        {
            printf("[BATTERY] Battery is not exist, power off bq24158 and system (%d) 2\n", baton_count);
            pchr_turn_off_charging_bq24158();
            #ifndef NO_POWER_OFF
            mt6575_power_off();
            #endif        
...
void mt65xx_bat_init(void)
{
    ...
    {
        bq24158_hw_init();        
        printf("[mt65xx_bat_init] bq24158 hw init\n");        
        bq24158_dump_register();            
        pchr_turn_on_charging_bq24158();
    }
