...
MBOOL IMemDrvImp::init(void)
{
    ...
    if(mLocal_InitCount==1)
    {
	   ...
	   if(mInitCount==1)
	   {
          ...
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_VID);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_IMG);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_LSCI);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_FLKI);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_LCEI);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_LCSO);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_ESFKO);
          mpM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_AAO);
          //
          ...
          ret = mpM4UDrv->m4u_config_port(&port);
          //
          port.ePortID = M4U_PORT_VIDO; 
          ret = mpM4UDrv->m4u_config_port(&port);
          //
          port.ePortID = M4U_PORT_VIDCO; 
          ret = mpM4UDrv->m4u_config_port(&port);
          //
          port.ePortID = M4U_PORT_VIDVO; 
          ret = mpM4UDrv->m4u_config_port(&port);
          //
...
//#elif   defined(__ISP_USE_STD_M4U__) || defined (__ISP_USE_ION__)
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            mm_data.config_buffer_param.eModuleID = M4U_CLNTMOD_IMG;    // 1;
			...
			///
            //judge the buffer is for vido or not, then do other config
            if(pInfo->bufSecu==1)
            {
               //IMEM_INF("config port for vido");
               M4U_PORT_STRUCT port;
               port.Virtuality = 1;
               port.Security = 1;  //for vido
               port.domain = 3;
               port.Distance = 1;
               port.Direction = 0; //M4U_DMA_READ_WRITE
               port.ePortID = M4U_PORT_VIDO; 
               ...
               port.ePortID = M4U_PORT_VIDVO; 
               ret = mpM4UDrv->m4u_config_port(&port);
			   //record the buffer addr for vido
               vidopAddr=pInfo->phyAddr;
            }
            //free for IMPORT ref cnt
            if(ion_free(
                mIonDrv,
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if ( 1 ) {
		...
		{//using ion
		    //judge the buffer is for vido or not, then do other config
            if(pInfo->phyAddr==vidopAddr)
            {
               //IMEM_INF("config port for vido");
               MINT32 ret = 0;
               M4U_PORT_STRUCT port;
               port.Virtuality = 1;
               port.Security = 0;  //for vido
               port.domain = 3;
               port.Distance = 1;
               port.Direction = 0; //M4U_DMA_READ_WRITE
               port.ePortID = M4U_PORT_VIDO; 
    ...
    void)
{
    ...
    mpM4UDrv->m4u_cache_flush_all(M4U_CLNTMOD_IMG);
#endif
...
#endif
    //
    M4U_MODULE_ID_ENUM eM4U_ID = M4U_CLNTMOD_IMG;
