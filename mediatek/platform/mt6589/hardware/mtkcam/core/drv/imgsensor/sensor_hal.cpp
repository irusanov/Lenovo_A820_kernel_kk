...
static SENSOR_CROP_INFO sensorCropInfo[4];
MINT32 SensorHalImp::mSearchSensorDev = SENSOR_DEV_NONE;
static SENSOREXIFDEBUG_STURCT sensorDebug[2];
static ACDK_SCENARIO_ID_ENUM previousScenarioId[3]={ACDK_SCENARIO_ID_CAMERA_PREVIEW,ACDK_SCENARIO_ID_CAMERA_PREVIEW,ACDK_SCENARIO_ID_CAMERA_PREVIEW};//0:main,1:sub,2:main_2
static MUINT32 previousExposureTime[3] = {33000,33000,33000};//unit: us 

//
static ACDK_SENSOR_RESOLUTION_INFO_STRUCT staticSensorResoultion[4];
static halSensorType_e staticSensorType[4];
static halSensorRawImageInfo_t staticSensorRawInfo[4][2];
static SENSOR_GRAB_INFO_STRUCT staticSensorGrabInfo[4][ACDK_SCENARIO_ID_MAX-1];//8 scenarios
...
MINT32 SensorHalImp::createImp()
{
    ...
    mImageSensorType[0] = IMAGE_SENSOR_TYPE_RAW;
    mIspSensorType[1] = SENSOR_TYPE_RAW;
    mImageSensorType[1] = IMAGE_SENSOR_TYPE_RAW;
...
MINT32 SensorHalImp::searchSensor()
{
    ...
    //ToDo: select TG2 GPIO
    ret = pSeninfDrv->setTg2PhaseCounter(
        1, CAM_PLL_48_GROUP, /*sensorInfo.SensorMasterClockSwitch,*/
        clkCnt,  sensorInfo[1].SensorClockPolarity ? 0 : 1,
       1, 0, 0);        
    if (ret < 0) {
        LOG_ERR("setTg1PhaseCounter fail\n");
        return 0;
    ...
    mSearchSensorDev |= SENSOR_DEV_ATV;
...
MINT32 SensorHalImp::setConf(halSensorIFParam_t halSensorIFParam[2])
{
    ...
    LOG_MSG("[setConf] main/atv src: %d/%d , sub/main2 src: %d/%d\n", halSensorIFParam[0].u4SrcW, halSensorIFParam[0].u4SrcH, halSensorIFParam[1].u4SrcW, halSensorIFParam[1].u4SrcH);
    LOG_MSG("[setConf] main/atv scenario: %d , sub/main2 scenario: %d\n", halSensorIFParam[0].scenarioId, halSensorIFParam[1].scenarioId);
    ...
    if(mSensorDev & SENSOR_DEV_MAIN_2 ) {
        sensorScenarioId[1] = halSensorIFParam[1].scenarioId;
        curScenario = halSensorIFParam[1].scenarioId;
        cameraId[1] = SENSOR_MAIN_2;
        if(halSensorIFParam[1].u4IsBypassSensorScenario != 0) {
            goto setConf_exit;
        }        
    }
    ...
    if(mSensorDev & SENSOR_DEV_MAIN) {
        ...
        if (mImageSensorType[0] == IMAGE_SENSOR_TYPE_RAW) {
            ...
            pixelX1[0] = pixelX0[0] + halSensorIFParam[0].u4CropW;
            pixelY1[0] = pixelY0[0] + halSensorIFParam[0].u4CropH;
    		padSel = PAD_10BIT;//mipi workaround, don't care
        ...
        if((mImageSensorType[0] == IMAGE_SENSOR_TYPE_RAW) || (mImageSensorType[0] == IMAGE_SENSOR_TYPE_RAW8)){
            ...
            bwc.SensorSize_Set(BWC_SIZE(halSensorIFParam[0].u4CropW,halSensorIFParam[0].u4CropH));
        }
        else {
            bwc.SensorSize_Set(BWC_SIZE(halSensorIFParam[0].u4CropW>>1,halSensorIFParam[0].u4CropH));
        }
    ...
    if(mSensorDev & SENSOR_DEV_SUB ) {
        ...
        ret = pSeninfDrv->setTg2GrabRange(pixelX0[1], pixelX1[1], pixelY0[1], pixelY1[1]);
        if (ret < 0) {
            LOG_ERR("setTg1GrabRange fail\n");
            goto setConf_exit;
        }
        //
        ret = pSeninfDrv->setTg2SensorModeCfg(sensorInfo[1].SensorHsyncPolarity ? 0 : 1, 
        ...
        //
        ret = pSeninfDrv->setTg2InputCfg(padSel, inSrcTypeSel, inDataType, senInLsb);
        if (ret < 0) {
            LOG_ERR("setTg1InputCfg fail\n");
            goto setConf_exit;
        }
        ...
        // Set view finder mode
        ret = pSeninfDrv->setTg2ViewFinderMode(halSensorIFParam[1].u4IsContinous ? 0 : 1, halSensorIFParam[1].u4IsBypassSensorDelay ? 0 : sensorInfo[1].CaptureDelayFrame);
        if (ret < 0) {
            LOG_ERR("setViewFinderMode fail\n");
            goto setConf_exit;
        }
    ...
    if(mSensorDev & SENSOR_DEV_MAIN_2 ) {
        ...
        sensorCropInfo[2].u4GrabX = sensorInfo[1].SensorGrabStartX;
        sensorCropInfo[2].u4GrabY = sensorInfo[1].SensorGrabStartY;
        sensorCropInfo[2].u4SrcW = halSensorIFParam[1].u4SrcW;
        sensorCropInfo[2].u4SrcH = halSensorIFParam[1].u4SrcH;
        sensorCropInfo[2].u4CropW = halSensorIFParam[1].u4CropW;
        sensorCropInfo[2].u4CropH = halSensorIFParam[1].u4CropH;
        sensorCropInfo[2].DataFmt = (MUINT32)sensorInfo[1].SensorOutputDataFormat;
        ...
        if((mImageSensorType[1] == IMAGE_SENSOR_TYPE_RAW) || (mImageSensorType[1] == IMAGE_SENSOR_TYPE_RAW8)) {       
            if(curScenario != previousScenarioId[2]) {
                sendCommand(SENSOR_DEV_SUB, CMD_SENSOR_SET_SENSOR_EXP_TIME, (int)&previousExposureTime[2],NULL,NULL);
                LOG_MSG("set Exposure Time = %d, curScenario = %d, previousScenario = %d\n",previousExposureTime[2],curScenario,previousScenarioId[2]);
        ...
        previousScenarioId[2] = curScenario;
    ...
    if(mSensorDev & SENSOR_DEV_ATV ) {
    	...
    	if (sensorInfo[0].SensroInterfaceType == SENSOR_INTERFACE_TYPE_PARALLEL) {
		inSrcTypeSel = PARALLEL_SENSOR;    	
		}
        else if (sensorInfo[0].SensroInterfaceType == SENSOR_INTERFACE_TYPE_SERIAL) {
            inSrcTypeSel = SERIAL_SENSOR;
        ...
        LOG_MSG("atv SensorType %d,  %d \n", mImageSensorType[0], sensorInfo[0].SensroInterfaceType);
        ret = pSeninfDrv->setTg1GrabRange(pixelX0[0], pixelX1[0], pixelY0[0], pixelY1[0]);
        if (ret < 0) {
            LOG_ERR("atv setTg1GrabRange fail\n");
            goto setConf_exit;
        }
        //
        ret = pSeninfDrv->setTg1SensorModeCfg(sensorInfo[0].SensorHsyncPolarity ? 0 : 1, 
                                        sensorInfo[0].SensorVsyncPolarity ? 0 : 1);
        if (ret < 0) {
            LOG_ERR("atv setTg1SensorModeCfg fail\n");
            goto setConf_exit;
        }

        ret = pSeninfDrv->setTg1InputCfg(padSel, inSrcTypeSel, inDataType, senInLsb);
        if (ret < 0) {
            LOG_ERR("atv setTg1InputCfg fail\n");
            goto setConf_exit;
        }
        ...
        ret = pSeninfDrv->setTg1ViewFinderMode(halSensorIFParam[0].u4IsContinous ? 0 : 1, halSensorIFParam[0].u4IsBypassSensorDelay ? 0 : sensorInfo[0].CaptureDelayFrame);
        if (ret < 0) {
            LOG_ERR("atv setViewFinderMode fail\n");
            goto setConf_exit;
        }
...
MINT32 SensorHalImp::initSensor()
{
    ...
    switch (mSensorDev)
    {
        ...
        case SENSOR_DEV_MAIN_2:
            eSensorDev = SENSOR_MAIN_2;
            if(!(mSearchSensorDev & SENSOR_DEV_MAIN_2)) {
                LOG_ERR("initSensor fail,mSensorDev = 0x%x, mSearchSensorDev = 0x%x\n",mSensorDev,mSearchSensorDev);
                return -1;
            }                  
            break;
        case SENSOR_DEV_MAIN_3D:
            eSensorDev = SENSOR_MAIN_3D;
            if(!((mSearchSensorDev & SENSOR_DEV_MAIN) &&(mSearchSensorDev & SENSOR_DEV_MAIN_2) )) {
                LOG_ERR("initSensor fail,mSensorDev = 0x%x, mSearchSensorDev = 0x%x\n",mSensorDev,mSearchSensorDev);
                return -1;
            }              
            break;
    ...
    */
    ...
    if(eSensorDev == SENSOR_DEV_MAIN_2) {
        if  ( IMAGE_SENSOR_TYPE_RAW == pSensorDrv->getCurrentSensorType(eSensorDev) )
        {
            //  Full Resolution
            u4PaddedWidth = sensorResolution[1].SensorFullWidth + ISP_RAW_WIDTH_PADD;
...
MINT32 SensorHalImp::setTgPhase() //CMMCLK: Main/sub, CMMCLK2:Main_2 (external signal design is not sync with internal signal in TG/I2C)
{
    ...
    if(mSensorDev & SENSOR_DEV_MAIN_2){
        ...
        clkInKHz2 = sensorInfo[1].SensorClockFreq * 1000;     
    ...
    case SENSOR_DEV_SUB:
        ...
        break;
    case SENSOR_DEV_MAIN_2:
        ret = pSensorDrv->sendCommand(SENSOR_MAIN_2, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv2);
        break;
    case SENSOR_DEV_MAIN_3D:
        ret = pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv1);        
        ret2 = pSensorDrv->sendCommand(SENSOR_MAIN_2, CMD_SENSOR_GET_PAD_PCLK_INV, &u4PadPclkInv2);
        break;
        case SENSOR_DEV_ATV:
    ...
    if(mSensorDev & SENSOR_DEV_MAIN_2){
        ret = pSeninfDrv->setTg2PhaseCounter(
            1, mclkSel2 /*sensorInfo.SensorMasterClockSwitch ? 0 : 1*/,
            clkCnt2, sensorInfo[1].SensorClockPolarity ? 0 : 1,
            sensorInfo[1].SensorClockFallingCount, sensorInfo[1].SensorClockRisingCount, u4PadPclkInv2);
...
MINT32 SensorHalImp::getRawInfo(halSensorDev_e sensorDevId,halSensorRawImageInfo_t *pinfo, MINT32 mode)
{
    ...
    else if ((SENSOR_DEV_MAIN_2==sensorDevId)||(SENSOR_DEV_SUB==sensorDevId)) {
        if ( mode == 1 ) { // preview
    ...
    int arg3)
{
    ...
    case SENSOR_CMD_SET_SENSOR_EXP_TIME:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            previousExposureTime[2] = *(MUINT32 *) arg1;
        }
        else{
    ...
    case SENSOR_CMD_GET_SENSOR_PRV_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[3].SensorPreviewWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[3].SensorPreviewHeight;
        }
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }
    ...
    case SENSOR_CMD_GET_SENSOR_FULL_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[3].SensorFullWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[3].SensorFullHeight;
        } 
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }        
    ...
    case SENSOR_CMD_GET_SENSOR_VIDEO_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[2].SensorVideoWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[2].SensorVideoHeight;
        }   
        else if(SENSOR_DEV_ATV==sensorDevId) {
            *(MUINT16 *) arg1 = staticSensorResoultion[3].SensorFullWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[3].SensorFullHeight;
    ...
    case SENSOR_CMD_GET_SENSOR_HIGH_SPEED_VIDEO_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[2].SensorHighSpeedVideoWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[2].SensorHighSpeedVideoHeight;
        }   
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }        
    ...
    case SENSOR_CMD_GET_SENSOR_3D_PRV_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[2].Sensor3DPreviewWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[2].Sensor3DPreviewHeight;
        }
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }
    ...
    case SENSOR_CMD_GET_SENSOR_3D_FULL_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[2].Sensor3DFullWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[2].Sensor3DFullHeight;
        }   
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }        
    ...
    case SENSOR_CMD_GET_SENSOR_3D_VIDEO_RANGE:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorResoultion[2].Sensor3DVideoWidth;
            *(MUINT16 *) arg2 = staticSensorResoultion[2].Sensor3DVideoHeight;
        }   
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }        
    ...
    case SENSOR_CMD_GET_SENSOR_ID:
        switch(sensorDevId) {
            ...
            break;
            case SENSOR_DEV_MAIN_2:
                *(MUINT32 *)arg1 = pDeviceDrv->getMain2SensorID();
            break;
            default:
    ...
    case SENSOR_CMD_GET_SENSOR_TYPE:
         if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MINT32 *) arg1 = staticSensorType[3];
         }
        break;
    ...
    case SENSOR_CMD_GET_RAW_INFO:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            ::memcpy((char *) arg1, &staticSensorRawInfo[3][arg2],sizeof(halSensorRawImageInfo_t));
        }  
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }
    ...
    case SENSOR_CMD_GET_SENSOR_CROPINFO:
        if (arg1) {
            if((SENSOR_DEV_MAIN==sensorDevId)||(SENSOR_DEV_SUB==sensorDevId)) {
    ...
    case SENSOR_CMD_GET_SENSOR_GRAB_INFO:
        ...
        if(SENSOR_DEV_MAIN==sensorDevId) {
            ...
            *(MUINT16 *) arg1 = staticSensorGrabInfo[3][arg3].u4SensorGrabStartX;
            *(MUINT16 *) arg2 = staticSensorGrabInfo[3][arg3].u4SensorGrabStartY;
        }  
        else{
            LOG_ERR("[sendCommand] sensorDevId is incorrect ! \n");
        }
...
MINT32 SensorHalImp::initCSI2Peripheral(MINT32 initCSI2)
{
    ...
    if((mSensorDev & SENSOR_DEV_MAIN_2)||(mSensorDev & SENSOR_DEV_SUB)) {
        if(sensorInfo[1].SensroInterfaceType == SENSOR_INTERFACE_TYPE_MIPI) {   // enable peripheral
            if(initCSI2) {
                ret = pSeninfDrv->initTg2CSI2(1);
                if (ret < 0) {
                    LOG_ERR("init Tg2 CSI2 peripheral fail\n");
                }
            } else {
                ret = pSeninfDrv->initTg2CSI2(0);
                if (ret < 0) {
...
MINT32 SensorHalImp::setCSI2Config(MINT32 enableCSI2)
{
    ...
    if((mSensorDev & SENSOR_DEV_MAIN_2)||(mSensorDev & SENSOR_DEV_SUB)) {
        if((enableCSI2 ==  1) && (sensorInfo[1].SensroInterfaceType == SENSOR_INTERFACE_TYPE_MIPI)) {   // enable csi2
            ...
            ret = pSeninfDrv->setTg2CSI2(sensorInfo[1].MIPIDataLowPwr2HighSpeedTermDelayCount, sensorInfo[1].MIPIDataLowPwr2HighSpeedSettleDelayCount, 
            ...
            if (ret < 0) {
                LOG_ERR("setTg2CSI2Config fail\n");
            }
        }
        else {
            LOG_MSG("disable Tg2 CSI2Config\n");
            ret = pSeninfDrv->setTg2CSI2(0, 0, 0, 0, 0, 0, 0, 0);
            if (ret < 0) {
                LOG_ERR("disable Tg2 CSI2Config fail\n");
            }
...
MINT32 SensorHalImp::setSensorIODrivingCurrent()
{
    ...
    if((mSensorDev & SENSOR_DEV_MAIN) ||(mSensorDev & SENSOR_DEV_SUB) || (mSensorDev & SENSOR_DEV_ATV)) { //Main/sub use TG1 mclk   
        switch(sensorInfo[0].SensorDrivingCurrent) {
            case ISP_DRIVING_2MA://4 //4mA
                increaseDivingCurrent1 = 0x00;
                break;
            case ISP_DRIVING_4MA:// 8mA
                increaseDivingCurrent1 = 0x20;
                break;
            case ISP_DRIVING_6MA://12mA
                increaseDivingCurrent1 = 0x40;
                break;
            case ISP_DRIVING_8MA://16mA
                increaseDivingCurrent1 = 0x60;
                break;
            default:
    ...
    if(mSensorDev & SENSOR_DEV_MAIN_2) {
        switch(sensorInfo[1].SensorDrivingCurrent) {
            ...
            case ISP_DRIVING_4MA:
                increaseDivingCurrent2 = 0x20;
                break;
            case ISP_DRIVING_6MA:
                increaseDivingCurrent2 = 0x40;
                break;
            case ISP_DRIVING_8MA:
                increaseDivingCurrent2 = 0x60;
                break;
            default:
                LOG_MSG("The driving current value is wrong\n");
                break;
...
MINT32 SensorHalImp::querySensorInfo()
{
    ...
    for (mSensorDev = SENSOR_MAIN; mSensorDev <= SENSOR_MAIN_2; mSensorDev <<= 1) {
        if ( 0x04 == mSensorDev ) continue;
        ret = pSensorDrv->init(mSensorDev);
        if (ret < 0) {
            LOG_ERR("halSensorInit fail \n");
        }
        ...
        switch (mSensorDev) {
            ...
            break;
            case SENSOR_MAIN_2:
                pSensorResInfo[1] = &staticSensorResoultion[2];
            break;            
            default:
                pSensorResInfo[1] = &staticSensorResoultion[1];
            break;
        ...
        for(scenario = 0; scenario < ACDK_SCENARIO_ID_MAX-1; scenario++ ) {
            ...
            if (scenario == 0) {
                switch (mSensorDev) {
                    ...
                    break;
                    case SENSOR_MAIN_2:
                        staticSensorType[2] = mIspSensorType[1];
                    break;            
                    default:
                    break;
                }
    ...
    // Get Sensor Resolution
    pSensorResInfo[0] = &staticSensorResoultion[3];
    ret = pSensorDrv->getResolution(pSensorResInfo);
    if (ret < 0) {
        LOG_ERR("halSensorGetResolution failn");
    } 
    ...
    for(scenario = 0; scenario < ACDK_SCENARIO_ID_MAX-1; scenario++ ) {
        ...
        staticSensorGrabInfo[3][scenario].u4SensorGrabStartX = sensorInfo[0].SensorGrabStartX;
        staticSensorGrabInfo[3][scenario].u4SensorGrabStartY = sensorInfo[0].SensorGrabStartY;  

        if (scenario == 0) 
        {
            staticSensorType[3] = mIspSensorType[0];
        }
    }
    ...
    for (mode = 0; mode <= 1; mode++) {
        ret = getRawInfo((halSensorDev_e)mSensorDev,&staticSensorRawInfo[3][mode] , mode);
    }
...
{
    MINT32 ret = 0, tg1Ret = 0, tg2Ret = 0;

    if((mSensorDev & SENSOR_DEV_MAIN) && (mSensorDev & SENSOR_DEV_MAIN_2) ) {
        tg1Ret = pSeninfDrv->checkSeninf1Input();
        tg2Ret = pSeninfDrv->checkSeninf2Input();
        ret = tg1Ret | tg2Ret;        
    }
    else if(mSensorDev & SENSOR_DEV_MAIN ) {
        ret = pSeninfDrv->checkSeninf1Input();
    }
    else if(mSensorDev & SENSOR_DEV_SUB ) {
        ret = pSeninfDrv->checkSeninf2Input();
    }
