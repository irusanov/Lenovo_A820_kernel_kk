...
void AeMgr::setRestore(int frm)
{
	...
	else if(frm==1+expSetFrm) //off exp
	{
		...
		frmRate = 1000000*10/(40000+g_rExp);
		g_defaultFrameRate = AAASensorMgr::getInstance().getPreviewDefaultFrameRate();
//#ifdef MTK_AF_SYNC_RESTORE_SUPPORT
		if (isSupportSetMaxFrameRate()) AAASensorMgr::getInstance().setPreviewMaxFrameRate(frmRate);
//#else
//#endif
	...
	{
//#ifdef MTK_AF_SYNC_RESTORE_SUPPORT
		if (isSupportSetMaxFrameRate()) AAASensorMgr::getInstance().setPreviewMaxFrameRate(g_defaultFrameRate);
//#else
//#endif
		setExp(g_rExp);
