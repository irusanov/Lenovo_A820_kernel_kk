...
MRESULT AfMgr::init()
{
    ...
    #ifdef FLKPAUS
    m_pFlickerHal = FlickerHalBase::createInstance();
	...
	*/
    ...
    if (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC_VIDEO)   
    {
        if (m_sAFOutput.i4IsMonitorFV == TRUE)   
        {
            ...
            #ifdef FLKPAUS
            if(m_pFlickerHal)
                m_pFlickerHal->sendCommand(FLKCmd_FlkResume,NULL);
            #endif
        }
        else   
        {
            ...
            #ifdef FLKPAUS
            if(m_pFlickerHal)
               m_pFlickerHal->sendCommand(FLKCmd_FlkPause,NULL);
            #endif
        }
    }
...
MVOID AfMgr::setFlkWinConfig()
{
     ...
     || (m_PreAFArea.sRect[0].i4Y !=i4Y) || (m_PreAFArea.sRect[0].i4H !=i4H))
    {
	    ...
	    ISP_WRITE_REG(m_pIspReg , CAM_AF_SIZE, (i4W & 0x1FF) + ((i4H&0x1FF)<<16) );
    }
...
MVOID AfMgr::setAFWinTH(AF_CONFIG_T a_sAFConfig)
{
    ...
    // 8 bits
    if ((m_i4AF_THEX[0] != a_sAFConfig.AF_TH[0]) || (m_i4AF_THEX[1] != a_sAFConfig.AF_TH[1]))   {
        MY_LOG("[setAFWinTHEX0]%d [setAFWinTHEX1]%d\n", a_sAFConfig.AF_TH[0], a_sAFConfig.AF_TH[1]);        
        ISP_WRITE_REG(m_pIspReg , CAM_AF_TH_E, (Boundary(0, a_sAFConfig.AF_TH[1]+1, 255)<<8) + Boundary(0, a_sAFConfig.AF_TH[0]+1, 255));
        m_i4AF_THEX[0] = a_sAFConfig.AF_TH[0];
        m_i4AF_THEX[1] = a_sAFConfig.AF_TH[1];
    }
}
