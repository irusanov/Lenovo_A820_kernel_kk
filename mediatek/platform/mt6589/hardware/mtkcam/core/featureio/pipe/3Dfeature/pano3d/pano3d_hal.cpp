...
)
{
    ...
    MUINT32 PanelWidth = LCM_HEIGHT;
    MUINT32 PanelHeight = LCM_WIDTH;
    MavTuningInfo mavTuningInfo;
    mavTuningInfo.RCWinBound = 30;              // default: 30
    mavTuningInfo.SearchRange = 15;             // default: 15
    mavTuningInfo.MatchRate = 48;               // default: 48
    mavTuningInfo.RectErrThre = 8;              // default: 8
    mavTuningInfo.IterNum = 5;                  // default: 5
    mavTuningInfo.MaxAngle = 10;                // default: 10
    mavTuningInfo.ClipRatio = 154;              // default: 154 (0.6)
    mavTuningInfo.DispBound = 2.5;              // default: 2.5
    mavTuningInfo.DispRatio = 0.5;              // default: 0.5
    mavTuningInfo.ActMatThre = 20;              // default: 20
    mavTuningInfo.SpecDispMin = -(16 * PanelWidth/800);
    mavTuningInfo.SpecDispMax = 8 * PanelWidth/800;
    mavTuningInfo.SpecPanelWidth = PanelWidth;  // default: 800
    mavTuningInfo.SpecPanelHeight = PanelHeight; // default: 480
    mavTuningInfo.CoreNumber = 2;               // default: 2
    CAM_LOGD("[mHal3dfInit] PanelWidth=%d, PanelHeight=%d", PanelWidth, PanelHeight);
#endif
...
#ifdef MTK_S3D_SUPPORT
    myInfo.pTuningInfo = &mavTuningInfo;
#else
...
#ifdef MTK_S3D_SUPPORT
    MyPano3DIamgeInfo.ImgFmt = PANO3D_IMAGE_NV21;
#endif
...
********************************************************************************/
MINT32
halPANO3D::mHal3dfGetWokSize(int SrcWidth, int SrcHeight, MUINT32 &WorkingSize)
{
    MINT32 err = S_MAV_OK;
	  MUINT32 Pano3D_buf_size = 0;
	  Pano3DImageInfo MyPano3DInfo;
    MyPano3DInfo.ImgWidth = SrcWidth;
    MyPano3DInfo.ImgHeight = SrcHeight;
    CAM_LOGD("[mHal3dfGetWokSize] 0x%x 0x%x ",&MyPano3DInfo,&Pano3D_buf_size);
    m_pMTKPano3dObj->Pano3DFeatureCtrl(PANO3D_FEATURE_GET_WORKBUF_SIZE, &MyPano3DInfo, &Pano3D_buf_size);
    WorkingSize = Pano3D_buf_size;
    return err;
}
...
********************************************************************************/
MINT32
halPANO3D::mHal3dfSetWokBuff(void* WorkingBuff)
{
    MINT32 err = S_MAV_OK;
    CAM_LOGD("[mHal3dfSetWokBuff] 0x%x",(MUINT32)WorkingBuff);
    m_pMTKPano3dObj->Pano3DFeatureCtrl(PANO3D_FEATURE_SET_WORKBUF_ADDR, &WorkingBuff, NULL);
    return err;
}
#endif
