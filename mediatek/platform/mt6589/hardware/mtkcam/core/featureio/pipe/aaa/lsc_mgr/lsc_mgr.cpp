...
SetTBAToISP()     //  ISP <- PA
{
    ...
    if (isTSFEnable() == MTRUE && mTSFState != LSCMGR_TSF_STATE_IDLE)
        ...
        phyAddr = m_TSFBuff[TSF_BUFIDX_INPUT].phyAddr;
        m_rIspLscCfg[m_eLscScenario].baseaddr.bits.BASE_ADDR = phyAddr;
        m_rIspLscCfg[m_eLscScenario].xsize.bits.XSIZE = u4Size;

        MY_LOG_IF(_bEnableMyLog, "[%s] User: phyAddr(0x%08x), virAddr(0x%08x), size(%d)", __FUNCTION__,
                phyAddr, virAddr, u4Size);
    }
    else
#endif  //ENABLE_TSF
    {
...
{
    ISP_NVRAM_LSC_T debug = m_rIspLscCfg[m_eLscScenario];
    //ISP_MGR_LSC_T::getInstance(m_eActive).get(debug);
...
LscMgr::init() {
    ...
    m_bTSF = isEnableTSF() ? MTRUE : MFALSE;
