...
FlickerHal::FlickerHal()
{
   ...
   // m_pFlickerSysram = NULL;
    ...
    m_pIspDrv = NULL;
	m_pIspRegMap=NULL;
    m_u4FlickerFreq = FLK_DEFAULT_FREQ;
    m_u4FlickerWidth = 0;
    m_u4FlickerHeight = 0;
    ...
    FLICKER_LOG("m_u4FlickerFreq:%d %d FLKSPEED:%d\n", m_u4FlickerFreq, FLK_DEFAULT_FREQ, FLKSPEED);
...
//    NSCamCustom::FlickerThresholdSetting_T strFlickerThreshold;
   ...
   /////////////////////////////
   //marine test
   getFlickerThresPara(FLKSPEED,&strFlickerThreshold);
...
MINT32 FlickerHal::uninitNoLock()
{


    FLICKER_LOG("uninitNoLock %d",(int)__LINE__);
    MINT32 err = 0;
...
{
    FLICKER_LOG("uninit %d",(int)__LINE__);
    Mutex::Autolock lock(mLock);
	return uninitNoLock();
}
