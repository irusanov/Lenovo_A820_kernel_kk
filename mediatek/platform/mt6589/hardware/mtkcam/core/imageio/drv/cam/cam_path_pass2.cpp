...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    if(p_parameter->capTdriCfg.isRunSegment) {
    	...
    	m_isp_function_list[16 ] = (IspFunction_B*)&DMAP2Img2o;
    	m_isp_function_count = 17;
    }
    //turning
if (p_parameter->tpipe != CAM_MODE_VRMRG)   // Only run this when non-GDMA mode.
{
    ispTurningCtrl.bypass = 0;
}
else    // GDMA Mode.
{
    ...
    ispTopCtrl.isEn2Nr3dStatusFixed = p_parameter->isEn2Nr3dStatusFixed;
    ...
    ISP_PATH_DBG("disp_vid_sel(%d)",cdpPipe.disp_vid_sel);
    ISP_PATH_DBG("[imgi] in[%d, %d] imgiCrop[%d, %d, %d, %d]_f(0x%x, 0x%x)",p_parameter->imgi.size.w,p_parameter->imgi.size.h, \
    ...
    if ( CAM_CTL_EN2_CDRZ_EN & p_parameter->en_Top.enable2 ) {
        ...
        {
            ret = -1;
            ISP_PATH_ERR("[Error]CalAlgoAndCStep fail(CDP_DRV_RZ_CDRZ)");
            goto EXIT;
        }
        ...
        {
            ret = -1;
            ISP_PATH_ERR("[Error]CalAlgoAndCStep fail(CDP_DRV_RZ_CURZ)");
            goto EXIT;
        }
    ...
    if ( CAM_CTL_EN2_PRZ_EN & p_parameter->en_Top.enable2 ) {
        //prz set for tpipe
        p_parameter->prz.prz_input_crop_width = cdpPipe.prz_crop.w;
        p_parameter->prz.prz_input_crop_height = cdpPipe.prz_crop.h;
        ...
        {
            ret = -1;
            ISP_PATH_ERR("[Error]CalAlgoAndCStep fail(CDP_DRV_RZ_PRZ)");
            goto EXIT;
        }
    ...
    //vipi
    DMAVipi.bypass = 1;
    if ( CAM_CTL_DMA_EN_VIPI_EN & ispTopCtrl.en_Top.dma ) {
        DMAVipi.bypass = 0;
    ...
    if ( CAM_CTL_DMA_EN_IMG2O_EN & ispTopCtrl.en_Top.dma ) {
        DMAP2Img2o.bypass = 0;
        DMAP2Img2o.dma_cfg = p_parameter->img2o_dma;
		ISP_PATH_INF("CAM_CTL_DMA_EN_IMG2O_EN set: DMAP2Img2o.bypass = %d", DMAP2Img2o.bypass);
		ISP_PATH_INF("size:0x%x", DMAP2Img2o.dma_cfg.memBuf.size);
		ISP_PATH_INF("base_vAddr:0x%x", DMAP2Img2o.dma_cfg.memBuf.base_vAddr);
		ISP_PATH_INF("base_pAddr:0x%x", DMAP2Img2o.dma_cfg.memBuf.base_pAddr);
		ISP_PATH_INF("ofst_addr:0x%x", DMAP2Img2o.dma_cfg.memBuf.ofst_addr);
		ISP_PATH_INF("alignment:0x%x", DMAP2Img2o.dma_cfg.memBuf.alignment);
    }
    ...
    if ( p_parameter->tpipe == CAM_MODE_TPIPE ) {
        ...
        ispTopCtrl.tcm_en = 0x90000000 | p_parameter->tcm_en;
...
int CamPathPass2::getCdpMuxSetting(struct CamPathPass2Parameter pass2Parameter, MINT32 *pDispVidSel)
{
    ...
    *pDispVidSel = -1; // set wrong initial value for debug
    ...
    if(enVido && endispo) { // vido:(1), dispo(1)
        ...
        if((!pass2Parameter.imgi.crop.w)||(!pass2Parameter.imgi.crop.h))
            ISP_PATH_ERR("pass2Parameter.imgi.crop.w(%d),pass2Parameter.imgi.crop.h(%d)",pass2Parameter.imgi.crop.w,pass2Parameter.imgi.crop.h);
        ...
        if(BASIC_SCALE_RATIO <= dispoXCropRatio && BASIC_SCALE_RATIO <= dispoYCropRatio) { // scale up
            ...
            dispoAbsRatio = (dispoCropRatio<<DIGITAL_ZOOM_FP_NUM) / BASIC_SCALE_RATIO;

        } else if (BASIC_SCALE_RATIO >= dispoXCropRatio && BASIC_SCALE_RATIO >= dispoYCropRatio) { // scale down

            dispoCropRatio = (dispoXCropRatio <= dispoYCropRatio) ? (dispoXCropRatio) : (dispoYCropRatio);

            dispoAbsRatio = (BASIC_SCALE_RATIO<<DIGITAL_ZOOM_FP_NUM) / dispoCropRatio;
        ...
        ISP_PATH_DBG("[dispo]cropRatiox(0x%x),cropRatioy(0x%x),cropRatio(0x%x),dispoAbsRatio(0x%x)",
                    dispoXCropRatio,dispoYCropRatio,dispoCropRatio,dispoAbsRatio);
        ...
        if(BASIC_SCALE_RATIO <= vidoXCropRatio && BASIC_SCALE_RATIO <= vidoYCropRatio) { // scale up
            ...
            vidoAbsRatio = (vidoCropRatio<<DIGITAL_ZOOM_FP_NUM) / BASIC_SCALE_RATIO;

        } else if (BASIC_SCALE_RATIO >= vidoXCropRatio && BASIC_SCALE_RATIO >= vidoYCropRatio) { // scale down

            vidoCropRatio = (vidoXCropRatio <= vidoYCropRatio) ? (vidoXCropRatio) : (vidoYCropRatio);
            vidoAbsRatio = (BASIC_SCALE_RATIO<<DIGITAL_ZOOM_FP_NUM) / vidoCropRatio;
        ...
        ISP_PATH_DBG("[vido]cropRatiox(0x%x),cropRatioy(0x%x),cropRatio(0x%x),vidoAbsRatio(0x%x)",
                    vidoXCropRatio,vidoYCropRatio,vidoCropRatio,vidoAbsRatio);
        ...
        if(enCurz) {
            ...
            }else if(dispoAbsRatio<vidoAbsRatio && dispoCropRatio>=PRZ_MIN_SCALE_RATIO && dispoCropRatio<=PRZ_MAX_SCALE_RATIO
                                && vidoCropRatio>=CURZ_MIN_SCALE_RATIO && vidoCropRatio<=CURZ_MAX_SCALE_RATIO) {
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
    ...
    tdriPipe.tdri.top.prz_opt_sel = ispTopCtrl.ctl_sel.bit_field.prz_opt_sel;
    tdriPipe.tdri.top.nr3d_dma_sel = ispTopCtrl.ctl_sel.bit_field.nr3d_dma_sel;	//pEn2->Bits.NR3D_EN;
    ISP_PATH_DBG("NR3D_EN(%d),nr3d_dma_sel(%d),NR3O_SEL_EN(%d)",pEn2->Bits.NR3D_EN,tdriPipe.tdri.top.nr3d_dma_sel,ispTopCtrl.ctl_mux_sel.bit_field.NR3O_SEL_EN);
    ...
    ISP_PATH_DBG("[Curz]HCoe(%d),VCoe(%d)",tdriPipe.tdri.curz.curz_horizontal_coeff_step,tdriPipe.tdri.curz.curz_vertical_coeff_step);
    ...
    ISP_PATH_DBG("setSimpleConfIdxNumVa(0x%x) segSimpleConfBufVa(0x%x)\n",tdriPipe.tdri.tdriCfg.setSimpleConfIdxNumVa,
...
ISP_DRV_WAIT_IRQ_STRUCT WaitIrq;
    ...
    ISP_PATH_INF("type(%d),irq(%d)",type,irq);
