...
//-----------------------------------------------------------------------------
CdpDrv* CdpDrv::CreateInstance(bool fgIsGdmaMode)
{
...
//-----------------------------------------------------------------------------
CdpDrv* CdpDrvImp::GetInstance(bool fgIsGdmaMode)
{
...
//-----------------------------------------------------------------------------
void CdpDrvImp::DestroyInstance(void)
{
    LOG_INF("");
}

//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::Init(void)
{
    MBOOL Result = MTRUE;
    ...
    LOG_INF(" - E. mInitCount: %d. mSysramUsageCount: %d. m_fgIsGdmaMode: %d.", mInitCount, mSysramUsageCount, (int)m_fgIsGdmaMode);
...
{
    LOG_VRB("- E. pIspReg: 0x%08x.", (UINT32)pIspReg);
...
{
    LOG_VRB("- E. pPhyIspDrv: 0x%08x.", (UINT32)pPhyIspDrv);
...
/// @return         OK or Fail. MTRUE: OK. MFALSE: Fail.
///////////////////////////////////////////////////////////////////////////
MBOOL CdpDrvImp::CheckReady(void)
{
    // 1. Check CDP Drv instance count. If count <= 0, it means no CDP Drv.
    if (mInitCount <= 0)
    {
...
)
{
    ...
    {
        LOG_DBG("Not support ratio. u4CroppedSize_H: %d, SizeOut_H: %d.", u4CroppedSize_H, SizeOut_H);
        Result = MFALSE;
        goto lbExit;
    }
    ...
    {
        LOG_DBG("Not support ratio. u4CroppedSize_V: %d, SizeOut_V: %d.", u4CroppedSize_V, SizeOut_V);
        Result = MFALSE;
        goto lbExit;
    }
...
)
{
    ...
    //
    LOG_VRB("- E. Algo: %d, IsWidth: %d, CoeffStep: %d, Offset: %f.", Algo, IsWidth, CoeffStep, Offset);
    ...
    //
    LOG_VRB("- X. LumaInt/Sub(%d, %d), ChromaInt/Sub(%d, %d).", *pLumaInt, *pLumaSub, *pChromaInt, *pChromaSub);
    //
    return MTRUE;
}
...
/// @return         OK or Fail. MTRUE: OK. MFALSE: Fail.
///////////////////////////////////////////////////////////////////////////
MBOOL CdpDrvImp::AllocateRotationBuf(void)
{
    MBOOL Result = MTRUE;


    LOG_INF("- E. mSysramUsageCount: %d.", mSysramUsageCount);
...
lbEXIT:
    LOG_INF("- X. ret: %d. mSysramUsageCount: %d.", Result, mSysramUsageCount);
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::FreeRotationBuf(void)
{
    MBOOL Result = MTRUE;
...
)
{
    ...
    LOG_DBG("- E. Luma/Chroma BufSize: (%d, %d). mSysramUsageCount: %d.", LumaBufSize, ChromaBufSize, mSysramUsageCount);
...
lbEXIT:
    LOG_DBG("- X. ret: %d. mSysramUsageCount: %d.", Result, mSysramUsageCount);
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
{
    LOG_VRB(" - E. eInFormat: %d. eInPlane: %d.", eInFormat, eInPlane);
    MBOOL Result = MTRUE;
    ...
    *pu4OutPlane = u4OutPlane;
    //
    LOG_VRB(" - X. Result: %d. u4OutPlane: %d.", *pu4OutPlane);
...
{
    LOG_VRB(" - E. eInFormat: %d. eInPlane: %d. eInSequence: %d.", eInFormat, eInPlane, eInSequence);
    MBOOL Result = MTRUE;
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CDRZ_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CDRZ_DumpReg(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
MBOOL CdpDrvImp::CDRZ_H_SetDeringThreshold(
    ...
    MUINT32     Threshold2)
{
    LOG_VRB("Threshold1(%d),Threshold2(%d)",Threshold1,Threshold2);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CDRZ_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CURZ_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CURZ_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::CURZ_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VRZ_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VRZ_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
MBOOL CdpDrvImp::VRZ_H_SetOffset(
    ...
    MUINT32     ChromaSub)
{
    LOG_VRB("LumaIntSub(%d, %d),ChromaInt/Sub(%d, %d)",LumaInt,LumaSub,ChromaInt,ChromaSub);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZ_V_SetOffset(
    ...
    MUINT32     ChromaSub)
{
    LOG_VRB("LumaInt/Sub(%d, %d), ChromaInt/Sub(%d, %d)",LumaInt,LumaSub,ChromaInt,ChromaSub);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VRZ_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::RSP_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::RSP_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::RSP_GetInterruptStatus(void)
{
    LOG_VRB("");
    MBOOL Result = MFALSE;
    //
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::PRZ_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::PRZ_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::PRZ_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VRZO_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VRZO_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
    ...
    CDP_DRV_SEQUENCE_ENUM   Sequence)
{
    MBOOL Result = MTRUE;
    //
    LOG_VRB("- E. InFormat: %d. InBlock: %d. InPlane: %d. InSequence: %d.", Format, Block, Plane, Sequence);
    //
    if (!CheckReady())
    {
        return MFALSE;
    ...
    //
    LOG_VRB("- X. Result : %d. OutFormat: %d. OutBlock: %d. OutPlane: %d. OutSequence: %d.", Result, Format, Block, u4OutPlane, Sequence);
...
{
    LOG_VRB("PaddingMode(%d)",PaddingMode);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetRotationBuf(
    ...
    MUINT32     Size)
{
    LOG_VRB("FIFO(%d),DoubleBuf(%d),LineNum(%d),Width(%d),Size(%d)",FIFO,DoubleBuf,LineNum,Width,Size);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetRotationBufAddr(
    ...
    MUINT32     ChromaAddr1)
{
    LOG_VRB("LumaAddr0(0x%08X),LumaAddr1(0x%08X),ChromaAddr0(0x%08X),ChromaAddr1(0x%08X)",LumaAddr0,LumaAddr1,ChromaAddr0,ChromaAddr1);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
{
    LOG_VRB("PriLowThr(%d),PriThr(%d),BurstLen(%d)",PriLowThr,PriThr,BurstLen);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetInterrupt(
    ...
    MBOOL   WriteClear)
{
    LOG_VRB("En(%d),WriteClear(%d)",En,WriteClear);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetOutputAddr(
    ...
    MUINT32     StrideC)
{
    LOG_VRB("PhyAddr(0x%08X),Offset(0x%08X),Stride(0x%08X)",PhyAddr,Offset,Stride);
    LOG_VRB("PhyAddrC(0x%08X),OffsetC(0x%08X),StrideC(0x%08X)",PhyAddrC,OffsetC,StrideC);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetDithering(
    ...
    MUINT32                 XRGBDummy)
{
    LOG_VRB("En(%d),Mode(%d),InitX(%d),InitY(%d),XRGBDummy(0x%02X)",En,Mode,InitX,InitY,XRGBDummy);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
MBOOL CdpDrvImp::VRZO_SetDitheringSeed(
    ...
    MUINT32     B)
{
    LOG_VRB("R(0x%08X),G(0x%08X),B(0x%08X)",R,G,B);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VIDO_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VIDO_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VIDO_GetResetStatus(void)
{
    LOG_VRB("");
    MBOOL Result = MFALSE;
    //
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VIDO_GetInterruptStatus(void)
{
    LOG_VRB("");
    MBOOL Result = MFALSE;
    //
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::VIDO_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::DISPO_ResetDefault(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::DISPO_DumpReg(void)
{
    LOG_VRB("");
    //
    if(!CheckReady())
...
MBOOL CdpDrvImp::DISPO_SetRotationBufAddr(
    ...
    MUINT32     ChromaAddr1)
{
    LOG_VRB(" - E. LumaAddr0: 0x%08X. LumaAddr1: 0x%08X. ChromaAddr0: 0x%08X. ChromaAddr1: 0x%08X.", LumaAddr0, LumaAddr1, ChromaAddr0, ChromaAddr1);
    //
    if(!CheckReady())
    {
        return MFALSE;
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::DISPO_GetResetStatus(void)
{
    LOG_VRB("");
    MBOOL Result = MFALSE;
    //
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::DISPO_GetInterruptStatus(void)
{
    LOG_VRB("");
    MBOOL Result = MFALSE;
    //
...
//-----------------------------------------------------------------------------
MBOOL CdpDrvImp::DISPO_Unconfig(void)
{
    LOG_DBG("");
    //
    if(!CheckReady())
