...
#define SET_TCM_SETTING \
    ...
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_IMGCI_EN)?0x00000004:0);/*(reg_4054[2])*/     \
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_VIPI_EN)? 0x00000008:0);/*(reg_4054[3])*/     \
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_VIP2I_EN)?0x00000010:0);/*(reg_4054[4])*/     \
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_LCEI_EN)? 0x00000040:0);/*(reg_4054[6])*/     \
    ...
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_DISPO_EN)?0x00002000:0);/*(reg_4054[13])*/    \
    tcm_en |= ((dma_en&CAM_CTL_DMA_EN_VIDO_EN)? 0x00004000:0);/*(reg_4054[14])*/    \
    tcm_en |= ((enable2&CAM_CTL_EN2_PRZ_EN)?    0x00008000:0);/*(reg_4054[15])*/    \
    tcm_en |= ((enable2&CAM_CTL_EN2_CDRZ_EN)?   0x00010000:0);/*(reg_4054[16])*/    \
    tcm_en |= ((enable2&CAM_CTL_EN2_CURZ_EN)?   0x00020000:0);/*(reg_4054[17])*/    \
    tcm_en |= ((enable1&CAM_CTL_EN1_UNP_EN)?    0x00100000:0);/*(reg_4054[20])*/    \
    tcm_en |= ((enable1&CAM_CTL_EN1_C02_EN)?    0x00200000:0);/*(reg_4054[21])*/    \
    tcm_en |= ((enable2&CAM_CTL_EN2_NR3D_EN)?   0x01000000:0);/*(reg_4054[24])*/
...
int idx_lcei = -1;
    ...
    int isEn1CfaStatusFixed = 0;  /* 0:Cfa(en1) can be clear */
    ...
    int isEn2Nr3dStatusFixed = 0;  /* 0:Nr3d(en2) can be clear */
    ...
    int pixel_byte_img2o = 2<<CAM_ISP_PIXEL_BYTE_FP;	//@TODO shift at here or in DMAO_B::_config
    ...
    switch (meScenarioFmt) {
        case eScenarioFmt_RAW:
            ...
            enable1 = CAM_CTL_EN1_CAM_EN|CAM_CTL_EN1_CFA_EN|CAM_CTL_EN1_UNP_EN;
...
*/
    ...
    switch (meScenarioID) {
        ...
        case eScenarioID_VSS:       //  video snap shot
            ...
            //
            if(m_Nr3dEn)
                ctl_mux_sel2 |= CAM_CTL_MUX_IMG2O_MUX_EN;
#else
...
//enable1 &= (~CAM_CTL_EN1_PAK_EN);
    ...
    m_camPass2Param.isEn2Nr3dStatusFixed = isEn2Nr3dStatusFixed;
    ...
    m_camPass2Param.bypass_ispImg2o = isImg2oStatusFixed;  // check img2o bypass
    ...
    if (-1 != idx_vipi ) {
        ...
        if(m_Nr3dEn){
            this->configDmaPort(&portInfo_vipi,m_camPass2Param.vipi,(MUINT32)pixel_byte_vipi,0,(MUINT32)1,ESTRIDE_1ST_PLANE);
        } else {
    ...
    //
	if (-1 != idx_img2o ) {
        PIPE_DBG("config img2o");
        PIPE_DBG("u4BufSize(0x%x)-u4BufVA(0x%x)-u4BufPA(0x%x)", vOutPorts[idx_img2o]->u4BufSize,vOutPorts[idx_img2o]->u4BufVA,
                    vOutPorts[idx_img2o]->u4BufPA);
        this->configDmaPort(vOutPorts[idx_img2o]
                , m_camPass2Param.img2o_dma	//@TODO check this
                , (MUINT32)pixel_byte_img2o
                , (MUINT32)0
                , (MUINT32)1
                , ESTRIDE_1ST_PLANE);
        m_camPass2Param.img2o_dma.size.stride = vOutPorts[idx_img2o]->u4Stride[ESTRIDE_1ST_PLANE];
        m_camPass2Param.img2o.img2o_stride = vOutPorts[idx_img2o]->u4Stride[ESTRIDE_1ST_PLANE]
        								* pixel_byte_img2o >> CAM_ISP_PIXEL_BYTE_FP;	//for tpipe
        //tpipe will update img2o crop ofst register if en.
        m_camPass2Param.img2o.img2o_crop_en = 1;
        if(m_Nr3dDmaSel) {
            m_camPass2Param.ctl_sel.bit_field.nr3d_dma_sel = 1;
            m_camPass2Param.ctl_mux_sel.bit_field.NR3O_SEL_EN = 1;
            m_camPass2Param.ctl_mux_sel.bit_field.NR3O_SEL = 0;
        } else {
            m_camPass2Param.ctl_sel.bit_field.nr3d_dma_sel = 0;
            m_camPass2Param.ctl_mux_sel.bit_field.NR3O_SEL_EN = 0;
            m_camPass2Param.ctl_mux_sel.bit_field.NR3O_SEL = 0;
        }

        PIPE_DBG("size:0x%x", m_camPass2Param.img2o_dma.memBuf.size);
        PIPE_DBG("base_vAddr:0x%x", m_camPass2Param.img2o_dma.memBuf.base_vAddr);
        PIPE_DBG("base_pAddr:0x%x", m_camPass2Param.img2o_dma.memBuf.base_pAddr);
        PIPE_DBG("ofst_addr:0x%x", m_camPass2Param.img2o_dma.memBuf.ofst_addr);
        PIPE_DBG("alignment:0x%x", m_camPass2Param.img2o_dma.memBuf.alignment);
	}
    if ( -1 != idx_dispo) {
