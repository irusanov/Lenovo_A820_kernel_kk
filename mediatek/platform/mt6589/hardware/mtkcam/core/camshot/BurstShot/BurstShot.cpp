/*
 *  0. determine interface
 *  1. fetch N raw frame
 *  2. apply AE settings to each frame
 *  3. multi-thread postproc
 *  4. multi-thread encode
 */
/* Copyright Statement:
 *
...
#define CAMIO_SKIP_REPEAT_BUFFER 1
...
start(SensorParam const & rSensorParam, MUINT32 shotNumber)
{
    ...
    pSensorHal->sendCommand(static_cast<halSensorDev_e>(rSensorParam.u4DeviceID),
    ...
    //
    if( u4TgInW * u4TgInH > 14.5e6 )
    {
        //for 16M sensor
        shotmode = 3;
    }
    else if( rSensorParam.u4RawType == 1 )
    {
        shotmode = 2;
    }
    else // u4RawType: 0, 2
    ...
    switch( shotmode )
    {
        case 1: //NCC
            mbCCmode = false;
            meProfile = EIspProfile_NormalCapture;
            if( mSensorParam.u4RawType == 1 )
                mSensorParam.u4RawType = 0;
            break;
        case 2: //CC
            mbCCmode = true;
            meProfile = EIspProfile_NormalCapture_CC;
            break;
        case 3: //16M
            mbCCmode = false;
            meProfile = EIspProfile_NormalCapture_16M;
    ...
    if (NULL == mpCamIOPipe)
    {
        mpCamIOPipe = ICamIOPipe::createInstance(mbCCmode?eSWScenarioID_VSS:eSWScenarioID_CAPTURE_NORMAL,
    ...
    if (ret && isDataMsgEnabled(ECamShot_DATA_MSG_YUV|ECamShot_DATA_MSG_POSTVIEW|ECamShot_DATA_MSG_JPEG))
    {
        ...
        for(MUINT32 i=0; i<mShotNumber; i++) {
            ...
            if(mCap3AParam) {
                ...
                p3AObj->setIspProfile( meProfile == EIspProfile_NormalCapture_16M ? EIspProfile_NormalCapture : static_cast<EIspProfile_T>(meProfile) );
...
#warning [TODO] this should callback from pipe
    ...
    //
    ret = ret
            && p3AObj->setIspProfile(static_cast<EIspProfile_T>(meProfile));
...
createSensorRawImgN(SensorParam const & rSensorParam, ImgBufInfo *rRawImgBufInfo, MUINT32 u4ShotCount, MUINT32 delayFrame)
{
    {   //debug
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("mediatek.burstshot.delay", value, "0");
        MUINT32 manualDelay = ::atoi(value);
        if(manualDelay) {
            delayFrame = manualDelay;
            MY_LOGD("manualDelay = %d", manualDelay);
        }
    }
    ...
    for(MUINT32 i=0; i<u4ShotCount; i++) {
        ...
        if(i==0) {
            ...
            for(MUINT32 d=0; d<delayFrame; d++) {
                ...
                BufInfo rBufInfo(rRawImgBufInfo[i].u4BufSize
#if CAMIO_SKIP_REPEAT_BUFFER
                                , rRawImgBufInfo[i].u4BufVA + (d + 1)*64 //workaround for camio can't enqueue two buffer with the same buffer addres
                                , rRawImgBufInfo[i].u4BufPA + (d + 1)*64 //workaround for camio can't enqueue two buffer with the same buffer addres
#else
                                , rRawImgBufInfo[i].u4BufVA
                                , rRawImgBufInfo[i].u4BufPA
#endif
                                , rRawImgBufInfo[i].i4MemID);
        ...
        if(!rSensorParam.fgBypassDelay) {
            for(MUINT32 d=0; d<delayFrame; d++) {
                if(i<u4ShotCount-1) {
...
#if CAMIO_SKIP_REPEAT_BUFFER
                    MY_LOGD("camio deque VA should be %p (delay frame)", rRawImgBufInfo[i].u4BufVA + (d + 1)*64); //workaround for camio can't enqueue two buffer with the same buffer addres
#else
                    MY_LOGD("camio deque VA should be %p (delay frame)", rRawImgBufInfo[i].u4BufVA);
#endif
...
#if CAMIO_SKIP_REPEAT_BUFFER
                    if(i<u4ShotCount-2) {
                        rQRawOutBuf.vBufInfo[1].u4BufVA = rRawImgBufInfo[i+1].u4BufVA + (d + 1)*64; //workaround for camio can't enqueue two buffer with the same buffer addres
                        rQRawOutBuf.vBufInfo[1].u4BufPA = rRawImgBufInfo[i+1].u4BufPA + (d + 1)*64; //workaround for camio can't enqueue two buffer with the same buffer addres
                    } else {
                        rQRawOutBuf.vBufInfo[1].u4BufVA = rRawImgBufInfo[i+1].u4BufVA; //workaround for camio can't enqueue two buffer with the same buffer addres
                        rQRawOutBuf.vBufInfo[1].u4BufPA = rRawImgBufInfo[i+1].u4BufPA; //workaround for camio can't enqueue two buffer with the same buffer addres
                    }
#else
                    ...
                    mpCamIOPipe->dequeBuf(PortID(EPortType_MemoryOut, 0, 1), rQRawOutBuf);
#if CAMIO_SKIP_REPEAT_BUFFER
                    MY_LOGD("camio deque VA should be %p (delayframe)", rRawImgBufInfo[i].u4BufVA + (d + 1)*64); //workaround for camio can't enqueue two buffer with the same buffer addres
#else
                    MY_LOGD("camio deque VA should be %p (delayframe)", rRawImgBufInfo[i].u4BufVA);
#endif
                    mpCamIOPipe->enqueBuf(PortID(EPortType_MemoryOut, 0, 1), rQRawOutBuf);
                }
            }
...
createYuvRawImg(ImgBufInfo const & rSrcImgBufInfo, Rect const rSrcCropRect, MUINT32 const u4Img1Rot, MUINT32 const u4Img1Flip, ImgBufInfo const & rDstImgBufInfo1, ImgBufInfo const &rDstImgBufInfo2 )
{
    ...
    IPostProcPipe    *pPostProcPipe = IPostProcPipe::createInstance(mbCCmode?eSWScenarioID_VSS:eSWScenarioID_CAPTURE_NORMAL,
                                                                    static_cast<EScenarioFmt>(mapScenarioType(rSrcImgBufInfo.eImgFmt)));
...
#if CAMIO_SKIP_REPEAT_BUFFER
            allocImgMem("SensorRaw", eImgFmt, u4SensorWidth+2, u4SensorHeight, mRawMem[i]); //workaround for camio can't enqueue two buffer with the same buffer addres
#else
