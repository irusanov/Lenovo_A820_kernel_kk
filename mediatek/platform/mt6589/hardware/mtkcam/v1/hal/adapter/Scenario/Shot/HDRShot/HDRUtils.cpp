...
updateInfo()
{
...
#endif
	mu4SourceSize = NSCamShot::queryImgBufSize(eImgFmt_I420, mu4W_yuv, mu4H_yuv);
	...
	//
	if(mu4W_yuv * mu4H_yuv < 10*1000000) {
		mHdrRoundTotal = 1;
	} else {
		mHdrRoundTotal = 2;
	}
...
requestFirstRunSourceImgBuf(void)
{
	...
	mu4FirstRunSourceSize = NSCamShot::queryImgBufSize(eImgFmt_I420, mu4W_first, mu4H_first);
	...
	for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
	{
		...
		{
			MY_ERR("[requestBufs] mpFirstRunSourceImgBuf[%d] fails to request %d bytes.", i, mu4FirstRunSourceSize);
			ret = MFALSE;
			goto lbExit;
		}
...
requestSmallImgBuf(void)
{
	...
	mu4SmallImgSize = NSCamShot::queryImgBufSize(eImgFmt_Y800, mu4W_small, mu4H_small);
...
requestSEImgBuf(void)
{
	...
	mu4SEImgSize = NSCamShot::queryImgBufSize(eImgFmt_Y800, mu4W_se, mu4H_se);
...
requestNormalJpegBuf(void)
{
	...
	mNormalJpegBuf.size = NSCamShot::queryImgBufSize(eImgFmt_JPEG, mShotParam.mi4PictureWidth, mShotParam.mi4PictureHeight);
...
requestNormalThumbnailJpegBuf(void)
{
	...
	mNormalThumbnailJpegBuf.size = NSCamShot::queryImgBufSize(eImgFmt_JPEG, mJpegParam.mi4JpegThumbWidth, mJpegParam.mi4JpegThumbHeight) * 2;
...
requestHdrJpegBuf(void)
{
	FUNCTION_LOG_START;
	MBOOL	ret = MTRUE;
    mHdrJpegBuf.size = NSCamShot::queryImgBufSize(eImgFmt_JPEG, mShotParam.mi4PictureWidth, mShotParam.mi4PictureHeight);
...
requestHdrThumbnailJpegBuf(void)
{
    ...
    mHdrThumbnailJpegBuf.size = NSCamShot::queryImgBufSize(eImgFmt_JPEG, mJpegParam.mi4JpegThumbWidth, mJpegParam.mi4JpegThumbHeight) * 2;
...
requestDownSizedWeightMapBuf(void)
{
	...
	mu4DownSizedWeightMapSize = NSCamShot::queryImgBufSize(eImgFmt_Y800, mu4W_dsmap, mu4H_dsmap);
...
requestPostviewImgBuf(void)
{
	...
	mu4PostviewImgSize = NSCamShot::queryImgBufSize(mPostviewFormat, mPostviewWidth, mPostviewHeight);
...
requestBlendingBuf()
{
	...
	mBlendingBuf.size = NSCamShot::queryImgBufSize(eImgFmt_I420, mu4W_yuv, mu4H_yuv);
...
allocateCaptureMemoryTask_First(MVOID* arg)
{
	...
	self->mu4FirstRunSourceSize = NSCamShot::queryImgBufSize(eImgFmt_I420, self->mu4W_first, self->mu4H_first);
	//MY_VERB("[requestBufs] mu4SourceSize: %d.", mu4FirstRunSourceSize);
	...
	self->mu4SmallImgSize = NSCamShot::queryImgBufSize(eImgFmt_Y800, self->mu4W_small, self->mu4H_small);
...
encodeHdrJpeg(MVOID *arg)
{
	...
	MUINT32 				u4HdrCroppedResultSize = NSCamShot::queryImgBufSize(eImgFmt_I420
