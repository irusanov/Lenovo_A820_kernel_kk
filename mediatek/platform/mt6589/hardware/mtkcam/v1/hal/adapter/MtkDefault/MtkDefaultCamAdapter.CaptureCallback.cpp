...
)
{
    ...
    if  ( image )
    {
        ...
        if(mpStateManager->isState(IState::eState_VideoSnapshot))
        {
            MY_LOGD("VSS+");
#if 1   //defined(MTK_CAMERA_BSP_SUPPORT)
            mpCamMsgCbInfo->mDataCb(MTK_CAMERA_MSG_EXT_DATA, image, 0, NULL, mpCamMsgCbInfo->mCbCookie);
            image->release(image);
#else
            mpCamMsgCbInfo->mDataCb(CAMERA_MSG_COMPRESSED_IMAGE, image, 0, NULL, mpCamMsgCbInfo->mCbCookie);
            image->release(image);
#endif
            MY_LOGD("VSS-");
        }
...
lbExit:
    if(mpStateManager->isState(IState::eState_VideoSnapshot))
    {
        MY_LOGD("Transit to recording state E");
        mpStateManager->transitState(IState::eState_Recording);
        //
        if ( ! mpResourceLock->Unlock(ResourceLock::eMTKVIDEO_VSS) )
        {
        ...
        MY_LOGD("Transit to recording state X");
    }
    //
    return  ret;
}
