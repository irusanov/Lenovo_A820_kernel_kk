...
private:
    ...
    threadLoop()
    {
        if  ( mfgIsFinalImage )
        {
            ...
            if (eShotMode_ZsdShot == mu4ShotMode || eShotMode_ContinuousShotCc == mu4ShotMode)
            {
        ...
        if  ( mfgIsFinalImage && (eShotMode_ContinuousShotCc == mu4ShotMode || eShotMode_ContinuousShot == mu4ShotMode) )
        {
#if 1   //defined(MTK_CAMERA_BSP_SUPPORT)
...
)
{
    ...
    if  ( image )
    {
        ...
        //
        sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, image, u4CallbackIndex, fgIsFinalImage, mu4ShotMode);
        if  ( pThread == 0 || pThread->run() != OK )
        {
