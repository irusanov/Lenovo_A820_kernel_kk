...
public:
    ...
    virtual bool            enqueProvider(unsigned int va, bool bIsFilled) {return false;};
...
resetBuffer()
{
    ...
    MY_LOGD("reset");
    //
    if(mImgBufQue.size() > 0)
    {
        list<CapBufQueNode>::iterator it;
        for(it = mImgBufQue.begin(); it != mImgBufQue.end(); it++)
        {
            (*it).mainImgNode.setStatus(ImgBufQueNode::eSTATUS_TODO);
            (*it).subImgNode.setStatus(ImgBufQueNode::eSTATUS_TODO);
        }
    }
    MY_LOGD("done");
}
