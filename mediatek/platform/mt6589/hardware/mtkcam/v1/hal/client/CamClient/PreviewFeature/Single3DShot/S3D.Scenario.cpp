...
ISShot(MVOID * bufadr, MVOID *arg1, MBOOL &shot)
{
    ...
    err = mpS3dshotObj->mHal3dfDoMotion((MUINT32 *)bufadr, (MUINT32 *) arg1, mMAVFrameWidth, mMAVFrameHeight);
...
mHalCamFeatureProc(MVOID * bufadr, int32_t& mvX, int32_t& mvY, int32_t& dir, MBOOL& isShot)
{
    ...
    MY_LOGD("[mHalCamS3dshotProc]");
    MINT32 err = NO_ERROR;
    isShot = false;
  
    
    AutoramaMotionResult S3dshotResult;
    memset((void*)&S3dshotResult,0,sizeof(AutoramaMotionResult));
    CreateMotionSrc(bufadr, mS3dshotFrameWidth, mS3dshotFrameHeight, (MVOID*)mpMotionBuffer.virtAddr);
    err = ISShot((MVOID*)mpMotionBuffer.virtAddr, &S3dshotResult, isShot);
    
    mStitchDir = (MTKPIPEAUTORAMA_DIRECTION_ENUM)S3dshotResult.Direction;
    MINT32 MV[3] = {S3dshotResult.MV_X, S3dshotResult.MV_Y, mStitchDir};
    if (mS3dshotFrameIdx == 0) 
    {
       isShot = true;
       S3dshotResult.MV_X = 0; 
       S3dshotResult.MV_Y = 0;
       mStitchDir = MTKPIPEAUTORAMA_DIR_NO;
       //mpMhalCam->mHalCam3ACtrl(1, 0, 1);
       //mpHal3AObj->setAutoExposureLock(0);
    }
    
    mp3dResult.ImageInfo[mS3dshotFrameIdx].MotionValue[0] = MV[0];
    mp3dResult.ImageInfo[mS3dshotFrameIdx].MotionValue[1] = MV[1];
    
    MY_LOGD("S3dshotResult.MV X: %d Y: %d dir %d isShot %d ", S3dshotResult.MV_X, S3dshotResult.MV_Y, mStitchDir, isShot); 
    if (isShot) 
    {
        mpHal3A->enableAELimiterControl(true);
        if (mS3dshotFrameIdx < S3dshotNum) 
        {
            MY_LOGD("[S3dshot]Num %d total %d", mS3dshotFrameIdx,S3dshotNum);
            
            MUINT8 *pbufIn = (MUINT8 *) bufadr;
            MUINT8 *pbufOut = (MUINT8 *) (mpframeBuffer.virtAddr + ((mS3dshotFrameWidth * mS3dshotFrameHeight * 3 / 2) * mS3dshotFrameIdx));
            memcpy(pbufOut, pbufIn, (mS3dshotFrameWidth * mS3dshotFrameHeight * 3 / 2) );
              
            #ifdef debug
            char sourceFiles[80];
            sprintf(sourceFiles, "%s%d.raw", "/sdcard/getpreview", mS3dshotFrameIdx);
            dumpBufToFile((char *) sourceFiles, pbufOut , (mS3dshotFrameWidth * mS3dshotFrameHeight * 3 / 2));
            #endif
        ...
        {
            MY_LOGD("[S3dshot]Num %d", mS3dshotFrameIdx);
            mS3dshotFrameIdx++; 
        }
    }    
    mvX = S3dshotResult.MV_X;
    mvY = S3dshotResult.MV_Y;
    dir = mStitchDir;
