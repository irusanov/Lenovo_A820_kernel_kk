...
MINT32 surfaceView::init()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        m_pM4UDrv->m4u_enable_m4u_func(M4U_CLNTMOD_OVL);        
    ...
    else
    {
        ...
        mfd = mFBfd;
        m_pFrameBuf = (MUINT8 *)mmap(0, mFinfo.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED, mfd, 0);
...
MINT32 surfaceView::uninit()
{
    ...
    if(g_overlayFlag == 0)
    {
        if(m_pM4UDrv != NULL)
        {
            m_pM4UDrv->m4u_disable_m4u_func(M4U_CLNTMOD_OVL);
