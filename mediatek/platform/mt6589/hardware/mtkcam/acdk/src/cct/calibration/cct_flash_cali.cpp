...
void FC_init(AcdkBase* p)
{
	...
	if(ini==1)
	{
		...
		g_baseObj->sendcommand(ACDK_CMD_PREVIEW_START/*ACDK_CCT_OP_PREVIEW_LCD_START*/, (UINT8 *)&rCCTPreviewConfig, sizeof(ACDK_CCT_CAMERA_PREVIEW_STRUCT), (MUINT8 *)0, 0, &u4RetLen22);
		usleep(5000000);
		//------------------------
	}
...
MRESULT takePicture2(fcExpPara* para, fcRgb* rgbRet)//MUINT32 a_i4PreCap, eACDK_CAP_FORMAT type)
{
