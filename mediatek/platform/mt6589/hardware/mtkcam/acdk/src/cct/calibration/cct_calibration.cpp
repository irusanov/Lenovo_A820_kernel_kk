...
MRESULT AcdkCalibration::mrAnalyzeRAWImage()
{
    ...
    */
    ...
    if (FAILED(mrRet))
    {
        ...
        mrRet = E_ACDK_CALIBRATION_API_FAIL;
    }
                                        ...
                                        ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    */
    ...
    //m_pAcdkBaseObj->sendcommand(ACDK_CCT_OP_AE_CAPTURE_MODE, (MUINT8 *)&a_i4PreCap, sizeof(a_i4PreCap), NULL, 0, &u4RetLen);
    ...
    for (MINT32 i4Exp = a_i4ExpStart; i4Exp <= a_i4ExpEnd; i4Exp += a_i4ExpInterval)
    {
        ...
        */
        ...
        m_pAcdkBaseObj->sendcommand(ACDK_CCT_OP_AE_SET_CAPTURE_PARA,
                                                        (UINT8 *)&ae_cfg,
                                                        sizeof(ae_cfg),
                                                        (UINT8 *)&ae_cfg,
                                                        sizeof(ae_cfg),
        ...
        *   Take Picture
        */
        if((mrRet = takePicture(a_i4PreCap, PURE_RAW8_TYPE)) != S_ACDK_CALIBRATION_OK)
        {
        ...
        *   Raw Analysis
        */
        if((mrRet = mrAnalyzeRAWImage()) != S_ACDK_CALIBRATION_OK) {
...
Exit:
    ...
    if(ae_cfg.u4ExposureMode) {    //Restore to expo. time base
                                     ...
                                     ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    *   Take Picture
    */
    if((mrRet = takePicture(a_i4PreCap, PROCESSED_RAW8_TYPE)) != S_ACDK_CALIBRATION_OK)
    {
        ACDK_LOGE("[takePicture] Get RAW Fail \n");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
...
//#if DOUBLE_BUFFER_WA
	...
	{
		case PREVIEW_MODE:
			LscMgr::getInstance()->RawLscTblClear(LscMgr::LSC_SCENARIO_01, ColorTemp);// pv  frame
			break;
		case CAPTURE_MODE:
			LscMgr::getInstance()->RawLscTblClear(LscMgr::LSC_SCENARIO_04, ColorTemp);// cap tile
			break;
		case VIDEO_MODE:
			LscMgr::getInstance()->RawLscTblClear(LscMgr::LSC_SCENARIO_09_17, ColorTemp);// vdo frame
			break;
		default:
		break;
	}
