    ...
    ,mIsFacotory(MFALSE)
{
    ...
    //====== CCT ======
#if (CCT_SUPPORT)
    m_pCCTIFObj = CCTIF::createInstance();
    if(!m_pCCTIFObj)
    {
        ACDK_LOGE("Can not create CCTIF obj");
    }
#endif
...
MINT32 AcdkMain::init()
{
    ...
    //======  CCT init ======
#if CCT_SUPPORT
    err = m_pCCTIFObj->init(mSensorDev);
    m_pCCTIFObj->setCCTSensorDev(mSensorDev);
#endif
...
MINT32 AcdkMain::uninit()
{
    ...
    if(m_pCCTIFObj!= NULL)
    {
        err = m_pCCTIFObj->uninit();
        if (err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("Faile to uninit m_pCCTIFObj(0x%x)", err);
        }
    }
#endif
                ...
                MINT32  const isSaveImg)
{
    ...
    MUINT32 qvFormat = eImgFmt_RGB565, capFormat = eImgFmt_YUY2;
...
)
{
    ...
    ACDK_LOGD("[AcdkMain-sendcommand] 0x%x", a_u4Ioctl);

#if (CCT_SUPPORT)

    if (a_u4Ioctl >= CCT_ISP_FEATURE_START && a_u4Ioctl < CCT_ISP_FEATURE_START + MAX_SUPPORT_CMD)
    {
        ACDK_LOGD("ISP Feature");
        err = m_pCCTIFObj->ispCCTFeatureControl(a_u4Ioctl,
                                                puParaIn,
                                                u4ParaInLen,
                                                puParaOut,
                                                u4ParaOutLen,
    ...
    {
        ACDK_LOGD("Sensor Feature, SensorDev(%d)", mSensorDev);

        err = m_pCCTIFObj->setCCTSensorDev(mSensorDev);
        if(err != ACDK_RETURN_NO_ERROR) {
            ACDK_LOGD("Unsupported sensor device");

            return ACDK_RETURN_INVALID_SENSOR;
        }

        err = m_pCCTIFObj->sensorCCTFeatureControl(a_u4Ioctl,
                                                   puParaIn,
                                                   u4ParaInLen,
                                                   puParaOut,
                                                   u4ParaOutLen,
    ...
    {
        ACDK_LOGD("CCT_NVRAM_FEATURE");
        
        err = m_pCCTIFObj->nvramCCTFeatureControl(a_u4Ioctl,
                                                  puParaIn,
                                                  u4ParaInLen,
                                                  puParaOut,
                                                  u4ParaOutLen,
    ...
    {
        ACDK_LOGD("CCT_3A_FEATURE_START");
        
        err = m_pCCTIFObj->aaaCCTFeatureControl(a_u4Ioctl,
                                                puParaIn,
                                                u4ParaInLen,
                                                puParaOut,
                                                u4ParaOutLen,
    ...
    else if(a_u4Ioctl == ACDK_CCT_FEATURE_SET_SRC_DEV)
    {
        ...
        if(err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("ACDK_CCT_FEATURE_SET_SRC_DEV fail(0x%x)",err);
        }
    }
    ...
    else if(a_u4Ioctl == ACDK_CCT_OP_PREVIEW_LCD_START)
    {
        ...
        if(err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("ACDK_CCT_OP_PREVIEW_LCD_START fail(0x%x)",err);
        }
    }
    else if(a_u4Ioctl == ACDK_CCT_OP_PREVIEW_LCD_STOP)
    {
        ACDK_LOGD("CCTOP_PreviewStop");
        err = stopPreview();

        if(err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("ACDK_CCT_OP_PREVIEW_LCD_STOP fail(0x%x)",err);
        }
    }
    else if(a_u4Ioctl == ACDK_CCT_OP_SINGLE_SHOT_CAPTURE_EX)
    {
        ...
        // To differentiate between  factory & meda mode using different enum
        if(pStillCapConfig->eOperaMode == ACDK_OPT_FACTORY_MODE)  // RAW
        {
            switch(pStillCapConfig->eOutputFormat)
            {
                case 4 : pStillCapConfig->eOutputFormat  = OUTPUT_PURE_RAW8;
                    break;
                case 8 : pStillCapConfig->eOutputFormat = OUTPUT_PURE_RAW10;
                    break;
                case 10 : pStillCapConfig->eOutputFormat = OUTPUT_PROCESSED_RAW8;
                    break;
                case 20 : pStillCapConfig->eOutputFormat = OUTPUT_PROCESSED_RAW10;
                    break;
                case 80 : pStillCapConfig->eOutputFormat = OUTPUT_YUV;
                    break;
                case 40 : pStillCapConfig->eOutputFormat = OUTPUT_JPEG;
                    break;
                default : pStillCapConfig->eOutputFormat  = OUTPUT_PURE_RAW8;
                    break;
            }
        }
        
        ACDK_LOGD("CCTOPCap Start ");
        ACDK_LOGD("Capture Mode:%d", pStillCapConfig->eCameraMode);
        ACDK_LOGD("Operation Mode:%d", pStillCapConfig->eOperaMode);
        ACDK_LOGD("Format:%d", pStillCapConfig->eOutputFormat);
        ...
        if (pStillCapConfig->eOutputFormat == OUTPUT_JPEG)
        {
            err = takePicture(pStillCapConfig->eCameraMode,
                              ...
                              1,
                              pStillCapConfig->i4IsSave);
        }
        else if(pStillCapConfig->eOutputFormat == OUTPUT_PURE_RAW8)
        {
            err = takePicture(pStillCapConfig->eCameraMode, 
                              PURE_RAW8_TYPE, 
                              pStillCapConfig->fpCapCB,
        ...
        else if(pStillCapConfig->eOutputFormat == OUTPUT_PURE_RAW10)
        {
            err = takePicture(pStillCapConfig->eCameraMode, 
                              PURE_RAW10_TYPE, 
                              pStillCapConfig->fpCapCB,
        ...
        else if(pStillCapConfig->eOutputFormat == OUTPUT_PROCESSED_RAW8)
        {
            err = takePicture(pStillCapConfig->eCameraMode, 
                              PROCESSED_RAW8_TYPE, 
                              pStillCapConfig->fpCapCB,
        ...
        else if(pStillCapConfig->eOutputFormat == OUTPUT_PROCESSED_RAW10)
        {
            err = takePicture(pStillCapConfig->eCameraMode, 
                              PROCESSED_RAW10_TYPE, 
                              pStillCapConfig->fpCapCB,
                              0,
                              0,
                              1,
        ...
        if(err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("ACDK_CCT_OP_SINGLE_SHOT_CAPTURE_EX fail(0x%x)",err);
        }
    }
    else if(a_u4Ioctl == ACDK_CCT_OP_MULTI_SHOT_CAPTURE_EX)
    {
        ...
        ACDK_LOGD("CCTOPCap Start ")
        ACDK_LOGD("Mode:%d", pMultiCapConfig->eCameraMode);
        ACDK_LOGD("Format:%d", pMultiCapConfig->eOutputFormat);
        ACDK_LOGD("Width:%d", pMultiCapConfig->u2JPEGEncWidth);
        ACDK_LOGD("Height:%d", pMultiCapConfig->u2JPEGEncHeight);
        ACDK_LOGD("Count:%d", pMultiCapConfig->u4CapCount);
        ...
        for (UINT32 i = 0 ; i < pMultiCapConfig->u4CapCount; i++)
        {
            ...
            else if (pMultiCapConfig->eOutputFormat == OUTPUT_PURE_RAW)
            {
                err = takePicture(pMultiCapConfig->eCameraMode,
                                  RAW_TYPE,
                                  pMultiCapConfig->fpCapCB
        ...
        ACDK_LOGD("[cotta--temp] not ready");
    }
