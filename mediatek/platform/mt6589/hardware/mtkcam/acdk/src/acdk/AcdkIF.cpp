...
MBOOL MDK_IOControl(MUINT32 a_u4Ioctl, ACDK_FEATURE_INFO_STRUCT *a_prAcdkFeatureInfo)
{
    ...
    if(a_u4Ioctl > ACDK_COMMAND_START && a_u4Ioctl < ACDK_COMMAND_END)
    {
        ...
        if(a_u4Ioctl == ACDK_CMD_PREVIEW_START)
            a_u4Ioctl = ACDK_CCT_OP_PREVIEW_LCD_START;
        else if(a_u4Ioctl == ACDK_CMD_PREVIEW_STOP)
            a_u4Ioctl = ACDK_CCT_OP_PREVIEW_LCD_STOP;
        else if(a_u4Ioctl == ACDK_CMD_CAPTURE)
            a_u4Ioctl = ACDK_CCT_OP_SINGLE_SHOT_CAPTURE_EX;
        else if(a_u4Ioctl == ACDK_CMD_QV_IMAGE)
            a_u4Ioctl = ACDK_CCT_FEATURE_QUICK_VIEW_IMAGE;
        else if(a_u4Ioctl == ACDK_CMD_RESET_LAYER_BUFFER)
            a_u4Ioctl = ACDK_CCT_FEATURE_RESET_LAYER_BUFFER;
        else  if(a_u4Ioctl == ACDK_CMD_SET_SRC_DEV)
            a_u4Ioctl = ACDK_CCT_FEATURE_SET_SRC_DEV;
        else  if(a_u4Ioctl == ACDK_CMD_SET_OPERATION_MODE)
            a_u4Ioctl = ACDK_CCT_FEATURE_SET_OPERATION_MODE;
        bRet = g_pAcdkBaseObj->sendcommand (a_u4Ioctl, 
                                            a_prAcdkFeatureInfo->puParaIn, 
                                            a_prAcdkFeatureInfo->u4ParaInLen, 
                                            a_prAcdkFeatureInfo->puParaOut, 
