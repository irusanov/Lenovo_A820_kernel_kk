    ...
    m_pInputFile(NULL)
{
    ...
    mDualMicTool_micGain_default = VolumeCustomParam.audiovolume_mic[VOLUME_NORMAL_MODE][4];
    if (mDualMicTool_micGain_default > UPLINK_GAIN_MAX)
    {
        mDualMicTool_micGain_default = UPLINK_GAIN_MAX;
    }
...
status_t AudioParamTuning::enableModemPlaybackVIASPHPROC(bool bEnable, bool bWB)//need record path?
{
    ...
    if (bEnable && (isPlaying() == false))
    {
        ...
        mAudioResourceManager->SetAudioMode((mSpeechDriverFactory->GetActiveModemIndex() == MODEM_1) ? AUDIO_MODE_IN_CALL : AUDIO_MODE_IN_CALL_2);
        switch (mMode)
        {
            case SPEECH_MODE_NORMAL:
            {
...
status_t AudioParamTuning::enableDMNRModem2Way(bool bEnable, bool bWBMode, unsigned short outputDevice, unsigned short workMode)
{
    ...
    if (bEnable && (isPlaying() == false))
    {
        ...
        if (m_pOutputFile == NULL)
        {
            ...
            if (m_pInputFile != NULL) { fclose(m_pInputFile); }
            pthread_mutex_unlock(&mP2WMutex);
            return BAD_VALUE;
        }
...
status_t AudioParamTuning::enableDMNRAtApSide(bool bEnable, bool bWBMode, unsigned short outputDevice, unsigned short workMode)
{
    ...
    if (bEnable && (isPlaying() == false))
    {
        ...
        mAudioResourceManager->setUlInputDevice(AUDIO_DEVICE_IN_BUILTIN_MIC);
        //set MIC gain
        mAudioVolumeInstance->SetMicGainTuning(Idle_Normal_Record, mDualMicTool_micGain);

        //set output and output gain in dB
        if ((!workMode) && (outputDevice == OUTPUT_DEVICE_RECEIVER))
        {
            mAudioResourceManager->setDlOutputDevice(AUDIO_DEVICE_OUT_EARPIECE);
            setPlaybackVolume(VOLUME_NORMAL_MODE, mDualMicTool_receiverGain);
            SXLOGD("Play+Rec set dual mic at apside, receiver gain: %d", mDualMicTool_receiverGain);
        }
        else if (!workMode)
        {
            mAudioResourceManager->setDlOutputDevice(AUDIO_DEVICE_OUT_WIRED_HEADSET);
            setPlaybackVolume(VOLUME_HEADSET_MODE, mDualMicTool_headsetGain);
            SXLOGD("Play+Rec set dual mic at apside, headset gain: %d", mDualMicTool_headsetGain);
        }
    ...
    else if (!bEnable && m_bDMNRPlaying)
    {
        ...
        usleep(200 * 1000);
        //set back MIC gain
        mAudioVolumeInstance->SetMicGainTuning(Idle_Normal_Record, mDualMicTool_micGain_default);
