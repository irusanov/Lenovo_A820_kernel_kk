...
AudioDigitalControl::AudioDigitalControl()
{
    ...
    for (int i = 0; i < sizeof(mAudioMcuMode)/sizeof(AudioIrqMcuMode); i++)
    {
        memset((void *)&mAudioMcuMode[i], 0, sizeof(AudioIrqMcuMode));
    }
    memset((void *)&mMrgIf, 0, sizeof(AudioMrgIf));
    mMrgIf.Mrg_I2S_SampleRate = AudioMrgIf::MRFIF_I2S_44K;

    memset((void *)&mDaiBt, 0, sizeof(mDaiBt));
...
{
    ALOGD("MT6589 doesn't support %s()\n", __FUNCTION__);
    return NO_ERROR;
}
...
status_t AudioDigitalControl::SetMrgI2SEnable(bool bEnable, unsigned int sampleRate)
{
    if (WCNChipController::GetInstance()->IsFMMergeInterfaceSupported() == true ||
        WCNChipController::GetInstance()->IsBTMergeInterfaceSupported() == true)
    {
        ...
        else
        {
            // To disable MrgI2S
            mAfeReg->SetAfeReg(AFE_MRGIF_CON, 0, 1 << 16); // Turn off I2S
            if (mMrgIf.MrgIf_En == true && mDaiBt.mDAIBT_ON == false)
            {
        ...
        mAudioDigitalBlock[AudioDigitalType::MRG_I2S_IN] = bEnable;
        // mAudioDigitalBlock[AudioDigitalType::MRG_I2S_OUT] = bEnable; // FM Tx??
    }
    return NO_ERROR;
}
...
#if 0
status_t AudioDigitalControl::SetMrgI2SLoopBack(bool bEnable)
{
#if defined(MTK_MERGE_INTERFACE_SUPPORT)
#if defined(AUDIO_MRG_LOOPBACK_TEST)

    ALOGD("+%s(), Set MergeIf HW of FM Chip loopback\n", __FUNCTION__);

    if (bEnable == true)
    {
        if (mMrgIf.MrgIf_En == true)
        {
            mAfeReg->SetAfeReg(AFE_MRGIF_CON, 1 << 19, 1 << 19); // set mergeif i2s loopback
        }
        else
        {
            // turn on merge Interface from off state
    ...
    {
        mAfeReg->SetAfeReg(AFE_MRGIF_CON, 0, 1 << 19); // disable  mergeif i2s loopback
    }
...
status_t AudioDigitalControl::SetMrgSineGen(bool bEnable)
{
#if defined(MTK_MERGE_INTERFACE_SUPPORT)
#if defined(AUDIO_MRG_LOOPBACK_TEST)

    ALOGD("+%s(), Set MergeIf HW of FM Chip SineGen %d\n", __FUNCTION__, bEnable);


    if (bEnable == true)
    {
        mAfeReg->SetAfeReg(AFE_SGEN_CON0, 0x747e17e1, 0xffffffff);  //O00 O01
    }
    else
    {
        mAfeReg->SetAfeReg(AFE_SGEN_CON0, 0x0 , 0xffffffff);    //O00 O01
    }
#endif
#endif
    return NO_ERROR;
...
status_t AudioDigitalControl::SetMrgBTSineGen(bool bEnable)
{
#if defined(MTK_MERGE_INTERFACE_SUPPORT)
#if defined(AUDIO_MRG_LOOPBACK_TEST)
    ...
    if (bEnable == true)
    {
        mAfeReg->SetAfeReg(AFE_SGEN_CON0, 0x840e10e1, 0xffffffff);  //O00 O01
    }
    else
    {
        mAfeReg->SetAfeReg(AFE_SGEN_CON0, 0x0 , 0xffffffff);    //O00 O01
