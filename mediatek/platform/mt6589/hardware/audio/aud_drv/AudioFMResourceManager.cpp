...
status_t AudioFMResourceManager::SetFmSourceModuleEnable(const bool enable)
{
    ...
    if (mWCNChipController->IsFMMergeInterfaceSupported() == true) // Merge Interface
    {
        // No HW SRC, check sampling rate here
        ASSERT(fm_chip_i2s_sample_rate == GetFmUplinkSamplingRate());

        // Audio Merge I2S master mode only
        ASSERT(mWCNChipController->IsFmChipUseSlaveMode() == true);

        // Enable/Disable Mrg I2S IN
        mAudioDigitalInstance->SetMrgI2SEnable(enable, fm_chip_i2s_sample_rate);
    }
    else // Enable 2nd I2S
    {
        if (enable == true)
        {
            ...
            // No HW ASRC, Check Sampling Rate Here
            ASSERT(fm_chip_i2s_sample_rate == GetFmUplinkSamplingRate());
...
status_t AudioFMResourceManager::SetFmDirectConnection(const bool enable)
{
    ...
    {
        // Set InterConnection
        if (mWCNChipController->IsFMMergeInterfaceSupported() == true)
        {
        ...
        mAudioDigitalInstance->SetHwDigitalCurrentGain(0, AudioDigitalType::HW_DIGITAL_GAIN2);
