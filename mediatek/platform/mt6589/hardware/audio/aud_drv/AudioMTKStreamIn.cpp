...
#define DAIBT_WB_SAMPLE_RATE (16000)
...
status_t AudioMTKStreamIn::SetClientSourceandMemType(AudioMTKStreamInClient *mStreamInClient)
{
    ...
    switch (mStreamInClient->mAttributeClient->mSource)
    {
        ...
        case AUDIO_SOURCE_MATV:
            if (AudioMATVController::GetInstance()->GetMatvType() == MATV_DIGITAL)
            {
    ...
    if (mStarting == false)
    {
        ...
        switch (mAudioResourceManager->GetAudioMode())
        {
            ...
            case AUDIO_MODE_IN_COMMUNICATION:
            {
                ...
                if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mUsingMagiASR || mVoIPRunning || mHDRecTunning16K)
                {
                    ...
                    const uint32_t TempSRCBufferSize = 0x1000; //need check the MemVULBufferSize buffer size
                    if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mUsingMagiASR || mVoIPRunning || mHDRecTunning16K)
                    {
                        process_sample_rate = VOICE_RECOGNITION_RECORD_SAMPLE_RATE;
                    }

                    ALOGD("need src? ori_sample_rate=%d, process_sample_rate=%d,num_channel=%d,mAttribute.mBufferSize=%x,TempSRCBufferSize=%x", num_sample_rate, process_sample_rate, num_channel, mAttribute.mBufferSize, TempSRCBufferSize);
                    if (process_sample_rate != num_sample_rate)
                    {
                        ...
                        mBliHandler1Buffer = new char[TempSRCBufferSize]; // tmp buffer for blisrc out
                        ...
                        mBliHandler2Buffer = new char[TempSRCBufferSize]; // tmp buffer for blisrc out
...
#if defined(MTK_DUAL_MIC_SUPPORT) && defined(MTK_HANDSFREE_DMNR_SUPPORT)
    //get DMNR param
...
void AudioMTKStreamIn::ConfigHDRecordParams(SPE_MODE mode)
{
...
#endif
    int RoutePath = GetVoIPRoutePath();

    ALOGD("ConfigHDRecordParams mode=%d,mStereoMode=%d, input source= %d, mdevices=%x,RoutePath=%d", mode, mStereoMode, mAttribute.mSource, mAttribute.mdevices, RoutePath);
    //speech parameters+++
    ...
    if (mode == SPE_MODE_VOIP) //VoIP case
    {
        ...
        if ((RoutePath == ROUTE_NORMAL) && mAudioSpeechEnhanceInfoInstance->GetDynamicSpeechEnhancementMaskOnOff(SPH_ENH_DYNAMIC_MASK_DMNR))
        {
        ...
        else if ((RoutePath == ROUTE_SPEAKER) && mAudioSpeechEnhanceInfoInstance->GetDynamicSpeechEnhancementMaskOnOff(SPH_ENH_DYNAMIC_MASK_LSPK_DMNR))  //speaker path
        {
            for (int i = 0; i < NUM_ABFWB_PARAM; i++)
            {
    ...
    else
    {
        ...
        if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mUsingMagiASR || mHDRecTunning16K)
        {
            mpSPELayer->SetSampleRate(mode, VOICE_RECOGNITION_RECORD_SAMPLE_RATE);
...
void AudioMTKStreamIn::CheckHDRecordVoIPSwitch(void)
{
    ...
    {
        ALOGD("CheckHDRecordVoIPSwitch enter REC, mUsingMagiASR=%d", mUsingMagiASR);
...
bool AudioMTKStreamIn::CheckVoIPNeeded(void)
{
    ...
    else
    {
        ...
        if (bNormalModeVoIP)
        {
            ...
            if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mUsingMagiASR || (mHDRecordSceneIndex != -1))
            {
                bRet = false;
            }
            else
...
int AudioMTKStreamIn::GetVoIPRoutePath(void)
{
    int RoutePath;
...
bool AudioMTKStreamIn::IsVoIPRouteChange(void)
{
    ...
    int new_route = GetVoIPRoutePath();
