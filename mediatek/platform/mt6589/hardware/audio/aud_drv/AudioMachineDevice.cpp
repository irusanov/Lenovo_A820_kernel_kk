...
void AudioMachineDevice::GetSleepEver()
{
    mDeviceSleepEver = ioctl(mFd, AUDDRV_GET_SUSPEND_EVER, NULL);
}

void AudioMachineDevice::ClearSleepEver()
{
    mDeviceSleepEver = false;
    ioctl(mFd, AUDDRV_CLEAR_SUSPEND_EVER, NULL);
}

bool AudioMachineDevice::CheckSleepEver()
{
    return mDeviceSleepEver;
}
...
status_t AudioMachineDevice::StartSpkTrimFunction()
{
       ...
       mAudioAnalogReg->SetAnalogReg(0x0104 , 0x0604, 0xffff); // close speaker powe
#else
    if (GetChipVersion() == CHIP_VERSION_E1)
    {
        GetSPKTrimOffset();
    ...
    else
    {
        ...
        AnalogClose(AudioAnalogType::DEVICE_OUT_SPEAKERL);
    }
#endif
    return NO_ERROR;
}
...
status_t AudioMachineDevice::AnalogOpen(AudioAnalogType::DEVICE_TYPE DeviceType)
{
    ...
    switch (DeviceType)
    {
        ...
        case AudioAnalogType::DEVICE_OUT_HEADSETL:
            ...
            if (CheckSleepEver() == true)
            {
                ALOGD("Analogpen DEVICE_OUT_HEADSETL sleep");
                usleep(30 * 1000);
            }
