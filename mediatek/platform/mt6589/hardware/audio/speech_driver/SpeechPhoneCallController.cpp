...
{
    return (audio_mode == AUDIO_MODE_IN_CALL || audio_mode == AUDIO_MODE_IN_CALL_2);
}
...
bool SpeechPhoneCallController::CheckTtyNeedOn() const
{
    ...
    const bool bt_device_on = android_audio_legacy::AudioSystem::isBluetoothScoDevice((android_audio_legacy::AudioSystem::audio_devices)GetRoutingForTty());
    return (bt_device_on == false && mVtNeedOn == false && mTty_Ctm != AUD_TTY_OFF && mTty_Ctm != AUD_TTY_ERR);
#else
    return false;
#endif
}
