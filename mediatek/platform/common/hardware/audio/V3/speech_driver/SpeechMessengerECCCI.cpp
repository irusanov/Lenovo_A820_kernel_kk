...
status_t SpeechMessengerECCCI::Initial()
{
    ...
    mECCCIShareBuf = new char[32000];
    share_buf_length = 32000;
    mCountCommand = 0;

    share_buf_address =  mECCCIShareBuf;
...
status_t SpeechMessengerECCCI::ReadMessage(ccci_buff_t &ccci_buff)
{
    ...
    if (length_read != -1)
    {
        ALOGD("ReadMessage ECCCI, length_read=%d, data[0](magic)=0x%x, data[1](message)=0x%x, ch=%d, reserved=0x%x", length_read, ccci_buff.magic, ccci_buff.message, ccci_buff.channel, ccci_buff.reserved);
    }
    modem_status = GetModemCurrentStatus();
    //dump
    if (GetMessageID(ccci_buff) == MSG_M2A_PCM_REC_DATA_NOTIFY)
    {
        ...
        if (mCountCommand <= 10)
        {
            ...
            char filename[128] ;
            sprintf(filename, "/sdcard/mtklog/ccci_payload_n%d.pcm", mCountCommand);
            FILE *fp = fopen(filename, "wb+");
            fwrite((char *)ccci_buff.payload + CCCI_PAYLOAD_BUFF_HEADER_LEN, sizeof(char), GetMessageLength(ccci_buff) - CCCI_PAYLOAD_BUFF_HEADER_LEN, fp);
            fclose(fp);
        }
#endif

#if 0//Tina_PCM_Record Test Dump
        mCountCommand++;
        {

            char filename[128] ;
            sprintf(filename, "/sdcard/mtklog/mobilelog/ccci_payload.pcm");
    ...
    while (pCCCI->CCCIEnable)
    {
        ...
        switch (m2a_message_id)
        {
...
#ifdef Tina_PCM_Record
                    // Phone record
                    AudioALSACaptureHandlerVoice *mAudioALSACaptureHandlerVoice = AudioALSACaptureHandlerVoice::getInstance();
                    mAudioALSACaptureHandlerVoice->CopyBufferToClientIncall(pCCCI->GetM2AUplinkBuffer(ccci_buff), Bytes_PCM);
#endif
            ...
            case MSG_M2A_REC_DATA_NOTIFY:   // meaning that we are recording, modem have some data
            {
                ...
                {
                    SLOGV("MSG_M2A_REC_DATA_NOTIFY(0x%x), data_length: %d", ccci_buff.message, pCCCI->GetMessageLength(ccci_buff) - CCCI_PAYLOAD_BUFF_HEADER_LEN);
...
char *SpeechMessengerECCCI::GetM2AUplinkBuffer(const ccci_buff_t &ccci_buff)
{
    uint16_t bytes_payload = GetMessageLength(ccci_buff);
    char *ul_buf = new char [bytes_payload];

    memcpy(ul_buf, (char *)ccci_buff.payload, bytes_payload);
    // share buffer header
    char *p_sync_word = ul_buf + 0; // 2 * size(unsigned short)
    char *p_data_type = ul_buf + 2; // 2 * size(unsigned short)
    char *p_data_len  = ul_buf + 4; // 2 * size(unsigned short)
    char *p_data_curIdx = ul_buf + 6; // 2 * size(unsigned short)
    char *p_data_totalIdx  = ul_buf + 8; // 2 * size(unsigned short)
    ...
    ul_buf += CCCI_PAYLOAD_BUFF_HEADER_LEN;//returned buffer skip the header

#if 0//Tina_PCM_Record Test Dump
    char filename[128] ;
    sprintf(filename, "/sdcard/mtklog/mobilelog/M2AUplinkBuffer.pcm");
    FILE *fp = fopen(filename, "ab+");
    fwrite((char *)ul_buf, sizeof(char), GetMessageLength(ccci_buff) - CCCI_PAYLOAD_BUFF_HEADER_LEN, fp);
    fclose(fp);
#endif
...
RingBuf SpeechMessengerECCCI::GetM2ACtmRingBuffer(const ccci_buff_t &ccci_buff)
{
    RingBuf ul_ring_buf;
    uint16_t size_copy1, size_copy2;
    ul_ring_buf.bufLen   = mM2AShareBufLen;
