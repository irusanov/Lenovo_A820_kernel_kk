...
#ifdef MTK_AUDIO
    if (AudioSystem::DEFAULT == stream)
    {
        ALOGW("getOutput() invalid stream %d, returns output NULL", stream);
        return 0;
    }
#endif
                                             ...
                                             int session)
{
    ...
    if (AudioSystem::DEFAULT == stream)
    {
        ALOGW("startOutput() invalid stream %d", stream);
        return BAD_VALUE;
    }
#endif
                                            ...
                                            int session)
{
    ...
    if (AudioSystem::DEFAULT == stream)
    {
        ALOGW("stopOutput() invalid stream %d", stream);
        return BAD_VALUE;
    }
#endif
    ...
    mSpeakerDrcEnabled(false)
{
...
#ifdef MTK_AUDIO
    {
        ...
        property_set(gaf_policy_invertSetModeRouting_propty,"1");
    }
#endif
...
#ifdef MTK_AUDIO_DDPLUS_SUPPORT
            if (device == AUDIO_DEVICE_OUT_AUX_DIGITAL && output != 0) {
                int supportedChannel = 2;
                for (uint32_t i = 0; i < profile->mChannelMasks.size(); ++i) {
                    ...
                    if (channelMask == AUDIO_CHANNEL_OUT_7POINT1) {
                        if (supportedChannel < 8) {
                            supportedChannel = 8;
                        }
                    } else if (channelMask == AUDIO_CHANNEL_OUT_5POINT1) {
                        if (supportedChannel < 6) {
                            supportedChannel = 6;
                        }
                    }
                }
                ds1ConfigureHdmiSupportedChannel(supportedChannel);
            }
#endif
            if (output == 0) {
...
#ifdef MTK_AUDIO_DDPLUS_SUPPORT
                if (device == AUDIO_DEVICE_OUT_AUX_DIGITAL) {
                    ds1ConfigureHdmiSupportedChannel(2);
                }
#endif
            } else {
                outputs.add(output);
...
#ifdef MTK_AUDIO_DDPLUS_SUPPORT
        if (device == AUDIO_DEVICE_OUT_AUX_DIGITAL) {
            ds1ConfigureHdmiSupportedChannel(0);
        }
#endif
    ...
    if (mA2dpSuspended) {
        if (((mScoDeviceAddress == "") ||
             ...
             ((mPhoneState != AudioSystem::MODE_IN_CALL) &&
               ...
               (mPhoneState != AudioSystem::MODE_IN_CALL_EXTERNAL)&&
#endif             	
...
int AudioMTKPolicyManager::GetStreamMaxLevels(int streamtype)
{
    ...
    switch(streamtype)
    {
        case AudioSystem::VOICE_CALL:
        {
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_SIP];
            break;
        }
        case AudioSystem::RING:
        case AudioSystem::ALARM:
        case AudioSystem::NOTIFICATION:
        {
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_RING];
            break;
        }
        case AudioSystem::MUSIC:
        {
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_MEDIA];
            break;
        }
        case AudioSystem::FM:
        {
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_FM];
            break;
        }
       case AudioSystem::MATV:
        {
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_MATV];
            break;
        }
        default:
            retVal = Audio_Ver1_Custom_Volume.audiovolume_level[VER1_VOL_TYPE_MEDIA];
