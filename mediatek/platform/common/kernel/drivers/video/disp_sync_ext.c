...
static struct sw_sync_timeline* sync_timelines[FENCE_TIMELINE_COUNT];
static sync_info layer_infos[FENCE_TIMELINE_COUNT] =
{
		{.layer_id = FENCE_TIMELINE_COUNT},
		{.layer_id = FENCE_TIMELINE_COUNT},
		{.layer_id = FENCE_TIMELINE_COUNT},
		{.layer_id = FENCE_TIMELINE_COUNT}
};
...
int disp_sync_inc_timeline(UINT layer, UINT cur_idx) {
	sync_info *info;
	struct sw_sync_timeline *timeline;
	buffer_info *buf;
	UINT diff = 0;
 ...
 */
void disp_sync_signal_fence(UINT layer) {
	sync_info *info;
	struct sw_sync_timeline *timeline;
	UINT num_fence = 0;
	if (layer >= FENCE_TIMELINE_COUNT) {
...
void disp_sync_release(UINT layer) {
	...
	disp_sync_inc_timeline(layer, index);
	disp_sync_signal_fence(layer);
	disp_sync_release_buffer(layer);
}

SYNC_STATUS disp_sync_prepare_buffer_deprecated(struct fb_overlay_buffer *buf) {
	sync_info *info;
	struct sw_sync_timeline *timeline;
...
void disp_sync_deinit(void) {
	struct sw_sync_timeline *timeline;
	UINT layer = 0;
    for (layer = 0; layer < FENCE_TIMELINE_COUNT; layer++) {
    	timeline = sync_timelines[layer];
    	if (timeline != NULL) {
