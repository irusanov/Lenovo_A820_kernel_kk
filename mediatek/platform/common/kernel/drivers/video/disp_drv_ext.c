...
disp_session_config disp_config = {
		.mode = DISP_DECOUPLE_MODE,
		.type = DISP_SESSION_PRIMARY,
		.device_id = 0,
		.session_id = MAKE_DISP_SESSION(DISP_DECOUPLE_MODE, DISP_SESSION_PRIMARY, 0)
};
...
static void _DISP_InitHWConfig (disp_job *job, OVL_CONFIG_STRUCT *pOVLConfig,
	...
	UINT layer = 0;
	if (pOVLConfig != NULL) {
		layer = pOVLConfig->layer;
		pOVLConfig->addr = job->input[layer].address;
		pOVLConfig->aen = job->input[layer].alpha_enable;
		pOVLConfig->alpha = job->input[layer].alpha;
		pOVLConfig->buff_idx = job->input[layer].index;
		pOVLConfig->connected_type = 0;	// NO USE now
		pOVLConfig->dst_h = job->input[layer].height;
		pOVLConfig->dst_w = job->input[layer].width;
		pOVLConfig->fmt = job->input[layer].format;
		pOVLConfig->identity = 0;			// NO USE now
		pOVLConfig->isDirty = job->input[layer].dirty;
		pOVLConfig->isTdshp = 0;			// NO USE now
		pOVLConfig->key = job->input[layer].color_key;
		pOVLConfig->keyEn = job->input[layer].color_key_enable;
		pOVLConfig->layer = job->input[layer].layer_id;
		pOVLConfig->layer_en = job->input[layer].layer_enable;
		pOVLConfig->security = job->input[layer].security;
		pOVLConfig->source = OVL_LAYER_SOURCE_MEM;
		pOVLConfig->src_h = job->input[layer].height;
		pOVLConfig->src_pitch = job->input[layer].pitch;
		pOVLConfig->src_w = job->input[layer].width;
		pOVLConfig->src_x = job->input[layer].src_x;
		pOVLConfig->src_y = job->input[layer].src_y;
		pOVLConfig->dst_x = job->input[layer].dst_x;
		pOVLConfig->dst_y = job->input[layer].dst_y;
	...
	if (pWDMAConfig != NULL) {
		...
		//pWDMAConfig->dstHeight = job->output.height;
...
static void _DISP_ConfigMemWriteDatapath (disp_job *job) {
	...
	for (i=0; i<DDP_OVL_LAYER_MUN; i++) {
		if (job->input[i].dirty) {
			ovl_config.layer = job->input[i].layer_id;
	...
	if (job->output.dirty) {
		...
		wdma_config.dstAddress = disp_deque_buffer(&normal_buffer_queue);
...
static void _DISP_WaitMemWriteDone (void) {
	...
	MMProfileLog(MTKFB_MMP_Events.OverlayOutDone, MMProfileFlagStart);

	//2. swap buffer queue
	if (disp_config.type == DISP_SESSION_PRIMARY) {
		disp_enque_buffer(&normal_buffer_queue);
	}
	write_mem_running = 0;
	...
	//4. release fence. This can be done later(Ex. clean_up_kthread)
	while ((job = disp_query_job()) != NULL) {
		for (i = 0; i < MAX_INPUT_CONFIG; i++) {
			if (job->input[i].dirty) {
				if (isAEEEnabled && job->input[i].layer_id == DISP_DEFAULT_UI_LAYER_ID) {
					disp_sync_release(DISP_DEFAULT_UI_LAYER_ID);
				} else {
					disp_sync_inc_timeline(job->input[i].layer_id, job->input[i].index);
				}
			}
		}
		//output layerId = MAX_INPUT_CONFIG
		if (job->output.dirty) {
			disp_sync_inc_timeline(job->output.layer_id, job->output.index);
		}
	}
	//5. recycle job. This can be done later(Ex. clean_up_kthread)
	disp_recycle_job();
	...
	MMProfileLog(MTKFB_MMP_Events.OverlayOutDone, MMProfileFlagEnd);
}
	...
	while(1) {
		...
		if (!is_early_suspended) {
			while ((job = disp_acquire_job()) != NULL) {
				MMProfileLogEx(MTKFB_MMP_Events.TrigOverlayOut, MMProfileFlagPulse, job->group_id, job);
...
static void _DISP_ConfigMemReadDatapath (disp_path_config_dirty *dirty_flag) {
	...
	config.address = disp_request_buffer(&normal_buffer_queue);
...
DISP_STATUS DISP_SwitchDisplayMode (struct fb_overlay_mode *pConfig) {
    ...
    if (!is_early_suspended) {
    	...
    	if (disp_mode == DISP_DIRECT_LINK_MODE) {
    		...
    		MemOutConfig.dstAddr = disp_deque_buffer(&normal_buffer_queue);
    	...
    	config.address = (pConfig->mode == DISP_DECOUPLE_MODE) ? disp_request_buffer(&normal_buffer_queue) : 0;
...
static void _DISP_DumpLayer(OVL_CONFIG_STRUCT* pLayer)
{
    if (gCaptureLayerEnable && pLayer->layer_en)
    {
        ...
        Bitmap.start_pos = pLayer->src_x*Bitmap.bpp/8+pLayer->src_y*pLayer->src_pitch;
		...
		for (i=0; i<DDP_OVL_LAYER_MUN; i++) {
			...
			if (ovl_config.isDirty) {
				...
				if (ovl_config.layer_en) {
					disp_sync_inc_timeline(ovl_config.layer, ovl_config.buff_idx-1);
				} else {
					disp_sync_inc_timeline(ovl_config.layer, ovl_config.buff_idx);
				}
			}
		}
...
static int _DISP_ConfigUpdateKThread(void *data)
{
    ...
    while(1)
    {
		...
		if (!is_early_suspended) {
			...
			if (DISP_IsDecoupleMode()) {
				if (disp_acquire_buffer(&normal_buffer_queue) || dirty) {
					disp_running = 1;
...
#if defined(MTK_OVL_DECOUPLE_SUPPORT)
	if (DISP_IsDecoupleMode()) {
		disp_release_buffer(&normal_buffer_queue);
	}
	read_mem_running = 0;
