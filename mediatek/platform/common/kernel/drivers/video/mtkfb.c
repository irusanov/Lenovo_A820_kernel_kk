...
unsigned int mtkfb_fb_lk_copy_size()
{
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    if(isAEEEnabled)
    {
        printk("[FB Driver] aee on so changed pan_display to layer %d \n",DISP_CHANGED_UI_LAYER_ID);
        layer = DISP_CHANGED_UI_LAYER_ID;
    }
    DISP_CHECK_RET(DISP_SetFrameBufferAddr(paStart));
    cached_layer_config[layer].vaddr = (unsigned int)vaStart;
    cached_layer_config[layer].layer_en = 1;
    cached_layer_config[layer].src_x = 0;
    cached_layer_config[layer].src_y = 0;
    cached_layer_config[layer].src_w = var->xres;
    cached_layer_config[layer].src_h = var->yres;
    cached_layer_config[layer].dst_x = 0;
    cached_layer_config[layer].dst_y = 0;
    cached_layer_config[layer].dst_w = var->xres;
    cached_layer_config[layer].dst_h = var->yres;
    {
        ...
        {
        case 16:
            cached_layer_config[layer].fmt = eRGB565;
            layerpitch = 2;
            cached_layer_config[layer].aen = FALSE;
            break;
        case 24:
            cached_layer_config[layer].fmt = eRGB888;
            layerpitch = 3;
            cached_layer_config[layer].aen = FALSE;
            break;
        case 32:
            cached_layer_config[layer].fmt = ePARGB8888;
            layerpitch = 4;
            cached_layer_config[layer].aen = TRUE;
            break;
        default:
            PRNERR("Invalid color format bpp: 0x%d\n", var->bits_per_pixel);
            return -1;
        }
        cached_layer_config[layer].alpha = 0xFF;
        cached_layer_config[layer].src_pitch = src_pitch * layerpitch;
    }
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer* layerInfo, unsigned int locked)
{
    ...
    OVL_CONFIG_STRUCT temp[DDP_OVL_LAYER_MUN];
    static unsigned int max_buf_index_setted[DDP_OVL_LAYER_MUN] = {0,0,0,0};
    memcpy(temp, cached_layer_config, sizeof(temp));
    ...
    if (!enable)
    {
        temp[id].layer_en = enable;
        temp[id].isDirty = true;
        temp[id].security = false;
        //notes: buff_idx must be max index ever setted!
    ...
    case MTK_FB_FORMAT_ARGB8888:
        temp[id].fmt = ePARGB8888;
        layerpitch = 4;
        layerbpp = 32;
        break;
    case MTK_FB_FORMAT_ABGR8888:
        temp[id].fmt = ePABGR8888;
        layerpitch = 4;
        layerbpp = 32;
        break;
    ...
    temp[id].vaddr = (unsigned int)layerInfo->src_base_addr;
    ...
    //layer is enabled here
    if((temp[id].buff_idx!=-1) && (temp[id].buff_idx>max_buf_index_setted[id]))
        max_buf_index_setted[id] = temp[id].buff_idx;

{
    ...
    {
        cnt_security_layer += temp[tl].security;
    }
    MTKFB_LOG("Totally %d security layer is set now\n", cnt_security_layer);
    MTK_HDMI_Set_Security_Output(!!cnt_security_layer);
#endif
}
    temp[id].identity = layerInfo->identity;
    temp[id].connected_type = layerInfo->connected_type;

    //set Alpha blending
    if (layerInfo->alpha_enable) {
    	temp[id].aen = TRUE;
    	temp[id].alpha = layerInfo->alpha;
    } else {
    	temp[id].aen = FALSE;
    }
    ...
    temp[id].src_x = layerInfo->src_offset_x;
    temp[id].src_y = layerInfo->src_offset_y;
    temp[id].src_w = layerInfo->src_width;
    temp[id].src_h = layerInfo->src_height;
    temp[id].dst_x = layerInfo->tgt_offset_x;
    temp[id].dst_y = layerInfo->tgt_offset_y;
    temp[id].dst_w = layerInfo->tgt_width;
    temp[id].dst_h = layerInfo->tgt_height;
    if (temp[id].dst_w > temp[id].src_w)
        temp[id].dst_w = temp[id].src_w;
    if (temp[id].dst_h > temp[id].src_h)
        temp[id].dst_h = temp[id].src_h;

    temp[id].src_pitch = layerInfo->src_pitch*layerpitch;
    ...
    //set color key
    temp[id].key = layerInfo->src_color_key;
    temp[id].keyEn = layerInfo->src_use_color_key;
...
LeaveOverlayMode:
    ...
    if(!(isAEEEnabled && (id==DISP_DEFAULT_UI_LAYER_ID)))
    {
        memcpy(cached_layer_config, temp, sizeof(temp));
    }
    atomic_set(&OverlaySettingDirtyFlag, 1);
    atomic_set(&OverlaySettingApplied, 0);
    if (!locked) 
...
static int mtkfb_auto_capture_framebuffer(struct fb_info *info,struct fb_slt_catpure *config)
{
    ...
    //DISP_Capture_Framebuffer(pvbuf, bpp, is_power_suspended);
    DISP_Auto_Capture_FB(pvbuf, config->format,bpp, is_power_suspended,config->wdma_width,config->wdma_height);
    if (is_power_suspended)
    {
...
static int mtkfb_capture_framebuffer(struct fb_info *info, unsigned int pvbuf)
{
    ...
    */
    if (is_power_suspended)
    {
        memset(pvbuf, 0, DISP_GetScreenWidth()*DISP_GetScreenHeight()*info->var.bits_per_pixel/8);
        goto EXIT;
    }
...
{
	unsigned int i;
	printk("[mtkfb] start dump layer info, power_suspend=%d \n", is_power_suspended);
...
#endif
{
    ...
    case MTKFB_SET_MULTIPLE_LAYERS:
    {
        ...
        else 
        {
            ...
            for (i = 0; i < HW_OVERLAY_COUNT; ++i) 
            {
                ...
                if (layerInfo[i].layer_enable && (layerInfo[i].next_buff_idx <= cached_layer_config[i].buff_idx))
                {
                    MTKFB_ERR("layerId(%d) HWC set the old buffer(%d),last idx(%d)!\n", layerId, layerInfo[i].next_buff_idx, cached_layer_config[i].buff_idx);
    ...
    case MTKFB_PREPARE_OVERLAY_BUFFER:
    {
        ...
        else 
        {
            ...
            {
                if (disp_sync_prepare_buffer(&overlay_buffer) != SYNC_STATUS_OK) 
                {
...
////////////////////////////////////////////////
    default:
    ...
    #endif
    	{
	        ...
	        printk("mtkfb_ioctl Not support, info=0x%08x, cmd=0x%08x, arg=0x%08x\n", (unsigned int)info, (unsigned int)cmd, (unsigned int)arg);
	        return -EINVAL;
        }
    }
}
