...
#my $ChangeHistory = "3.3 Support ABTC\n";
my $ChangeHistory = " 3.4 Support Shared SD Card and CIP";
...
my $COLUMN_FB_DL;                   
# emmc support
my $COLUMN_PARTITION_IDX;          
my $COLUMN_REGION;		    
my $COLUMN_RESERVED;		    
if($MTK_CIP_SUPPORT eq "yes"){
	$COLUMN_PARTITION                = 1 ;
	$COLUMN_TYPE                     = $COLUMN_PARTITION + 1 ;
	$COLUMN_SIZE                     = $COLUMN_TYPE + 1 ;
	$COLUMN_SIZEKB                   = $COLUMN_SIZE + 1 ;
	$COLUMN_SIZE2                    = $COLUMN_SIZEKB + 1 ;
	$COLUMN_SIZE3                    = $COLUMN_SIZE2 + 1 ;
	$COLUMN_DL                       = $COLUMN_SIZE3 + 1 ;
	$COLUMN_FB_ERASE                 = $COLUMN_DL + 1;  # fastboot support
	$COLUMN_FB_DL                    = $COLUMN_FB_ERASE + 1;  # fastboot support
	# emmc support
	$COLUMN_REGION		    = $COLUMN_FB_DL + 1;
	$COLUMN_RESERVED		    = $COLUMN_REGION + 1;
}else{
	$COLUMN_PARTITION                = 1 ;
	$COLUMN_TYPE                     = $COLUMN_PARTITION + 1 ;
	$COLUMN_START                    = $COLUMN_TYPE + 1 ;
	$COLUMN_END                      = $COLUMN_START + 1 ;
	$COLUMN_SIZE                     = $COLUMN_END + 1 ;
	$COLUMN_SIZEKB                   = $COLUMN_SIZE + 1 ;
	$COLUMN_SIZE2                    = $COLUMN_SIZEKB + 1 ;
	$COLUMN_SIZE3                    = $COLUMN_SIZE2 + 1 ;
	$COLUMN_DL                       = $COLUMN_SIZE3 + 1 ;
	...
	$COLUMN_FB_DL                    = $COLUMN_FB_ERASE + 1;  # fastboot support
	# emmc support
	$COLUMN_PARTITION_IDX            = $COLUMN_FB_DL + 1 ;
	$COLUMN_REGION		    = $COLUMN_PARTITION_IDX + 1;
	$COLUMN_RESERVED		    = $COLUMN_REGION + 1;
}
...
sub ReadExcelFile()
{
	...
	while($pt_name ne "END"){
		...
		{
				if($MTK_SHARED_SDCARD eq "yes"){
					if($TYPE_FIELD[$row_t-1] eq "EXT4" || $pt_name eq "EBR1"){
						$p_index_count ++;
						$PARTITION_IDX_FIELD[$row_t-1] = $p_index_count;
					}else{
						$PARTITION_IDX_FIELD[$row_t-1] = 0;
					}		
				}else{
			    		$PARTITION_IDX_FIELD[$row_t-1] = &xls_cell_value($sheet, $row, $COLUMN_PARTITION_IDX,$SHEET_NAME) ;
				}
				$REGION_FIELD[$row_t-1]        = &xls_cell_value($sheet, $row, $COLUMN_REGION,$SHEET_NAME) ;
				$RESERVED_FIELD[$row_t-1]		= &xls_cell_value($sheet, $row, $COLUMN_RESERVED,$SHEET_NAME) ;
			}
...
#init start_address of partition
	...
	for($row=1;$row < @PARTITION_FIELD;$row++){
		...
		if($PARTITION_FIELD[$row] eq "BMTPOOL" || $PARTITION_FIELD[$row] eq "OTP"){
			if($MTK_SHARED_SDCARD eq "yes"){
				$START_FIELD_Byte[$row] = &xls_cell_value($sheet, $row+2, $COLUMN_START,$SHEET_NAME);
			}else{
			$START_FIELD_Byte[$row] = &xls_cell_value($sheet, $row+1, $COLUMN_START,$SHEET_NAME);
			}
			next; 
...
#endif
__TEMPLATE
		print $partition_define_c_fd $G_MBR_Struct;
	}
...
#****************************************************************************************
# subroutine:  GenMBRFile_new_new  (for cip) 
# return:
#****************************************************************************************

sub GenMBRFile_new_new {
	#my $eMMC_size_block = $User_Region_Size_KB*1024/512;
	my $iter = 0;
	my $iter_p = 0;
# MBR & EBR table init
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] =~ /EBR(\d)/){
			...
			printf ("name:%s index:%d start_by_block:%x\n",$BR[$1][0], $1 ,$ebr_start_block[$1]);
			$ebr_count ++;
			next;
		}
...
#****************************************************************************************
# subroutine:  GenMBRFile_new  (for share SDCard) 
# return:
#****************************************************************************************
...
my @BR = (
	...
	["$PRODUCT_OUT/EBR1", [[0x83,0x0,0x0],
						[0x05,0x0,0x0]]],
	["$PRODUCT_OUT/EBR2", [[0x83,0x0,0x0],[0x05,0x0,0x0]]]
);

	if($p_index_count>6){
		error_handler("$p_index_count > 6, need to add EBR3 fail!\n", __FILE__, __LINE__)	
	}
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		if($PARTITION_FIELD[$iter] eq "MBR"){
			$mbr_start = $START_FIELD_Byte[$iter];
			$br_start[$br_count] =  $START_FIELD_Byte[$iter]/512;
			$br_count++;
			next;
		}
		if($PARTITION_FIELD[$iter] eq "EBR1"){
			$start_block[1] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$size_block[1] = 0xffffffff;
			$br_start[$br_count] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$br_count++;
		}
		if($PARTITION_FIELD[$iter] eq "EBR2"){

			$br_start[$br_count] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$br_count++;
		}
		if($PARTITION_FIELD[$iter] eq "CACHE"  || $PARTITION_FIELD[$iter] eq "SEC_RO" || $PARTITION_FIELD[$iter] eq "ANDROID"){
				$start_block[$PARTITION_IDX_FIELD[$iter]] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			#sync with img size -1MB
				$size_block[$PARTITION_IDX_FIELD[$iter]] =  ($SIZE_FIELD_KB[$iter]-1024)*1024/512; 
		}
		if($PARTITION_FIELD[$iter] eq "USRDATA"){
			        $start_block[$PARTITION_IDX_FIELD[$iter]] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
			$size_block[$PARTITION_IDX_FIELD[$iter]] =  0xffffffff;
		}
	}
	#MBR
	$br_count = 0;
	for($iter=1;$iter<=$p_index_count;$iter++){
		if($iter<=4){
			$BR[$br_count][1][$iter-1][1] = $start_block[$iter];
			$BR[$br_count][1][$iter-1][2] = $size_block[$iter];
			if($iter == 4){
				$br_count++;
			}
		}else{
			$BR[$br_count][1][0][1] = $start_block[$iter] - $br_start[$br_count];
			$BR[$br_count][1][0][2] = $size_block[$iter];
			$BR[$br_count][1][1][1] = $br_start[$br_count+1] - $br_start[$br_count];
			$BR[$br_count][1][1][2] = 0xffffffff;
			$br_count++;
