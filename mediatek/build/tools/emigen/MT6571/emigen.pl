        ...
        elsif($row == 1)
        {
        	 my $value;
        	   for(0..$#$mareas)
        	   {
        	   	  if($col >= 9 && $col <= 37)
        	   	  {
        	   	  	   	$value = $sheet->get_cell(1,9)->value();
        	            #print "col : $col, $mareas->[$_]->[1], $mareas->[$_]->[3], $value\n";
        	            return $value;
        	   	  }
        	   	  elsif($col >= 38 && $col <= 66)
        	   	  {
        	   	  	$value = $sheet->get_cell(1,38)->value();
        	   	  	#print "col : $col, $mareas->[$_]->[1], $mareas->[$_]->[3], $value\n";
        	   	  	return $value;
        	   	  }
        	   	  elsif($col >= 67 && $col <= 95)
        	   	  {
        	   	  	$value = $sheet->get_cell(1,67)->value();
        	   	  	#print "col : $col, $mareas->[$_]->[1], $mareas->[$_]->[3], $value\n";
        	   	  	return $value;
        	   	  }
        	   	  elsif($col == 0)
...
sub write_tag()
{
    ...
    open FILE,">$INFO_TAG";
    print FILE pack("a24", "MTK_BLOADER_INFO_v14");
...
sub Parse_MDL
{
	...
	my ($result, $result0, $result1);
		 ...
		 if($os eq "MSWin32")
	  {
        for (1..$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT})
        {
            $result = &get_info($$MEMORY_DEVICE_LIST_XLS_LOCAL, 'mt6571', $IDX_COUNT, 2, $MCP_LIST[$_], &ret_tmp_hash_ref($_), \%MEM_DEV_LIST_INDEX);
            if (($result ne 'TRUE'))
            {
                $result = &get_info($$MEMORY_DEVICE_LIST_INT_XLS_LOCAL, 'mt6571', $IDX_COUNT, 2, $MCP_LIST[$_], &ret_tmp_hash_ref($_), \%MEM_DEV_LIST_INDEX);
            }
            if ($result ne 'TRUE')
            {
                my $p_number;
	  ...
	  else
	  {
	  	    for (1..$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT})
         {
             $result = &get_info($$MEMORY_DEVICE_LIST_XLS_LOCAL, 'mt6571', $IDX_COUNT, 1, $MCP_LIST[$_], &ret_tmp_hash_ref($_), \%MEM_DEV_LIST_INDEX);
             if (($result ne 'TRUE'))
             {
             	  # print "result : $result\n";
                 $result = &get_info($$MEMORY_DEVICE_LIST_INT_XLS_LOCAL, 'mt6571', $IDX_COUNT, 1, $MCP_LIST[$_], &ret_tmp_hash_ref($_), \%MEM_DEV_LIST_INDEX);
             }
             if ($result ne 'TRUE')
             {
                 my $p_number;
