...
{CCCI_ICUSB_TX, 	CCCI_ICUSB_RX,		1,		1,		0xFF,	0xFF,	4,	&char_port_ops, 	MD_CD_MAJOR,	6,	"eemcs_ICUSB",	},
...
{CCCI_ICUSB_TX, 	CCCI_ICUSB_RX,		1,		1,		0xFF,	0xFF,	4,	&char_port_ops, 	MD_CD_MAJOR,	6,	"ttyC3",	},
...
static int cldma_rx_collect(struct md_cd_queue *queue, int budget)
{
	...
	while((rgpd->gpd_flags&0x1) == 0) { // not hardware own
		...
		if(ret>=0 || ret==-CCCI_ERR_DROP_PACKET) {
			...
			// allocate a new skb
			new_skb = ccci_alloc_skb(rx_queue_buffer_size[queue->index]);
			// change skb pointer
			req->skb = new_skb;
	...
	queue->tr_done = req;
	/*
	 * do not use if(count == RING_BUFFER_SIZE) here.
...
static void md_cd_clear_tgpd(struct ccci_request *req)
{
	struct cldma_tgpd *tgpd;
...
retry:
	...
	if(queue->free_slot > 0) {
		...
		req->policy = NOOP;
		ccci_free_req(req); // here we don't wait on 
		// update GPD
...
static int md_cd_send_runtime_data(struct ccci_modem *md)
{
	...
	struct ccci_request *req = ccci_alloc_req(OUT, packet_size, 1, 1);
	struct ccci_header *ccci_h = (struct ccci_header *)req->skb->data;
	struct modem_runtime *runtime = (struct modem_runtime *)(req->skb->data + sizeof(struct ccci_header));
...
{	
	...
	case 0:
		...
		count += snprintf(buf, 128, "[CLDMA IN reg dump(%d):]\n", length);
		for(i=0; i<length; i++) {
			count += snprintf(buf+count, 128, "%X: %X\n", 
				(unsigned int)(md_ctrl->cldma_ap_base+CLDMA_AP_UL_START_ADDR_0+i*sizeof(int)), 
				*((int *)(md_ctrl->cldma_ap_base+CLDMA_AP_UL_START_ADDR_0)+i));
		}
		type++;
		break;
	case 1:
		length = (CLDMA_AP_DEBUG_ID_EN-CLDMA_AP_SO_ERROR)/sizeof(int);
		count += snprintf(buf, 128, "[CLDMA OUT reg dump(%d):]\n", length);
		for(i=0; i<length; i++) {
			count += snprintf(buf+count, 128, "%X: %X\n", 
				(unsigned int)(md_ctrl->cldma_ap_base+CLDMA_AP_SO_ERROR+i*sizeof(int)), 
				*((int *)(md_ctrl->cldma_ap_base+CLDMA_AP_SO_ERROR)+i));
		}
		type++;
		break;
	case 2:
		length = (CLDMA_AP_CHNL_IDLE-CLDMA_AP_L2TISAR0)/sizeof(int);
		count += snprintf(buf, 128, "[CLDMA MISC reg dump(%d):]\n", length);
		for(i=0; i<length; i++) {
			count += snprintf(buf+count, 128, "%X: %X\n", 
				(unsigned int)(md_ctrl->cldma_ap_base+CLDMA_AP_L2TISAR0+i*sizeof(int)), 
				*((int *)(md_ctrl->cldma_ap_base+CLDMA_AP_L2TISAR0)+i));
		}
		type = 0;
		break;
	};
