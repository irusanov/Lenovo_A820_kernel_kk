...
static int tz_client_mmap(struct file *filp, struct vm_area_struct *vma);
...
static struct file_operations tz_client_fops = {
    .open = tz_client_open,
...
static struct tz_sharedmem_info *tz_client_get_sharedmem(struct file *file, KREE_SHAREDMEM_HANDLE handle)
{
    struct tz_client_info *info;
    struct tz_sharedmem_info *shm_info;
...
static int tz_client_unregister_sharedmem(struct file *file, struct tz_sharedmem_info *shm_info)
{
    struct tz_client_info *info;
    int ret = 0;
...
static long tz_client_unreg_sharedmem (struct file *file, unsigned long arg)
{
    ...
    /* Unmap user pages
       */
    shm_info = tz_client_get_sharedmem (file, cparam.mem_handle);
    if (shm_info == NULL)
    {
        printk ("tz_client_unreg_sharedmem: get shared memory handle\n");
        return -EINVAL;
    }
    ...
    /* unregister to fd
       */
    ret = tz_client_unregister_sharedmem (file, shm_info);
    if (ret != TZ_RESULT_SUCCESS)
    {
