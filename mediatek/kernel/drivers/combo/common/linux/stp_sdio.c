...
static INT32 _stp_sdio_wait_for_msg(void *pvData);
static void stp_sdio_tx_rx_handling(void *pData);
#endif
...
#define STPSDIO_INFO_FUNC(fmt, arg...)   if (gStpSdioDbgLvl >= STPSDIO_LOG_INFO) { printk(DFT_TAG "[I]%s:"  fmt, __FUNCTION__ ,##arg);}
#define STPSDIO_WARN_FUNC(fmt, arg...)   if (gStpSdioDbgLvl >= STPSDIO_LOG_WARN) { printk(DFT_TAG "[W]%s:"  fmt, __FUNCTION__ ,##arg);}
...
INT32 stp_sdio_host_info_op(INT32 opId)
{
    INT32 iRet = 0;
#if KMALLOC_UPDATE
    static UINT8 *p_tx_buffer = NULL;
    static UINT8 *p_rx_buffer = NULL;
    if (NULL == p_tx_buffer)
    {
        p_tx_buffer = kmalloc(STP_SDIO_TX_BUF_CNT * STP_SDIO_TX_ENTRY_SIZE, GFP_ATOMIC);
        if (NULL == p_tx_buffer)
        {
            STPSDIO_ERR_FUNC("memory allocate for g_stp_sdio_host_info.pkt_buf.tx_buf fail!\n");
            iRet = -1;
        }else
        {
            STPSDIO_INFO_FUNC("memory allocate for g_stp_sdio_host_info.pkt_buf.tx_buf succeed!\n");
            iRet = 0;
        }
    }
    else
    {
        STPSDIO_INFO_FUNC("memory already allocated for g_stp_sdio_host_info.pkt_buf.tx_buf!\n");
        iRet = 0;
    }
    if (NULL == p_rx_buffer)
    {
        ...
        if (NULL == p_rx_buffer)
        {
            STPSDIO_ERR_FUNC("memory allocate for g_stp_sdio_host_info.pkt_buf.rx_buf fail!\n");
            iRet = -1;
        }else
        {
            STPSDIO_INFO_FUNC("memory allocate for g_stp_sdio_host_info.pkt_buf.rx_buf succeed!\n");
            iRet = 0;
        }
    }
    else
    {
        STPSDIO_INFO_FUNC("memory already allocated for g_stp_sdio_host_info.pkt_buf.rx_buf!\n");
        iRet = 0;
    }
...
INT32 stp_sdio_host_info_init(UINT8 **ppTxBuf, UINT8 **ppRxBuf)
{
    /* Init host count */
...
INT32 stp_sdio_host_info_deinit(UINT8 **ppTxBuf, UINT8 **ppRxBuf)
{
...
static INT32 stp_sdio_do_own_set (void)
{
    ...
    int ret = -1;
    unsigned int chlcpr_value = 0x0;
    unsigned int retry = SET_OWN_RETRY;
    unsigned int delay_us = 500;
    unsigned int tx_fifo_clear_cnt=0;
    MTK_WCN_HIF_SDIO_CLTCTX clt_ctx;
    ...
    while (retry) {
        ...
        if ((comp_count == p_info->firmware_info.tx_packet_num) && (0 == p_info->rx_pkt_len))
        {
        ...
        {
            spin_unlock_irqrestore(&gp_info->pkt_buf.rd_idx_lock, gp_info->pkt_buf.rd_irq_flag);
            STPSDIO_WARN_FUNC("set own (sleep) is temporarily not permitted! complete count(%d), tx_packet_num(%d), rx_pkt_len(%d) wait\n",
                comp_count, p_info->firmware_info.tx_packet_num, p_info->rx_pkt_len);
            retry--;
            tx_fifo_clear_cnt=0;
            msleep(2);
...
{
    INT32 ret = -1;
    P_OSAL_SIGNAL pOsalSignal = &gp_info->isr_check_complete;
...
#if STP_SDIO_NEW_TXRING
    STPSDIO_LOUD_FUNC("len(%u), wr_cnt(%u), rd_cnt(%u), irq_pending(%u), sleep(%u), wakeup(%u)\n",
    pInfo->rx_pkt_len,
    pInfo->pkt_buf.wr_cnt,
    pInfo->pkt_buf.rd_cnt,
    pInfo->irq_pending,
    pInfo->sleep_flag,
    pInfo->wakeup_flag
    );
...
#else
    STPSDIO_LOUD_FUNC("len(%u), rd_idx(%u), wr_idx(%u), irq_pending(%u), sleep(%u), wakeup(%u)\n",
    pInfo->rx_pkt_len,
    pInfo->pkt_buf.rd_idx,
    pInfo->pkt_buf.wr_idx,
    pInfo->irq_pending,
    pInfo->sleep_flag,
    pInfo->wakeup_flag
    );
        ...
        if ((0 != pInfo->irq_pending) && (1 == pInfo->awake_flag))
        {
            ...
            if ( chisr & RX_DONE ) {
                ...
                if ((0 == pInfo->rx_pkt_len)
                    ...
                    STPSDIO_ERR_FUNC("abnormal rx_pkt_len(%d) in CHISR(0x%08x) skip rx_worker\n",
                ...
                */
                    ...
                    {
                        STPSDIO_WARN_FUNC("rx data len is not 4 bytes allignment, CHISR(0x%08x), rx len (%d).\n", chisr, pInfo->rx_pkt_len);
                    }
                }
                /*Rx job*/
   ...
   using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    STPSDIO_ERR_FUNC("**************Fatal Error, enable COM IRQ fail*********\n");
                }
                #endif
            }
...
#else
                        iRet = mtk_wcn_hif_sdio_writel(clt_ctx, CHLPCR, C_FW_OWN_REQ_CLR);
#endif /* COHEC_00006052 */
                    }
                }
            }
    ...
    )
{
    ...
    /* Case 3: buffer is full (w-r >= s), (try aggregation) */
        ...
        if (!((pb->wr_cnt - pb->rd_cnt) >= STP_SDIO_TX_BUF_CNT)) {
            STPSDIO_ERR_FUNC("abnormal condition and flow, wr_cnt(0x%x), rd_cnt(0x%x)\n", pb->wr_cnt, pb->rd_cnt);
        }
        prev_wr_idx = (pb->wr_cnt - 1) & STP_SDIO_TX_BUF_CNT_MASK;
        prev_size = pb->tx_buf_sz[prev_wr_idx];
        ...
        if ((size + prev_size) <= STP_SDIO_TX_FIFO_SIZE) {
            if (0 != prev_size) {
                STPSDIO_ERR_FUNC("abnormal condition and flow, wr_cnt(0x%x), rd_cnt(0x%x), prev(%d), prev_size(%d)\n",
        ...
        /* Case 3.2 Buffer is full */
            ...
            wait_event_interruptible ( pb->fullwait_q, (MTK_WCN_BOOL_FALSE == pb->full_flag) );
    ...
    )
{
    ...
    *written_size = 0;
...
#if KMALLOC_UPDATE
        *(gp_info->pkt_buf.tx_buf + gp_info->pkt_buf.wr_idx * STP_SDIO_TX_ENTRY_SIZE + 0) = (UINT8)((size+STP_SDIO_HDR_SIZE)&0xff);
        *(gp_info->pkt_buf.tx_buf + gp_info->pkt_buf.wr_idx * STP_SDIO_TX_ENTRY_SIZE + 1) = (UINT8)((size+STP_SDIO_HDR_SIZE)>>8);
#else
        gp_info->pkt_buf.tx_buf[gp_info->pkt_buf.wr_idx][0] = (UINT8)((size+STP_SDIO_HDR_SIZE)&0xff);
        gp_info->pkt_buf.tx_buf[gp_info->pkt_buf.wr_idx][1] = (UINT8)((size+STP_SDIO_HDR_SIZE)>>8);
#endif
        gp_info->pkt_buf.tx_buf_ts[gp_info->pkt_buf.wr_idx] = jiffies;

        STPSDIO_DBG_FUNC("(Empty) Enqueue done\n");
#if KMALLOC_UPDATE
        pkt_bufp = gp_info->pkt_buf.tx_buf + gp_info->pkt_buf.wr_idx * STP_SDIO_TX_ENTRY_SIZE + STP_SDIO_HDR_SIZE;
#else
    ...
    {
        prev_wr_idx = (gp_info->pkt_buf.wr_idx-1+STP_SDIO_TX_BUF_CNT)%STP_SDIO_TX_BUF_CNT;
        /* set the packet size form previous SDIO packet header */
#if KMALLOC_UPDATE
        buf_allocation = *(gp_info->pkt_buf.tx_buf + prev_wr_idx * STP_SDIO_TX_ENTRY_SIZE + 1);
        buf_allocation = (buf_allocation<<8)| *(gp_info->pkt_buf.tx_buf + prev_wr_idx * STP_SDIO_TX_ENTRY_SIZE + 0);
#else
...
#if KMALLOC_UPDATE
            pkt_bufp = gp_info->pkt_buf.tx_buf + prev_wr_idx * STP_SDIO_TX_ENTRY_SIZE + buf_allocation;

            buf_allocation += size;
            *(gp_info->pkt_buf.tx_buf + prev_wr_idx * STP_SDIO_TX_ENTRY_SIZE + 0) = (UINT8)(buf_allocation&0xff);
            *(gp_info->pkt_buf.tx_buf + prev_wr_idx * STP_SDIO_TX_ENTRY_SIZE + 1) = (UINT8)(buf_allocation>>8);
#else
        ...
        /* Case 2.2 Not aggregation */
            ...
            room = (gp_info->pkt_buf.wr_idx >= gp_info->pkt_buf.rd_idx) ?
                (STP_SDIO_TX_BUF_CNT - (gp_info->pkt_buf.wr_idx - gp_info->pkt_buf.rd_idx)) :
                (gp_info->pkt_buf.rd_idx - gp_info->pkt_buf.wr_idx);
            if ( room == 1 )
            {
    ...
    else if (MTK_WCN_BOOL_FALSE != gp_info->pkt_buf.full_flag)
    {
        ...
        else
        {
            ...
            // TODO:[FixMe][George] This wait() call IS a problem if caller runs in interrrupt context (sw or hw) !!
            // TODO:[FixMe][George] should use timeout version, not interruptible version. Return error when timeout!
            wait_event_interruptible ( gp_info->pkt_buf.fullwait_q, (!gp_info->pkt_buf.full_flag) );
            STPSDIO_INFO_FUNC("wait event return\n");
    ...
    )
{
    ...
    if ( (p_info) && (0 == p_info->firmware_info.tx_packet_num) ) {
        if (!(p_info->tx_pkt_list.pkt_rd_cnt == p_info->tx_pkt_list.pkt_wr_cnt)) {
            STPSDIO_ERR_FUNC("abnormal fifo_size(%d) pkt_num(%d) pkt_rd(0x%x, %ld) pkt_wr(0x%x, %ld)!(%d)\n",
    ...
    )
{
    ...
    if (p_info->firmware_info.tx_packet_num >= comp_count) {
        STPSDIO_DBG_FUNC("tx_pack_num(%d), comp_count(%d),tx_comp_num(%d)\n",
            p_info->firmware_info.tx_packet_num, comp_count, atomic_read(&p_info->firmware_info.tx_comp_num));
    ...
    while (comp_count > 0) {
        if (p_info->tx_pkt_list.pkt_rd_cnt == p_info->tx_pkt_list.pkt_wr_cnt) {
            STPSDIO_ERR_FUNC("tx complete count(%d) but tx_pkt_list empty, rd_cnt(0x%ud, idx:%lx), wr_cnt(0x%ud, idx:%lx)!\n",
                comp_count,
                p_info->tx_pkt_list.pkt_rd_cnt,
    ...
    )
{
    ...
    do {
        ...
        if (STP_SDIO_TX_PKT_MAX_CNT <= p_info->firmware_info.tx_packet_num)  {
            STPSDIO_DBG_FUNC("tx_packet_num(%ld) limit,  tx_fifo_size(%ld), four_byte_align_len(%ld)\n",
                p_info->firmware_info.tx_packet_num,
                p_info->firmware_info.tx_fifo_size,
                four_byte_align_len);
    #if STP_SDIO_DBG_SUPPORT && STP_SDIO_TXPERFDBG
        ...
        if (p_info->firmware_info.tx_fifo_size >= four_byte_align_len) {
            ...
            if (STP_SDIO_TX_ENTRY_SIZE < bus_txlen) {
                STPSDIO_ERR_FUNC("abnormal bus_txlen(%d) > STP_SDIO_TX_ENTRY_SIZE(%ld)!!\n",
            ...
            STPSDIO_DBG_FUNC("update tx packet size list wr(0x%x, %ld) rd(0x%x, %ld), tx fifo(size:%d), pkt_num(%d)done\n",
                p_info->tx_pkt_list.pkt_wr_cnt,
                STP_SDIO_GET_PKT_AR_IDX(p_info->tx_pkt_list.pkt_wr_cnt),
                p_info->tx_pkt_list.pkt_rd_cnt,
                STP_SDIO_GET_PKT_AR_IDX(p_info->tx_pkt_list.pkt_rd_cnt),
            ...
            ret = mtk_wcn_hif_sdio_write_buf(p_info->sdio_cltctx, CTDR, (PUINT32)buf_tx, bus_txlen);
            ...
            do {
                ...
                stp_sdio_txdbg_buffer[idx].four_byte_align_len = four_byte_align_len;
                // store content
                if (bus_txlen <= STP_SDIO_TX_ENTRY_SIZE) {
                    memcpy(&stp_sdio_txdbg_buffer[idx].tx_pkt_buf[0], buf_tx, bus_txlen);
                }
                else {
                    memcpy(&stp_sdio_txdbg_buffer[idx].tx_pkt_buf[0], buf_tx, STP_SDIO_TX_ENTRY_SIZE);
           ...
           if (MTK_WCN_BOOL_FALSE != pb->full_flag) {
            ...
            STPSDIO_DBG_FUNC("tx_fifo_size(%d), four_byte_align_len(%d), tx_packet_num(%d)\n",
                p_info->firmware_info.tx_fifo_size,
                four_byte_align_len,
                p_info->firmware_info.tx_packet_num);
            break;
    ...
    )
{
    ...
    do {
...
#if KMALLOC_UPDATE
        buf_tx = gp_info->pkt_buf.tx_buf + p_info->pkt_buf.rd_idx * STP_SDIO_TX_ENTRY_SIZE + 0;
#else
        ...
        if ( (p_info->firmware_info.tx_fifo_size >= four_byte_align_len)
            ...
            spin_lock_irqsave(&p_info->pkt_buf.rd_idx_lock, p_info->pkt_buf.rd_irq_flag);
            ...
            spin_unlock_irqrestore(&p_info->pkt_buf.rd_idx_lock, p_info->pkt_buf.rd_irq_flag);
        }
        else {
#if STP_SDIO_DBG_SUPPORT && STP_SDIO_TXPERFDBG
            stp_sdio_txperf_fifo_left += p_info->firmware_info.tx_fifo_size;
...
)
{
    ...
    ret = mtk_wcn_hif_sdio_read_buf(p_info->sdio_cltctx,
        CRDR,
        (PUINT32)&p_info->pkt_buf.rx_buf[0],
        bus_rxlen);
    if (ret) {
        // TODO: error handling!
    ...
    do {
        ...
        /* store content */
        memcpy(&stp_sdio_rxdbg_buffer[idx].rx_pkt_buf[0], &p_info->pkt_buf.rx_buf[0], bus_rxlen);
    } while(0);
#endif
    ...
    if (STP_SDIO_HDR_SIZE < bus_rxlen) {
        ...
        mtk_wcn_stp_parser_data(bufp, bus_rxlen);
    }
    else {
...
using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
    ...
    else
    {
        ...
        else
        {
            ...
            {
                STPSDIO_ERR_FUNC("**********disable COM IRQ fail, don't signal stp-sdio thread******\n");
            }
        }
    ...
    )
{
    ...
    */
    ...
    if (ret) {
        ...
        if (ret)
        {
            STPSDIO_ERR_FUNC("get CHISR information rx still error!\n");
            return 0;
        }
    }
    STPSDIO_HINT_FUNC("CHISR(0x%08x)\n", chisr);
    ...
    )
{
    ...
    memset(&g_stp_sdio_host_info.tx_pkt_list.pkt_size_list[0], 0, sizeof(g_stp_sdio_host_info.tx_pkt_list.pkt_size_list));
    ...
    g_stp_sdio_host_info.tx_rx_thread.pThreadData =  (VOID *) &g_stp_sdio_host_info;
    osal_strncpy ((char *)&g_stp_sdio_host_info.tx_rx_thread.threadName,
        "stp_sdio_tx_rx",
        osal_sizeof(g_stp_sdio_host_info.tx_rx_thread.threadName));
...
using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
    ...
    ret = mtk_wcn_hif_sdio_writel(clt_ctx, CHIER, (FIRMWARE_INT|TX_FIFO_OVERFLOW| \
        FW_INT_IND_INDICATOR|TX_COMPLETE_COUNT|TX_UNDER_THOLD|TX_EMPTY|RX_DONE));
    if (ret) {
...
using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
    ...
    /*set STP sdio mode*/
    printk(KERN_INFO DFT_TAG"%s: set stp sdio mode \n", __FUNCTION__);
    mtk_wcn_stp_set_sdio_mode(1);

    /*indicate the stp the sdio ready*/
    printk(KERN_INFO DFT_TAG"%s: stp enable \n", __FUNCTION__);
...
static VOID stp_sdio_txperf_dump (VOID)
{
    ...
    printk(KERN_INFO DFT_TAG"txwait_fifo_left(%d), txwait_to_send(%d), txwait_count(%d)\n",
        fifo, data, cnt);
    if (cnt) {
        printk(KERN_INFO DFT_TAG"avg left(%d), to_send(%d)\n",
            (fifo / cnt), (data / cnt));
    }
    printk(KERN_INFO DFT_TAG"tx_worker_cnt(%d), pkt_num(%d), pkt_num_lmt_cnt(%d)\n",
        wkr, pkt_num, lmt_cnt);
...
static void stp_sdio_txdbg_dump(void)
{
    ...
    for (i = 0; i < STP_SDIO_TXDBG_COUNT; ++i) {
        ...
        if (0 == len) {
            printk(KERN_INFO DFT_TAG"idx(%x) 0 == len dump skip\n", idx);
        }
        printk(KERN_INFO DFT_TAG"stp_sdio_txdbg_buffer idx(%x) bus_txlen(0x%x, %d) ts(%d)\n",
            idx, len, len, stp_sdio_txdbg_buffer[idx].ts);
        ...
        printk(KERN_INFO DFT_TAG"stp_sdio_txdbg_buffer dump ok\n");
    }
    ...
    printk(KERN_INFO DFT_TAG"Dump tx info: pkt_num(%d) fifo(%d) pkt_list.rd(0x%x, %ld) pkt_list.wr(0x%x, %ld)\n",
        gp_info->firmware_info.tx_packet_num,
        gp_info->firmware_info.tx_fifo_size,
    ...
    for (i = 0; i < STP_SDIO_TX_PKT_LIST_SIZE; ++i) {
        ...
        printk(KERN_INFO DFT_TAG"tx_pkt_list idx(0x%x, %d) size(0x%x, %d), in_ts(%d), out_ts(%d)\n",
            (gp_info->tx_pkt_list.pkt_wr_cnt - 1 - i), idx,
            gp_info->tx_pkt_list.pkt_size_list[idx], gp_info->tx_pkt_list.pkt_size_list[idx],
            gp_info->tx_pkt_list.in_ts[idx], gp_info->tx_pkt_list.out_ts[idx]
            );
    }

#if STP_SDIO_NEW_TXRING
    printk(KERN_INFO DFT_TAG"\n\ndump pkt_buf.tx_buf: rd(%d) wr(%d) full(%d)\n",
    ...
    for (i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
        ...
        printk(KERN_INFO DFT_TAG"pkt_buf.tx_buf dump ok\n");
    }
...
static int stp_sdio_init (void)
{
    ...
    g_stp_sdio_cltinfo.func_tbl_size = sizeof(mtk_stp_sdio_id_tbl)/sizeof(MTK_WCN_HIF_SDIO_FUNCINFO)-1;
    ...
    if (ret) {
        STPSDIO_ERR_FUNC("mtk_wcn_stp_wmt_sdio_op_reg(mtk_wcn_stp_sdio_own_ctrl) fail(%d)!\n", ret);
    }
    ...
    STPSDIO_INFO_FUNC("blk_size(%ld), tx_buf_cnt(%ld), fifo tx(%ld) rx(%ld), buf tx(%ld) rx(%ld)\n",
        STP_SDIO_BLK_SIZE, STP_SDIO_TX_BUF_CNT,
        STP_SDIO_TX_FIFO_SIZE, STP_SDIO_RX_FIFO_SIZE,
