    ...
    )
{
    ...
    do {
        ...
        prScanReqV2 = (P_MSG_SCN_SCAN_REQ_V2)cnmMemAlloc(prAdapter, RAM_TYPE_MSG, (sizeof(MSG_SCN_SCAN_REQ_V2) + (sizeof(PARAM_SSID_T)*prScanReqInfo->ucSsidNum)));
        if (!prScanReqV2) {
            ASSERT(0); // Can't trigger SCAN FSM
            DBGLOG(P2P, ERROR, ("p2pFuncRequestScan: Memory allocation fail, can not send SCAN MSG to scan module\n"));
            break;
        }
        ...
        prScanReqV2->prSsid = ((UINT_32)prScanReqV2 + sizeof(MSG_SCN_SCAN_REQ_V2));
    ...
    )
{
    ...
    P_STA_RECORD_T prStaRec = (P_STA_RECORD_T)NULL;
    UINT_8 ucRetryLimit = TX_DESC_TX_COUNT_NO_LIMIT;
    ...
    )
{
    ...
    do {
        ...
        /* Indicate disconnect. */
        if (prP2pBssInfo->eCurrentOPMode == OP_MODE_ACCESS_POINT) {
            P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo = P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prP2pBssInfo->u4PrivateData);
    ...
    )
{
    ...
    do {
        ASSERT_BREAK((prAdapter != NULL) && (pucBcnBuffer != NULL) && (prP2pBssInfo != NULL));
