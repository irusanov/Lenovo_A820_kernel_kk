    ...
    )
{
    ...
    do {
        ...
        /* Funcion Dissolve should already enter IDLE state. */
        p2pRoleFsmStateTransition(prAdapter, prP2pRoleFsmInfo, P2P_ROLE_STATE_IDLE);
    ...
    )
{
    ...
    do {
        ...
        case P2P_ROLE_STATE_SCAN:
            if (!fgIsTransitionOut) {
                p2pRoleStateInit_SCAN(prAdapter, prP2pRoleFsmInfo->ucBssIndex, &(prP2pRoleFsmInfo->rScanReqInfo));
            }
            else {
        ...
        case P2P_ROLE_STATE_REQING_CHANNEL:
            if (!fgIsTransitionOut) {
                p2pRoleStateInit_REQING_CHANNEL(prAdapter, prP2pRoleFsmInfo->ucBssIndex, &(prP2pRoleFsmInfo->rChnlReqInfo));
            }
            else {
                p2pRoleStateAbort_REQING_CHANNEL(prAdapter, prP2pRoleBssInfo, prP2pRoleFsmInfo, eNextState);
            }
            break;
...
{
    P_P2P_ROLE_FSM_INFO_T prP2pRoleFsmInfo = (P_P2P_ROLE_FSM_INFO_T)u4Param;
    P_P2P_CHNL_REQ_INFO_T prP2pChnlReqInfo = (P_P2P_CHNL_REQ_INFO_T)NULL;
    ...
    do {
        ...
        default:
            ASSERT(FALSE);
            DBGLOG(P2P, ERROR, ("Current P2P Role State %d is unexpected for FSM timeout event.\n", prP2pRoleFsmInfo->eCurrentState));
            break;
        }
    } while (FALSE);
    ...
    do {
        ...
        case OP_MODE_INFRASTRUCTURE:
            if (authProcessRxDeauthFrame(prSwRfb,
                ...
                p2pRoleFsmStateTransition(prAdapter, P2P_ROLE_INDEX_2_ROLE_FSM_INFO(prAdapter, prP2pBssInfo->u4PrivateData), P2P_ROLE_STATE_IDLE);
            }
            break;
    ...
    )
{
    ...
    do {
        ...
        if (prP2pBssInfo->eCurrentOPMode != OP_MODE_INFRASTRUCTURE) {
            DBGLOG(P2P, ERROR, ("Error case, P2P BSS %d not INFRA mode but beacon timeout\n", prP2pRoleFsmInfo->ucRoleIndex));
            break;
        }

        DBGLOG(P2P, TRACE, ("p2pFsmRunEventBeaconTimeout: BSS %d Beacon Timeout\n", prP2pRoleFsmInfo->ucRoleIndex));
        ...
        if (prP2pBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
            ...
            if (prP2pBssInfo->prStaRecOfAP != NULL) {
                ...
                p2pFuncDisconnect(prAdapter, prP2pBssInfo, prStaRec, FALSE, REASON_CODE_DISASSOC_LEAVING_BSS);
    ...
    )
{
    ...
    do {
        ...
        if (!prP2pRoleFsmInfo) {
            DBGLOG(P2P, ERROR, ("p2pRoleFsmRunEventStartAP: Corresponding P2P Role FSM empty: %d.\n", prP2pStartAPMsg->ucRoleIdx));
            break;
        }
        ...
        if (prP2pStartAPMsg->u2SsidLen != 0) {
            kalMemCopy(prP2pConnReqInfo->rSsidStruct.aucSsid, prP2pStartAPMsg->aucSsid, prP2pStartAPMsg->u2SsidLen);
            prP2pConnReqInfo->rSsidStruct.ucSsidLen = prP2pSpecificBssInfo->u2GroupSsidLen = prP2pStartAPMsg->u2SsidLen;
    ...
    )
{
    ...
    do {
        ...
        if (!prP2pRoleFsmInfo) {
            DBGLOG(P2P, ERROR, ("p2pRoleFsmRunEventStopAP: Corresponding P2P Role FSM empty: %d.\n", prP2pSwitchMode->ucRoleIdx));
            break;
        }
    ...
    )
{
    ...
    do {
        ...
        if (!prP2pRoleFsmInfo) {
            DBGLOG(P2P, ERROR, ("p2pRoleFsmRunEventConnectionRequest: Corresponding P2P Role FSM empty: %d.\n", prP2pConnReqMsg->ucRoleIdx));
            break;
        }
    ...
    )
{
    ...
    do {
        ...
        if (!prP2pRoleFsmInfo) {
            DBGLOG(P2P, ERROR, ("p2pRoleFsmRunEventConnectionAbort: Corresponding P2P Role FSM empty: %d.\n", prDisconnMsg->ucRoleIdx));
            break;
        }
        ...
        case OP_MODE_INFRASTRUCTURE:
            {
                ...
                p2pFuncDisconnect(prAdapter, prP2pBssInfo, prP2pBssInfo->prStaRecOfAP, prDisconnMsg->fgSendDeauth, prDisconnMsg->u2ReasonCode);
        ...
        case OP_MODE_ACCESS_POINT:
            {
                ...
                LINK_FOR_EACH(prLinkEntry, prStaRecOfClientList) {
                    ...
                    if (EQUAL_MAC_ADDR(prCurrStaRec->aucMacAddr, prDisconnMsg->aucTargetID)) {
                        ...
                        p2pFuncDisconnect(prAdapter, prP2pBssInfo, prCurrStaRec, prDisconnMsg->fgSendDeauth, prDisconnMsg->u2ReasonCode);
        ...
        if (prP2pBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE) {
            ...
            if (prJoinCompMsg->ucSeqNum == prJoinInfo->ucSeqNumOfReqMsg) {
                ...
                if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
                    ...
                    nicUpdateRSSI(prAdapter, prP2pBssInfo->ucBssIndex, (INT_8)(RCPI_TO_dBm(prStaRec->ucRCPI)), 0);
#endif
    ...
    do {
        ...
        if (prScanDoneMsg->ucSeqNum != prScanReqInfo->ucSeqNumOfScnMsg) {
            ...
            /* The scan request has been cancelled.
              ...
              */
            DBGLOG(P2P, TRACE, ("P2P Role Scan Don SeqNum Received:%d <-> P2P Role Fsm SCAN Seq Issued:%d\n",
        ...
        case P2P_ROLE_STATE_SCAN:
            {
                ...
                if (prConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_GC) {
                    ...
                    if ((prP2pJoinInfo->prTargetBssDesc = p2pFuncKeepOnConnection(prAdapter,
                                                                    prAdapter->aprBssInfo[prP2pRoleFsmInfo->ucBssIndex],
                                                                    prConnReqInfo,
                                                                    &prP2pRoleFsmInfo->rChnlReqInfo,
                                                                    &prP2pRoleFsmInfo->rScanReqInfo)) == NULL) {
    ...
    )
{
    ...
    do {
        ...
        if (prChnlReqInfo->u4MaxInterval != prMsgChGrant->u4GrantInterval) {
            DBGLOG(P2P, WARN, ("P2P Role:%d Request Channel Interval:%d, Grant Interval:%d\n",
                                    prP2pRoleFsmInfo->ucRoleIndex,
        ...
        if (ucTokenID == prChnlReqInfo->ucSeqNumOfChReq) {
            ...
            case P2P_ROLE_STATE_REQING_CHANNEL:
                ...
                case CH_REQ_TYPE_GO_START_BSS:
                    ...
                    break;
                default:
                    DBGLOG(P2P, WARN, ("p2pRoleFsmRunEventChnlGrant: Invalid Channel Request Type:%d\n",
...
p2pProcessEvent_UpdateNOAParam (
    IN P_ADAPTER_T    prAdapter,
    IN UINT_8  ucBssIdx,
    IN P_EVENT_UPDATE_NOA_PARAMS_T prEventUpdateNoaParam
    )
{
