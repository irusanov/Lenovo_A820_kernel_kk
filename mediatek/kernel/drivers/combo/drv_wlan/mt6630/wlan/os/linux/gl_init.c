    ...
    )
{
    ...
    do {
        ...
        wlanBindBssIdxToNetInterface(prGlueInfo, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex, (PVOID)prWdev->netdev);
#else
        wlanBindBssIdxToNetInterface(prGlueInfo, prGlueInfo->prAdapter->prAisBssInfo->ucBssIndex, (PVOID)prWdev->netdev);
    ...
    )
{
    ...
    prGlueInfo->prDevHandler = alloc_netdev_mq(sizeof(NETDEV_PRIVATE_GLUE_INFO), NIC_INF_NAME, ether_setup, CFG_MAX_TXQ_NUM);
...
wlanGetIPV4Address(
    IN  P_GLUE_INFO_T prGlueInfo,
    OUT PUINT_8 pucIpv4Addrs,
    OUT PUINT_32 pu4NumOfIpv4Addr
)
{
    //TODO: currently, only copy one IP address
    ...
    kalMemCopy(pucIpv4Addrs, &(((struct in_device *)(prDev->ip_ptr))->ifa_list->ifa_local), 4);
    DBGLOG(INIT, INFO, ("ip is %d.%d.%d.%d\n",
            pucIpv4Addrs[0],pucIpv4Addrs[1],pucIpv4Addrs[2],pucIpv4Addrs[3]));
...
wlanGetIPV6Address(
    IN  P_GLUE_INFO_T prGlueInfo,
    PUINT_8 pucIpv6Addrs,
    OUT PUINT_32 pu4NumOfIpv6Addr
)
{
    //TODO: currently, only copy one IP address
    //
    ...
    kalMemCopy(pucIpv6Addrs, &(((struct in_device *)(prDev->ip6_ptr))->ifa_list->ifa_local), 16);

    DBGLOG(INIT, INFO, ("ipv6 is %d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d\n",
                pucIpv6Addrs[0],pucIpv6Addrs[1],pucIpv6Addrs[2],pucIpv6Addrs[3],
                pucIpv6Addrs[4],pucIpv6Addrs[5],pucIpv6Addrs[6],pucIpv6Addrs[7],
                pucIpv6Addrs[8],pucIpv6Addrs[9],pucIpv6Addrs[10],pucIpv6Addrs[11],
                pucIpv6Addrs[12],pucIpv6Addrs[13],pucIpv6Addrs[14],pucIpv6Addrs[15]
                ));
...
int set_p2p_mode_handler(struct net_device *netdev, PARAM_CUSTOM_P2P_SET_STRUC_T p2pmode) {
	...
	kalPrint("set_p2p_mode_handler ret = 0x%08lx\n", (UINT_32)rWlanStatus);
