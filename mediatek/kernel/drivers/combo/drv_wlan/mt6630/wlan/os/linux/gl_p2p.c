    ...
    )
{
    ...
    do {
        ...
        if (prGlueInfo->prP2PInfo == NULL) {
            ...
            // TODO: It can be moved to the inteface been created.
/*            
            for (u4Idx = 0; u4Idx < BSS_P2P_NUM; u4Idx++) {
                prWifiVar->aprP2pRoleFsmInfo[u4Idx] = kalMemAlloc(sizeof(P2P_ROLE_FSM_INFO_T),VIR_MEM_TYPE);
            }
*/
    ...
    )
{
    ...
    *((P_GLUE_INFO_T *) wiphy_priv(prGlueInfo->prP2PInfo->wdev.wiphy)) = prGlueInfo;
    ...
    /* 3. allocate netdev */
    prGlueInfo->prP2PInfo->prDevHandler = alloc_netdev_mq(sizeof(NETDEV_PRIVATE_GLUE_INFO), prDevName, ether_setup, CFG_MAX_TXQ_NUM);
...
//    prGlueInfo->prP2PInfo->prDevHandler->wireless_handlers    = &mtk_p2p_wext_handler_def;
    ...
    /* 11. Currently wpasupplicant can't support create interface. so initial the corresponding data structure here. */
    wlanBindBssIdxToNetInterface(prGlueInfo, prNetDevPriv->ucBssIdx, (PVOID)prGlueInfo->prP2PInfo->aprRoleHandler[0]);
