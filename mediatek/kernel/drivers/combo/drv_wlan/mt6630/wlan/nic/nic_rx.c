    ...
    )
{
    ...
    prSWRfb->pucPayload = (PUINT_8)((UINT_32)((P_UINT_8)prSWRfb->pvHeader + prSWRfb->u2HeaderLen));
    prSWRfb->u2PayloadLength = (UINT_16)(HAL_RX_STATUS_GET_RX_BYTE_CNT(prRxStatus)-((UINT_32)prSWRfb->pucPayload-(UINT_32)prRxStatus));
    ...
    if(RXM_IS_QOS_DATA_FRAME(u2FrameCtrl)){  /* WLAN header shall exist when doing duplicate detection */
        if(prSwRfb->prStaRec->aprRxReorderParamRefTbl[prSwRfb->ucTid]){
            ...
            */
            ...
            if(g_prMqm->arRxBaTable[prSwRfb->prStaRec->aucRxBaTable[prSwRfb->ucTID]].ucStatus == BA_ENTRY_STATUS_DELETING){
                DBGLOG(RXM, LOUD, ("RXM: SC= 0x%X (Cache[%d] updated) with DELETING BA ****************\n",\
                    u2SequenceControl, prSwRfb->ucTID));
            }
            #endif
            ...
            if (prSwRfb->prStaRec->ucDesiredPhyTypeSet & (PHY_TYPE_BIT_HT | PHY_TYPE_BIT_VHT)) {
                ...
                if(prSwRfb->ucTid < 8){ /* UP = 0~7 */
                    ...
                    DBGLOG(RX, WARN, ("RXM: (Warning) Unkown QoS Data with TID=%d\n",\
                        prSwRfb->ucTid));
    ...
    )
{
    ...
    if (fgDrop == FALSE){
        ...
        u4TcpUdpIpCksStatus = *((PUINT_32)((UINT_32)prRxStatus +
                (UINT_32)(ALIGN_4(prRxStatus->u2RxByteCount))));
    ...
    case EVENT_ID_802_11_PMKID:
        {
            ...
            kalMemCopy(cp, (P_EVENT_PMKID_CANDIDATE_LIST_T)(prEvent->aucBuffer), prEvent->u2PacketLength - 8);
...
#endif
#if CFG_ENABLE_BT_OVER_WIFI
            else if(GET_BSS_INFO_BY_INDEX(prAdapter, prEventBssBeaconTimeout->ucBssIndex)->eNetworkType == NETWORK_TYPE_BOW) {
            }
#endif
            else {
        ...
        if (prAdapter->rRxCtrl.u4RxPktsDumpTypeMask & BIT(HIF_RX_PKT_TYPE_MANAGEMENT)) {
            if (u2TxFrameCtrl == MAC_FRAME_BEACON ||
                ...
                DBGLOG(SW4, INFO, ("QM RX MGT: net %u sta idx %u wlan idx %u ssn %u ptype %u subtype %u 11 %u\n",
                    ...
                    prSwRfb->ucStaRecIdx,
                    prSwRfb->ucWlanIdx,
                    prWlanMgmtHeader->u2SeqCtrl,  /* The new SN of the frame */
    ...
    )
{
    ...
    #if DBG
            dumpMemory8((PUINT_8)prRxStatus, (HAL_RX_STATUS_GET_RX_BYTE_CNT(prRxStatus) > 4096) ? 4096 : prRxStatus->u2RxByteCount);
    #endif
            ASSERT(0);
        }
    ...
    )
{
    ...
    do {
        ...
        u4HwAppendDW = *((PUINT_32)((UINT_32)prRxStatus +
            (UINT_32)(ALIGN_4(prRxStatus->u2RxByteCount))));
...
#if CFG_SUPPORT_MULTITHREAD
    set_bit(GLUE_FLAG_RX_BIT, &(prAdapter->prGlueInfo->u4Flag));
    ...
    )
{
    ...
    if (fgRobustAction && prSwRfb->prStaRec && 
        ...
        if (prAisSpecBssInfo->fgMgmtProtection && (HAL_RX_STATUS_GET_SEC_MODE(prSwRfb->prRxStatus) != CIPHER_SUITE_BIP)) {
            DBGLOG(RSN, TRACE, ("[MFP] Not handle and drop un-protected robust action frame!!\n"));
            return WLAN_STATUS_INVALID_PACKET;
        }
    }
