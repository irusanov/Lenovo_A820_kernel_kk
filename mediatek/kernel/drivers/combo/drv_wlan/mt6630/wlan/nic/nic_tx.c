    ...
    )
{
    ...
    switch(prCmdInfo->eCmdType) {
        ...
        case COMMAND_TYPE_MANAGEMENT_FRAME:
            ucTC = ((P_MSDU_INFO_T)prCmdInfo->prPacket)->ucTC;
            break;
...
*/
    ...
    if(prMsduInfo->fgIsTXDTemplateValid) {
        ...
        if(HAL_MAC_TX_DESC_IS_LONG_FORMAT(prTxDescTemplate)) {
            *pucTxDescLength = NIC_TX_DESC_LONG_FORMAT_LENGTH;
        }
        else {
            *pucTxDescLength = NIC_TX_DESC_SHORT_FORMAT_LENGTH;
        }
    ...
    /* Compose TXD by Msdu info */
        ...
        *pucTxDescLength = NIC_TX_DESC_LONG_FORMAT_LENGTH;
        nicTxComposeDesc(prAdapter, prMsduInfo, *pucTxDescLength, FALSE, prTxDescBuffer);
    }
...
*/
    ...
    HAL_MAC_TX_DESC_SET_TX_BYTE_COUNT(
        prTxDesc, 
        *pucTxDescLength + NIC_TX_DESC_PADDING_LENGTH + prMsduInfo->u2FrameLength);
    ...
    )
{
    ...
    if(prQue->u4NumElem > 0) {
        ...
        while(prMsduInfo) {
            ...
            if (prMsduInfo->eSrc == TX_PACKET_MGMT) {
                ...
                if (prMsduInfo->pfTxDoneHandler == NULL) {
                    ...
                    KAL_SPIN_LOCK_DECLARATION();
                    DBGLOG(INIT, TRACE,("Wait WIDX:PID[%u:%u] SEQ[%u]\n", 
        ...
        // return
        nicTxReturnMsduInfo(prAdapter, (P_MSDU_INFO_T)QUEUE_GET_HEAD(prFreeQueue));
    }
    ...
    )
{
    ...
    if(prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME) {
        ...
        if (prStaRec) {
            ...
            DBGLOG(TX, TRACE, ("TX SEC Frame: BSS[%u] STA_REC[%u] WLAN_IDX[DEFAULT] LEN[%u]\n", 
                prCmdInfo->ucBssIndex,
        ...
        DBGLOG(TX, TRACE, ("TX MGMT Frame: BSS[%u] WIDX:PID[%u:%u] SEQ[%u] STA[%u] LEN[%u]\n", 
            prCmdInfo->ucBssIndex,
            prMsduInfo->ucWlanIndex,
            prMsduInfo->ucPID,
            prMsduInfo->ucTxSeqNum,
    ...
    )
{
    ...
    if(prDataPort1->u4NumElem) {
        ...
        if(prDataPort1->u4NumElem > nicTxGetFreeCmdCount(prAdapter)) {
