    ...
    )
{
    ...
    ASSERT(QM_INITIAL_RESIDUAL_TC_RESOURCE);
#endif
    ...
    )
{
    ...
    do{
        ...
        ucPacketType = TX_PACKET_TYPE_DATA;
        ...
        if(IS_BSS_ACTIVE(prBssInfo)) {
            ...
            switch (prCurrentMsduInfo->ucStaRecIndex){
                case STA_REC_INDEX_BMCAST:
                    ...
                    ucTC = arNetwork2TcResource[prCurrentMsduInfo->ucBssIndex][NET_TC_NON_STAREC_NON_QOS_INDEX];
                ...
                default:
                    ...
                    do {
                        ...
                        if (prStaRec->fgIsQoS){
                            if(prCurrentMsduInfo->ucUserPriority < TX_DESC_TID_NUM) {
                                eAci = aucTid2ACI[prCurrentMsduInfo->ucUserPriority];
                                prTxQue = &prStaRec->arTxQueue[aucACI2TxQIdx[eAci]];
                                ucTC = arNetwork2TcResource[prCurrentMsduInfo->ucBssIndex][eAci];
                            }
                            else {
                                ...
                                eAci = WMM_AC_BE_INDEX;
                                ASSERT(0);
                                DBGLOG(QM, TRACE, ("Packet TID is not in [0~7]\n"));
                            }
             ...
             if((prCurrentMsduInfo->eSrc == TX_PACKET_FORWARDING) && (prTxQue->u4NumElem > 32)) {
        ...
        */
        ...
        if(prStaRec && prStaRec->aprTxDescTemplate[prCurrentMsduInfo->ucUserPriority]) {
            prCurrentMsduInfo->fgIsTXDTemplateValid = TRUE;
        }
        else {
            if(prStaRec) {
                DBGLOG(QM, INFO, ("Cannot get TXD template for STA[%u] QoS[%u] MSDU UP[%u]\n", 
                    prStaRec->ucIndex, 
                    prStaRec->fgIsQoS,
                    prCurrentMsduInfo->ucUserPriority));
            }
            prCurrentMsduInfo->fgIsTXDTemplateValid = FALSE;
        }
...
qmDequeueTxPacketsFromPerStaQueues(
    ...
    OUT P_QUE_T         prQue,
    IN UINT_8           ucTC,
    IN UINT_32          u4CurrentQuota,
    IN UINT_32          u4TotalQuota
    )
{
    UINT_32         ucLoop;  /* Loop for */

    UINT_32         u4CurStaIndex;
    UINT_32         u4CurStaUsedResource;
    ...
    u4CurStaUsedResource = prQM->au4ForwardCount[ucTC];
    ...
    while(ucLoop < CFG_NUM_OF_STA_RECORD) {
        ...
        if(prStaRec->fgIsTxAllowed) {
            ...
            while(!QUEUE_IS_EMPTY(prCurrQueue)) {
                ...
                else if((u4CurStaForwardFrameCount >= u4MaxForwardFrameCountLimit) ||
    ...
    prQM->au4ForwardCount[ucTC] = u4CurStaUsedResource;   
    ...
    )
{
    ...
    DBGLOG(QM, LOUD, ("Enter qmDequeueTxPacketsFromPerStaQueues (TC = %u, quota = %u)\n", ucTC, u4CurrentQuota));
    ...
    )
{
    ...
    DBGLOG(QM, LOUD, ("Enter qmDequeueTxPacketsFromPerTypeQueues (TC = %d, quota = %u)\n", ucTC, u4CurrentQuota));
    ...
    )
{
    ...
    /* TC0 to TC5: AC0~AC3 (commands packets are not handled by QM) */
    for(i = TC5_INDEX; i >= TC0_INDEX; i--) {
        /* Skip AC4 */
        if(i == TC4_INDEX) {
            continue;
        }
        ...
        if(prAdapter->rQM.u4TxAllowedStaCount == 1) {
	...
	)
{
    ...
    while(prMsduInfo) {
        ...
        prTcqStatus->au2FreeBufferCount[prMsduInfo->ucTC] = (prTcqStatus->au2FreePageCount[prMsduInfo->ucTC] / NIC_TX_MAX_PAGE_PER_FRAME);      
	...
	)
{
    ...
    //4 <2> Adjust TcqStatus according to the updated prQM->au4CurrentTcResource
        ...
        for (i = 0; i < QM_ACTIVE_TC_NUM; i++){
            ai4ExtraQuota[i] = (INT_32)prTcqStatus->au2MaxNumOfBuffer[i] - (INT_32)prQM->au4CurrentTcResource[i];
    ...
    )
{
    ...
    for(u4Tc = 0; u4Tc < QM_ACTIVE_TC_NUM; u4Tc++) {
        ...
        if (prQM->au4AverageQueLen[u4Tc] == 0){
            prQM->au4AverageQueLen[u4Tc] = (u4CurrQueLen << QM_QUE_LEN_MOVING_AVE_FACTOR);
        }
        else{
            prQM->au4AverageQueLen[u4Tc] -= (prQM->au4AverageQueLen[u4Tc] >> QM_QUE_LEN_MOVING_AVE_FACTOR);
    ...
    if (prQM->au4AverageQueLen[TC5_INDEX] == 0){
        ...
        prQM->au4AverageQueLen[TC5_INDEX] -= (prQM->au4AverageQueLen[TC5_INDEX] >> QM_QUE_LEN_MOVING_AVE_FACTOR);
    ...
    )
{
    ...
    if(!prQM->au4CurrentTcResource[ucTc]) {
        ...
        DBGLOG(QM, INFO, ("Trigger TC Resource adjustment for TC[%u]\n", ucTc));
    }
}
#endif
    ...
    do{
        ...
        if (HAL_RX_STATUS_GET_HEADER_TRAN(prRxStatus) == TRUE) {//(!HIF_RX_HDR_GET_80211_FLAG(prHifRxHdr)){
            ...
            if (prCurrSwRfb->prStaRec == NULL) {
                ...
                if (prCurrSwRfb->prStaRec == NULL) {
                    DBGLOG(QM, TRACE, ("Mark NULL the Packet for StaRec == NULL, wlanIdx %d, but via Header Translation\n", prRxStatus->ucWlanIdx));
        ...
        if (prCurrSwRfb->fgReorderBuffer && !fgIsBMC) {
    ...
    )
{
    ...
    prSwRfb->ucTid = (*((PUINT_16)((PUINT_8)prBarCtrlFrame + CTRL_BAR_BAR_CONTROL_OFFSET)))>>BAR_CONTROL_TID_INFO_OFFSET;
                    ...
                    case VENDOR_OUI_SUBTYPE_WMM_INFO:
                        {
                            ...
                            prStaRec->fgIsUapsdSupported = ((ucQosInfoAC)? TRUE: FALSE) &
                                                IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucUAPSD);
...
//    if(prAdapter->rWifiVar.fgSupportUAPSD){
    if(IS_FEATURE_ENABLED(prAdapter->rWifiVar.ucUAPSD) && prStaRec->fgIsUapsdSupported){
    ...
    )
{
    ...
    for(eAci = 0; eAci < WMM_AC_INDEX_NUM; eAci++){
       ...
       *( ((PUINT_8)(&prIeWmmParam->ucAciAifsn_BE)) + (eAci <<2) ) = (UINT_8) (aucACI[eAci]
                                   | (prBssInfo->arACQueParmsForBcast[eAci].ucIsACMSet ? WMM_ACIAIFSN_ACM:0 )
    ...
    )
{
    ...
    for(i = TC0_INDEX; i <= TC5_INDEX; i++) {
        ...
        DBGLOG(SW4, INFO,("TC%u AvgQLen[%02u] minRsv[%02u] CurTcRes[%02u] GrtdTcRes[%02u]\n",
            i,
...
/*----------------------------------------------------------------------------*/
VOID
mqmRxModifyBaEntryStatus(
	IN P_ADAPTER_T     			prAdapter,
    IN P_RX_BA_ENTRY_T 			prRxBaEntry,
	...
	do {
		...
		if((!prAdapter->rWifiVar.fgSupportQoS) || \
		   ...
		   (!prStaRec->fgRxAmpduEn)){
			DBGLOG(QM, WARN, ("[Puff][%s]: (Warning) BA ACK Policy not supported fgSupportQoS(%d), fgSupportAmpduRx(%d), fgRxAmpduEn(%d)\n", \
				__func__, \
				prAdapter->rWifiVar.fgSupportQoS, \
				prAdapter->rWifiVar.fgSupportAmpduRx, \
				prStaRec->fgRxAmpduEn));
		...
		if((((rAddBaReqBody.u2BAParameterSet) & BA_PARAM_SET_ACK_POLICY_MASK) >> BA_PARAM_SET_ACK_POLICY_MASK_OFFSET)
		...
		if(fgIsReqAccepted){
			...
			if(!prRxBaEntry){
				...
				if(prQM->ucRxBaCount < CFG_NUM_OF_RX_BA_AGREEMENTS){
					...
					fgIsNewEntryAdded = qmAddRxBaEntry(
						prAdapter,
						(UINT_8)u4StaRecIdx,
						(UINT_8)u4Tid,
						u2WinStart,
		...
		DBGLOG(QM, WARN, ("[Puff][%s]: (Warning) ADDBA_RSP u4BuffSize(%d) StatusCode(%d) BAParameterSet(0x%08X) BATimeoutValue(%d) \n", 
			__func__, \
			u4BuffSize, \
		...
		if(fgIsNewEntryAdded){ /* Only when a new BA entry is indeed added will the host be notified */
			...
			if(!prSwRfbEventToHost){
				...
				*/
				...
				DBGLOG(MQM, INFO, ("MQM: (RX_BA) Event ADDBA ---> driver (STA=%ld TID=%ld WinStart=%d)\n", \
					u4StaRecIdx, u4Tid, (prEventRxAddBa->u2BAStartSeqCtrl >> 4)));

				/* Configure the SW_RFB for the Event packet */
				RXM_SET_EVENT_PACKET(\
				/* P_SW_RFB_T*/ 				(P_SW_RFB_T)prSwRfbEventToHost,\
				/* HIF RX Packet pointer */ 	(PUINT_8)prEventRxAddBa,\
