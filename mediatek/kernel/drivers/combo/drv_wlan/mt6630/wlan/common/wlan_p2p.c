    ...
    )
{
    ...
    *pu4SetInfoLen = u4SetBufferLen;
    ...
    if ((prBssInfo->ucBMCWlanIndex >= WTBL_SIZE)) {
        prBssInfo->ucBMCWlanIndex = secPrivacySeekForBcEntry(prAdapter, prBssInfo->ucBssIndex, prBssInfo->aucBSSID,
    ...
    if (rCmdKey.ucTxKey) {
        ...
        if (((rCmdKey.aucPeerAddr[0] & rCmdKey.aucPeerAddr[1] & rCmdKey.aucPeerAddr[2] &
             rCmdKey.aucPeerAddr[3] & rCmdKey.aucPeerAddr[4] & rCmdKey.aucPeerAddr[5]) == 0xFF) ||
             ((rCmdKey.aucPeerAddr[0] | rCmdKey.aucPeerAddr[1] | rCmdKey.aucPeerAddr[2] |
             rCmdKey.aucPeerAddr[3] | rCmdKey.aucPeerAddr[4] | rCmdKey.aucPeerAddr[5]) == 0x00)) {
                 rCmdKey.ucWlanIndex = 255; /* GC WEP ? */
                 ASSERT(FALSE);
        }
        else {
            if (prStaRec) { /* GC Rx RSN Group key */
                rCmdKey.ucWlanIndex = secPrivacySeekForBcEntry(prAdapter, prStaRec->ucBssIndex,
                              prStaRec->aucMacAddr, prStaRec->ucIndex, rCmdKey.ucAlgorithmId, rCmdKey.ucKeyId,
    ...
    )
{
    ...
    if (!prStaRec) {
        if (prAdapter->rWifiVar.rConnSettings.eAuthMode < AUTH_MODE_WPA && prAdapter->rWifiVar.rConnSettings.eEncStatus != ENUM_ENCRYPTION_DISABLED) {
            rCmdKey.ucWlanIndex = prBssInfo->ucBMCWlanIndex;
        }
        else {
    ...
    )
{
    ...
    if (u4QueryBufferLen!=0) {
        ASSERT(pvQueryBuffer);
                                                                                                   // TODO: FIXME
        *(PPARAM_POWER_MODE) pvQueryBuffer = (PARAM_POWER_MODE)(prAdapter->rWlanInfo.arPowerSaveMode[P2P_DEV_BSS_INDEX].ucPsProfile);
    ...
    )
{
    ...
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_UAPSD_PARAM_STRUC_T);
    ...
    prAdapter->rWifiVar.ucUAPSD = prUapsdParam->fgEnAPSD ? FEATURE_ENABLED: FEATURE_DISABLED;
