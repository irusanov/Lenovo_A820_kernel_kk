...
//	DBGLOG(AIS, INFO, ("BssErr: prStaRec->aucMacAddr = "MACSTR"\n",
//			MAC2STR(prStaRec->aucMacAddr)));

	/* count connection reject from AP */
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {

//		DBGLOG(AIS, INFO, ("BssErr: Check BSS "MACSTR"\n",
//				MAC2STR(prBssDesc->aucBSSID)));

		if (EQUAL_MAC_ADDR(prStaRec->aucMacAddr, prBssDesc->aucBSSID))
		{
			if (fgCountReset == TRUE)
			{
				prBssDesc->ucBssErrTime = 0;
				prBssDesc->ucBssErrCnt = 0;

				DBGLOG(AIS, INFO, ("BssErr: Reset Bss Err Count %d\n",
						prBssDesc->ucBssErrCntTotal));
			}
			else
			{
				GET_CURRENT_SYSTIME(&rCurrentTime);
				prBssDesc->ucBssErrTime = rCurrentTime; /* reset timeout */
				prBssDesc->ucBssErrCnt ++;
				prBssDesc->ucBssErrCntTotal ++; /* never be reset */

				DBGLOG(AIS, INFO, ("BssErr: BSS "MACSTR" error count %d\n",
						MAC2STR(prBssDesc->aucBSSID), prBssDesc->ucBssErrCnt));
			}
			break;
		}
	}
					...
					if (aisFsmStateInit_RetryJOIN(prAdapter, prStaRec) == FALSE) {
						...
						if (prBssDesc == NULL)
						{
							...
							/*
								...
								(2) We will do LINK_INITIALIZE(&prScanInfo->rBSSDescList); in nicUninitMGMT().
								(3) We will handle prMsduInfo->pfTxDoneHandler in nicTxRelease().
								(4) prMsduInfo->pfTxDoneHandler will point to saaFsmRunEventTxDone().
								(5) Then jump to saaFsmSteps() -> saaFsmSendEventJoinComplete()
								(6) Finally mboxSendMsg() -> aisFsmRunEventJoinComplete().
								(7) In aisFsmRunEventJoinComplete(), we will check "prBssDesc = scanSearchBssDescByBssid(prAdapter, prStaRec->aucMacAddr);"
								(8) And prBssDesc will be NULL and hangs in "ASSERT(prBssDesc->fgIsConnecting);" when DBG=0.
										ASSERT(prBssDesc);
...
*/
/*----------------------------------------------------------------------------*/
UINT32 IsrCnt = 0, IsrPassCnt = 0, TaskIsrCnt = 0;
VOID
    ...
    )
{
    ...
    if (fgDoAbortIndication) {
		...
		DBGLOG(AIS, INFO, ("Remove BSS entry ["MACSTR"]\n",
			MAC2STR(prAisBssInfo->aucBSSID)));
		scanRemoveBssDescByBssid(prAdapter, prAisBssInfo->aucBSSID);

		DBGLOG(INIT, INFO, ("fgDoAbortIndication. delay to indicate\n"));
    ...
    )
{
    ...
    do {
        ...
        if (rTxDoneStatus != TX_RESULT_SUCCESS) {
            printk("Mgmt Frame TX Fail, Status:%d.\n", rTxDoneStatus);
        }
        else {
            fgIsSuccess = TRUE;
    ...
    )
{
    ...
    do {
        ...
        if (prStaRec != NULL) {
            //printk("Mgmt with station record: "MACSTR" .\n", MAC2STR(prStaRec->aucMacAddr));
        }
