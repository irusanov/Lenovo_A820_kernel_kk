...
{
    EEMCS_EXCEPTION_STATE cur_mode = EEMCS_EX_INVALID;
...
static void ex_monitor_func(unsigned long data)
{
	...
	if ((eemcs_exception_state < EEMCS_EX_INIT_DONE) && \
		...
		snprintf(ex_info, EE_BUF_LEN,"\n[INVALID ASSERT] lte sdio exception handshake timeout(>2s)\n");
	} else if (eemcs_exception_state == EEMCS_EX_INIT_DONE) {
		snprintf(ex_info, EE_BUF_LEN,"\n[INVALID ASSERT] AP receive MD_EX msg timeout(>5s)\n");
	} else if (eemcs_exception_state == EEMCS_EX_MSG_OK) {
		snprintf(ex_info, EE_BUF_LEN,"\n[INVALID ASSERT] AP receive MD_EX_REC_OK msg timeout(>5s)\n");
	}

	eemcs_aed(NULL, 0, ex_info);
}
...
KAL_INT32 eemcs_expt_log_port(struct sk_buff *skb, KAL_UINT32 port_id)
{
    ...
    DBGLOG(EXPT, DBG, "PORT%d CCCI_H(0x%X, 0x%X, %d, 0x%X) add to expt list",\
		port_id, \
...
void eemcs_ccci_expt_log_df_q_pkts(void)
{
    ...
    for (i = 0; i < SDIO_RX_Q_NUM; i++) {
        ...
        if (!test_bit(i, (unsigned long *)&nonstop_rxq)) {
            ...
            if (pkts == 0) {
                DBGLOG(EXPT, WAR, "Nothing in DL SWQ %d", i);
                continue;
            } else {
...
KAL_INT32 eemcs_expt_flush()
{
	...
	for (i = 0; i < CCCI_PORT_NUM; i++) {
		...
		for (j = 0; j < pkts; j++) {
			...
			if (skb == NULL) {
				DBGLOG(EXPT, WAR, "dequeue NULL skb from port%d list", i);
			} else {
...
#else
    DEBUG_LOG_FUNCTION_ENTRY;
    DBGLOG(EXPT, TRA, "[EXPT] Exception Callback (%d)", msg_id);
    DEBUG_LOG_FUNCTION_LEAVE;
#endif    
}
