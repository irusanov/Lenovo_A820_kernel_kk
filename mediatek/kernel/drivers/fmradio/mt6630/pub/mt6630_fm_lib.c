...
static struct fm_patch_tbl mt6630_patch_tbl_tx[5] = {
    {FM_ROM_V1, "/etc/firmware/mt6630/mt6630_fm_v1_patch_tx.bin", "/etc/firmware/mt6630/mt6630_fm_v1_coeff_tx.bin", NULL, NULL},
//    {FM_ROM_V2, "/etc/firmware/mt6630/mt6630_fm_v2_patch_tx.bin", "/etc/firmware/mt6630/mt6630_fm_v2_coeff_tx.bin", NULL, NULL},
//    {FM_ROM_V3, "/etc/firmware/mt6630/mt6630_fm_v3_patch_tx.bin", "/etc/firmware/mt6630/mt6630_fm_v3_coeff_tx.bin", NULL, NULL},
//    {FM_ROM_V4, "/etc/firmware/mt6630/mt6630_fm_v4_patch_tx.bin", "/etc/firmware/mt6630/mt6630_fm_v4_coeff_tx.bin", NULL, NULL},
//    {FM_ROM_V5, "/etc/firmware/mt6630/mt6630_fm_v5_patch_tx.bin", "/etc/firmware/mt6630/mt6630_fm_v5_coeff_tx.bin", NULL, NULL},
};
...
static fm_u32 fm_sant_flag = 0;//1,Short Antenna;  0, Long Antenna
...
static fm_s32 mt6630_pwron(fm_s32 data)
{
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_FM)) {
        FM_LOG_ERR(CHIP, "WMT turn on FM Fail!\n");
        return -FM_ELINK;
    } else {
        FM_LOG_NTC(CHIP, "WMT turn on FM OK!\n");
        return 0;
    }
}
...
static fm_s32 mt6630_pwroff(fm_s32 data)
{
    if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_FM)) {
        FM_LOG_ERR(CHIP, "WMT turn off FM Fail!\n");
        return -FM_ELINK;
    } else {
...
static fm_s32 mt6630_RampDown(void)
{
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "ramp down failed\n");
    }
...
static fm_s32 mt6630_get_patch_path(fm_s32 ver, const fm_s8 **ppath, struct fm_patch_tbl *patch_tbl)
{
    fm_s32 i;
    ...
    for (i = max; i > 0; i--) 
    {
        if (fm_file_exist(patch_tbl[i-1].patch) == 0) 
        {
            *ppath = patch_tbl[i-1].patch;
            FM_LOG_ERR(CHIP, "undefined ROM version\n");
            return 0;
        }
    }

    // get path failed
    FM_LOG_ERR(CHIP, "No valid patch file\n");
    return -FM_EPATCH;
}


static fm_s32 mt6630_get_coeff_path(fm_s32 ver, const fm_s8 **ppath, struct fm_patch_tbl *patch_tbl)
{
    fm_s32 i;
    fm_s32 max = FM_ROM_MAX;
    ...
    // get path failed
    FM_LOG_ERR(CHIP, "No valid coeff file\n");
    return -FM_EPATCH;
}
    ...
    switch (type) 
    {
        ...
        case IMG_PATCH:
            ...
            for (seg_id = 0; seg_id < seg_num; seg_id++) 
            {
                ...
                if (ret) 
                {
                    FM_LOG_ERR(CHIP, "mt6630_patch_download failed\n");
                    return ret;
                }
            }
        ...
        case IMG_COEFFICIENT:
            ...
            for (seg_id = 0; seg_id < seg_num; seg_id++) {
                ...
                if (ret) {
                    FM_LOG_ERR(CHIP, "mt6630_coeff_download failed\n");
                    return ret;
                }
            }
...
static fm_s32 mt6630_pwrup_top_setting(void)
{
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.1 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x81020008, value|0x00000030);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.1 wr failed\n");
        return ret;
    }
    //A0.2
    Delayus(20);
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.3 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x81020008, value|0x00000001);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.3 wr failed\n");
        return ret;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.4 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x80000108, value|0x00000100);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " A0.4 wr failed\n");
        return ret;
    }
    return ret;
}
...
static fm_s32 mt6630_pwrdown_top_setting(void)
{
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.1 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x80000104, value|0x00000100);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.1 wr failed\n");
        return ret;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.2 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x81020008, value&0xFFFFFFFE);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.2 wr failed\n");
        return ret;
    }
    //B0.3
    Delayus(20);
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.4 rd failed\n");
        return ret;
    }
    ret = mt6630_host_write(0x81020008, value&0xFFFFFFEF);
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " B0.4 wr failed\n");
        return ret;
    }
    return ret;
}
static fm_s32 mt6630_pwrup_DSP_download(struct fm_patch_tbl *patch_tbl)
{
    ...
    {
        FM_LOG_ERR(CHIP, "get ROM version failed\n");
	    //ret=-4 means signal got when control FM. usually get sig 9 to kill FM process.
	    //now cancel FM power up sequence is recommended.
        return ret;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " mt6630_get_patch_path failed\n");
        goto out;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " DL DSPpatch failed\n");
        goto out;
    }
    ...
    if(ret == -FM_EPATCH)
    {
        FM_LOG_ERR(CHIP, " mt6630_get_coeff_path failed\n");
        goto out;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, " DL DSPcoeff failed\n");
        goto out;
    }
...
static fm_s32 mt6630_PowerUp(fm_u16 *chip_id, fm_u16 *device_id)
{
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "packaging rd failed\n");
    }
	else
	{
		fm_packaging = (reg & 0x00008000)>>15;
		FM_LOG_NTC(CHIP, "fm_packaging: %d\n", fm_packaging);
	} 
    ret = mt6630_pwrup_top_setting();
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "mt6630_pwrup_top_setting failed\n");
        return ret;
    }
    if (FM_LOCK(cmd_buf_lock)) return (-FM_ELOCK);
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "mt6630_pwrup_clock_on failed\n");
        return ret;
    }
    //read HW version
    ...
    *device_id = tmp_reg;
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "mt6630_pwrup_DSP_download failed\n");
        return ret;
    }

    if ((mt6630_fm_config.aud_cfg.aud_path == FM_AUD_MRGIF)||(mt6630_fm_config.aud_cfg.aud_path == FM_AUD_I2S))
    {
        mt6630_I2s_Setting(FM_I2S_ON, mt6630_fm_config.aud_cfg.i2s_info.mode, mt6630_fm_config.aud_cfg.i2s_info.rate);
    ...
    FM_UNLOCK(cmd_buf_lock);
    if (ret) {
        FM_LOG_ERR(CHIP, "mt6630_pwrup_digital_init failed\n");
        return ret;;
    }
...
static fm_s32 mt6630_PowerDown(void)
{
    ...
    if (ret) {
        FM_LOG_ERR(CHIP, "mt6630_pwrdown failed\n");
        return ret;
    }
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "mt6630_pwrdown_top_setting failed\n");
        return ret;
    }
...
static fm_bool mt6630_SetFreq(fm_u16 freq)
{
    ...
    if (ret) {
        FM_LOG_ERR(CHIP, "mt6630_tune failed\n");
        return ret;
    }
...
static fm_s32 mt6630_full_cqi_get(fm_s32 min_freq, fm_s32 max_freq, fm_s32 space, fm_s32 cnt)
{
    ...
    for (k = 0; (10000 == orig_freq) && (0xffffffff == g_dbg_level) && (k < cnt); k++)
    {
        ...
        for (j = 0; j < num; j++) 
        {
            ...
            {
                FM_LOG_ERR(CHIP, "smt get CQI failed\n");
                ret = -1;
            }
            freq += space_val;
        }
...
static fm_s32 MT6630_Rds_Tx(fm_u16 pi, fm_u16 *ps, fm_u16 *other_rds, fm_u8 other_rds_cnt)
{
	...
	WCN_DBG(FM_NTC | RDSC, "+%s():PI=0x%04x, PS=0x%04x/0x%04x/0x%04x/0x%04x, other_rds_cnt=%d \n",__func__, pi, ps[0], ps[1], ps[2], ps[3],other_rds_cnt);
    if (FM_LOCK(cmd_buf_lock)) 
		return (-FM_ELOCK);
		                        ...
		                        fm_u16 space)
{
    ...
    if(scandir == FM_TX_SCAN_UP)
    {
        for (i=((*pFreq-min_freq)/step); i<total_no; i++)
        {
            ...
            ret = mt6630_soft_mute_tune(freq,&rssi,&valid);
            if(ret == fm_false)
            {
...
static fm_s32 mt6630_PowerDownTx(void)
{
    ...
    if (ret) 
    {
        FM_LOG_ERR(CHIP, "mt6630_PowerDownTx failed\n");
        return ret;
    }
...
fm_s32 MT6630fm_low_ops_register(struct fm_lowlevel_ops *ops)
{
    ...
    if (!cmd_buf) {
        FM_LOG_ERR(CHIP, "6630 fm lib alloc tx buf failed\n");
        ret = -1;
    }
